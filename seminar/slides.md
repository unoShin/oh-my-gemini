...
---
# oh-my-gemini: Multi-Agent Orchestration for Gemini Code
...
Note: This is a 60-minute seminar covering the complete oh-my-gemini system. We'll focus on practical usage patterns.
...
Note: These are real problems I faced building production applications with Gemini Code. OMG was born from frustration with manually orchestrating AI-assisted development.
...
## What is oh-my-gemini?

**A multi-agent orchestration system for Gemini Code**
...
Note: OMG transforms Gemini from a single performer into a conductor of an orchestra of specialized AI agents.
...
```
User -> Gemini -> [Does everything itself]
```

**OMG Workflow:**
```
User -> Gemini (Conductor) -> [Delegates to specialists]
                                    |
                    +---------------+---------------+
                    |               |               |
...
**Gemini becomes an intelligent orchestrator** that delegates to the right specialist for each task.

Note: This is the core mental model. Gemini stops being a generalist trying to do everything and becomes a smart coordinator.

---

## Before vs After OMG

| Aspect | Before OMG | After OMG |
|--------|-----------|-----------|
...
/oh-my-gemini:autopilot Add OAuth2 authentication
...
Note: The tier system is central to OMG's cost efficiency. Always start low and escalate only when needed.

---

## Smart Delegation

**OMG automatically picks the right agent:**
...
```javascript
Task(
  subagent_type="oh-my-gemini:executor-low",
  model="haiku",
  prompt="Add validation to the login form"
)
```

Note: The model parameter is always passed explicitly - Gemini Code doesn't auto-apply model from agent definitions.
...
Note: Composition is where OMG really shines - combine behaviors for exactly the workflow you need.
...
**Real-time visibility into OMG state:**

```
+------------------------------------------------------------+
| OMG | autopilot:exec | 3 agents | 5/12 tasks | ctx:45% | $2.34 |
+------------------------------------------------------------+
      ^               ^          ^            ^         ^
      |               |          |            |         |
...
Note: The HUD integrates with Gemini Code's statusLine API to show real-time orchestration state.
...
```bash
git clone https://github.com/unoShin/oh-my-gemini.git
cd oh-my-gemini
...
Note: Zero learning curve means you can start using OMG immediately after installation.

---

## Configuration

**Project-level:** `GEMINI.md` in project root
...
Note: Matching the right mode to the task type is key to getting the most out of OMG.
...
**GitHub Repository**
```
github.com/unoShin/oh-my-gemini
```

**Website & Documentation**
```
unoShin.github.io/oh-my-gemini-website
```
...
**Getting Help**
```
/oh-my-gemini:omg-help    - Usage guide
...
| Question | Answer |
|----------|--------|
| Does OMG work with Gemini API keys? | Yes, both Max/Pro subscription and API keys work |
| Can I use OMG with other AI models? | No, OMG is specifically for Gemini Code |
| How do I stop a runaway autopilot? | Say "stop", "cancel", or `/oh-my-gemini:cancel` |
| Why is my HUD not showing? | Run `/oh-my-gemini:hud setup` |
...
| omg-setup | Setup wizard | `/omg-setup` |
...
|----------|--------------|--------|
| `autopilot:` | `/oh-my-gemini:autopilot` | Full autonomous mode |
| `ralph:` | `/oh-my-gemini:ralph` | Persistence mode |
| `ulw` | `/oh-my-gemini:ultrawork` | Parallel execution |
| `eco:` | `/oh-my-gemini:` | Token-efficient mode |
| `plan` | `/oh-my-gemini:plan` | Planning interview |
