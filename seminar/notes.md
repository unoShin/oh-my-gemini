...
- "Right now, when you use Gemini Code or similar tools, you're essentially getting a very smart intern. They can do anything you ask, but YOU have to manage the workflow."
...
**The Reveal**
- "That's exactly what oh-my-gemini does. It transforms Gemini Code from a single generalist assistant into a coordinated team of 28 specialists."
- "Today, I'm going to show you how this changes everything about AI-assisted development."

### Transition
"Let me start by showing you exactly what OMG is and how it thinks differently about AI assistance."
...
## Section 2: What is OMG? (8 min, Slides 4-8)

### Opening Line
"The fundamental insight behind OMG is simple: your AI should be a conductor, not a performer."
...
- "That's the shift OMG makes. Gemini becomes the conductor, and 28 specialized agents are the orchestra."
- "When you say 'build a feature,' Gemini doesn't do it all personally. It delegates: Explorer finds relevant code, Architect designs the solution, Executor implements, QA Tester verifies."

**Before vs After** (Slide 5)
- "BEFORE: 'Gemini, search for auth code.' [wait] 'Now analyze it.' [wait] 'Now write tests.' [wait] You're the project manager micromanaging every step."
- "AFTER: 'Build authentication for the API.' OMG automatically: explores codebase, analyzes requirements, generates plan, implements in parallel, writes tests, verifies. You're the product owner stating the goal."
...
- "2. Gemini detects intent: This is a parallel execution task"
...
### Opening Line
"Execution modes are where OMG's power becomes concrete. Each mode is optimized for a specific type of work. Let's explore all five."
...
### Transition
"These modes are powered by OMG's agent system. Let's look at how those 28 specialists are organized."

### Audience Engagement
- "Quick question: Which mode sounds most useful for YOUR daily work?" (Take 2-3 responses)
- "The beauty is you don't have to memorize this. Say 'fast parallel fixes' and OMG activates ultrawork. Say 'efficient batch fixes' and it activates ."
...
- "OMG organizes agents into 13 specializations:"
...
- "Each domain has up to three tiers corresponding to Gemini model versions:"
...
- "How does OMG decide which agent to use? Three factors:"
...
### Opening Line
"Enough talking about it. Let's see OMG in action across five different scenarios."

### Demo 1: Autopilot - Full Feature Build (5 min, Slide 29)

**Setup**
- Have terminal ready with OMG installed
...
- Could be the OMG codebase itself with intentional errors
...
### Opening Line
"Powerful technology is useless if it's hard to use. OMG is designed for zero learning curve."
...
```
[OMG] Mode: ultrawork | Agents: 3 active | executor-1: fixing auth.ts | executor-2: fixing api.ts | architect: reviewing
```
- "At a glance you know:"
- "  Which mode is active"
- "  How many agents are working"
- "  What each agent is doing"
- "Installation: `/oh-my-gemini:hud setup` - adds to your shell prompt"

**Notepad Wisdom** (Slide 36)
- "OMG learns from every session via the notepad system:"
...
- "These persist across sessions. Your OMG gets smarter over time."

**Cost Analytics** (Slide 37)
- "OMG tracks token usage per session:"
...
### Key Points
- Requires Gemini Code CLI installed
...
- "You need Gemini Code installed: `npm install -g gemini-cli`"
- "You need a Gemini subscription (Pro or Team) or an API key"
...
gemini-cli "/oh-my-gemini:omg-setup"

# Step 3: Start using it
gemini-cli "autopilot: build me a todo app"
```
...
### Transition
"That's OMG. Let's recap and open for questions."
...
### Opening Line
"Let's recap what makes OMG transformative."
...
- "OMG transforms Gemini Code from a single assistant into a coordinated team."
...
- "If you like it, share it with your team. OMG shines with real-world complexity."
...
**Answer:**
"OMG itself is completely free - it's MIT licensed open-source. What you pay for is the Gemini API usage.

You need either:
- A Gemini Pro subscription ($20/month) which includes API access
- Or a Gemini Team subscription with API credits
- Or direct API access via Anthropic

The key cost benefit: OMG's smart model routing saves you 30-50% on token costs compared to manually using Gemini. For example, simple searches use Haiku (super cheap), complex debugging uses Opus (expensive but necessary). Without OMG, everything might default to Opus.
...
**Answer:**
"Currently OMG is designed specifically for Gemini Code and leverages Gemini's three model tiers - Haiku, Sonnet, and Opus.

The architecture relies on Gemini's specific capabilities for the multi-agent orchestration. We don't support GPT-4, Gemini, or other models at this time.
...
### 3. "How is this different from just using Gemini Code?"

**Answer:**
"Great question. Without OMG, Gemini Code gives you one very smart generalist assistant. You tell it every step: 'search for this, analyze that, now implement this, now test that.'

With OMG, you get 28 specialized agents orchestrated automatically. You state the goal - 'build authentication' - and OMG:
...
Real-world impact: Tasks that took 30 minutes of back-and-forth with Gemini Code now take 5 minutes of autonomous execution with OMG."

### 4. "What about security? Is my code safe?"

**Answer:**
"OMG runs entirely locally via Gemini Code. Your code never leaves your machine except through the normal Gemini API calls that you'd be making anyway.

Additionally, OMG includes a security-reviewer agent that can audit code for common vulnerabilities. You can invoke it explicitly: '/pipeline security' runs a security audit pipeline.
...
Place custom agent definitions in `~/.gemini/agents/{agent-name}.md` and they'll override the defaults.

For example, if you want a specialized Python testing agent:
```markdown
# ~/.gemini/agents/pytest-specialist.md
You are an expert in pytest and Python testing best practices.
Focus on: fixtures, parametrization, mocking with pytest-mock.
```

Then invoke: `Task(subagent_type="oh-my-gemini:pytest-specialist")`

You can also customize execution modes, delegation categories, and model routing rules via the config file at `~/.gemini/.omg-config.json`.
...
**Answer:**
"Yes. OMG works with any language that Gemini Code supports - which is basically all mainstream languages.
...
**Answer:**
"Honestly? You don't need to think about it. Just describe what you want in natural language and OMG auto-detects the right mode.
...
But more seriously - OMG has built-in verification at multiple levels:
...
In practice, failures happen - maybe a test fails, maybe there's a linting error. OMG catches these and fixes them before claiming completion.
...
**Answer:**
"OMG is designed for interactive development, not CI/CD automation.
...
For CI/CD, you'd use Gemini Code's built-in capabilities or traditional CI tools.

That said, some teams use OMG-generated tests in their CI pipeline. The tests themselves are standard - jest, pytest, etc. - they just happened to be generated via OMG.
...
OMG: Literally zero learning time. If you can describe what you want in English, you can use OMG.
...
"Let me show you the mental model shift. BEFORE OMG: [read the before section slowly]. You're the micromanager. AFTER OMG: [read the after section with rising energy]. You're the product owner. This isn't a 10% improvement - this is a complete paradigm shift."
...
3. Feel confident they could install and use OMG today
4. Be excited about the paradigm shift from interactive to autonomous

**Your Success Metric:**
"How many people install OMG in the next week?"
