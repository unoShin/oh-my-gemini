...
# Step 2: Run quick OMG test
log_step "Step 2/2: Quick OMG test ($TEST_LIMIT instances)..."
echo ""
"$SCRIPT_DIR/run_omg.sh" --limit $TEST_LIMIT --model "$MODEL" --timeout $TIMEOUT
OMG_STATUS=$?
echo ""
...
if [ $VANILLA_STATUS -eq 0 ] && [ $OMG_STATUS -eq 0 ]; then
    ...
    [ $OMG_STATUS -ne 0 ] && log_error "  OMG test: FAILED (exit code $OMG_STATUS)"
    echo ""
