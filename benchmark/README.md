...
Automated benchmark comparison between vanilla Gemini Code and OMG-enhanced Gemini Code.
...
- Tests both vanilla and OMG modes
...
### run_vanilla.sh
Run vanilla Gemini Code benchmark:
- Standard Gemini Code without OMG
...
- `--model MODEL` - Gemini model to use (default: gemini-sonnet-4-6-20260217)
...
### run_omg.sh
Run OMG-enhanced benchmark:
- Gemini Code with oh-my-gemini orchestration
- Saves predictions to `predictions/omg/`
- Logs to `logs/omg_*.log`
...
**Examples:**
```bash
./run_omg.sh                    # Full benchmark
./run_omg.sh --limit 100        # First 100 instances
```
...
- `--skip-omg` - Skip OMG benchmark run
...
./run_full_comparison.sh --skip-vanilla     # Only run OMG (reuse vanilla results)
```
...
│   └── omg_*.log            # OMG run logs
├── results/
│   ├── vanilla_results.json  # Vanilla evaluation
│   ├── omg_results.json      # OMG evaluation
...
4. **Review Results**:
   ...
   - Inspect predictions in `predictions/vanilla/` and `predictions/omg/`
   - Review logs in `logs/`
...
### Custom Model
```bash
./run_vanilla.sh --model gemini-opus-4-6-20260205
./run_omg.sh --model gemini-opus-4-6-20260205
```
...
2. **Parallel Runs**: Don't run vanilla and OMG in parallel (share API rate limits)
