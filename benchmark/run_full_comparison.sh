...
LIMIT=""
SKIP=""
MODEL="gemini-sonnet-4-6-20260217"
TIMEOUT="300"
SKIP_VANILLA=false
SKIP_OMG=false
SKIP_EVAL=false
...
if [ "$SKIP_VANILLA" = false ]; then
    log_step "Step 1/4: Running vanilla Gemini Code benchmark..."
    echo ""
...
# Step 2: Run OMG benchmark
if [ "$SKIP_OMG" = false ]; then
    log_step "Step 2/4: Running OMG-enhanced benchmark..."
    echo ""
    "$SCRIPT_DIR/run_omg.sh" $ARGS
    if [ $? -ne 0 ]; then
        log_error "OMG benchmark failed. Aborting."
        exit 1
    fi
    echo ""
else
    log_warn "Skipping OMG benchmark (--skip-omg)"
    echo ""
fi
...
if [ "$SKIP_EVAL" = false ]; then
    ...
    log_step "Step 4/4: Evaluating OMG predictions..."
    echo ""
    if [ -f "$SCRIPT_DIR/evaluate.py" ]; then
        python3 "$SCRIPT_DIR/evaluate.py" \
            --predictions "$SCRIPT_DIR/predictions/omg" \
            --output "$SCRIPT_DIR/results/omg_results.json"
        if [ $? -ne 0 ]; then
            log_warn "OMG evaluation had issues (continuing...)"
        fi
    else
        log_warn "evaluate.py not found, skipping evaluation"
    fi
...
if [ -f "$SCRIPT_DIR/compare_results.py" ]; then
    python3 "$SCRIPT_DIR/compare_results.py" \
        ...
        --omg "$SCRIPT_DIR/predictions/omg/predictions.jsonl" \
...
### OMG-Enhanced
Location: \`predictions/omg/\`
Results: \`results/omg_results.json\`
...
log_info "  OMG predictions:     $SCRIPT_DIR/predictions/omg/"
