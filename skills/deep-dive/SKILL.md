...
triggers:
  ...
  - "trace and interview"
  - "investigate deeply"
pipeline: [deep-dive, omg-plan, autopilot]
next-skill: omg-plan
...
3. **Detect brownfield vs greenfield**:
   - Run `explore` agent (flash): check if cwd has existing source code, package files, or git history
...
4. **Generate 3 trace lane hypotheses**:
   ...
   - For brownfield: run `explore` agent (flash) to identify relevant codebase areas, store as `codebase_context` for later injection
...
Run the trace autonomously using the `oh-my-gemini:trace` skill's behavioral contract.

### Team Mode Orchestration

Use **Gemini built-in team mode** to run 3 parallel tracer lanes:
...
After saving:
- Persist `trace_path` in state: `state_write` with `state.trace_path = ".omg/specs/deep-dive-trace-{slug}.md"`
...
- Save to `.omg/specs/deep-dive-{slug}.md`
- Persist `spec_path` in state: `state_write` with `state.spec_path = ".omg/specs/deep-dive-{slug}.md"`
...
1. **Ralplan → Autopilot (Recommended)**
   ...
   - Action: Invoke `Skill("oh-my-gemini:omg-plan")` with `--consensus --direct` flags and the spec file path (`spec_path` from state) as context. The `--direct` flag skips the omg-plan skill's interview phase (the deep-dive interview already gathered requirements), while `--consensus` triggers the Planner/Architect/Critic loop. When consensus completes and produces a plan in `.omg/plans/`, invoke `Skill("oh-my-gemini:autopilot")` with the consensus plan as Phase 0+1 output — autopilot skips both Expansion and Planning, starting directly at Phase 2 (Execution).
   - Pipeline: `deep-dive spec → omg-plan --consensus --direct → autopilot execution`

2. **Execute with autopilot (skip ralplan)**
   - Description: "Full autonomous pipeline — planning, parallel implementation, QA, validation. Faster but without consensus refinement."
   - Action: Invoke `Skill("oh-my-gemini:autopilot")` with the spec file path as context. The spec replaces autopilot's Phase 0 — autopilot starts at Phase 1 (Planning).
...
3. **Execute with ralph**
   ...
   - Action: Invoke `Skill("oh-my-gemini:ralph")` with the spec file path as the task definition.

4. **Execute with team**
   - Description: "N coordinated parallel agents — fastest execution for large specs."
   - Action: Invoke `Skill("oh-my-gemini:team")` with the spec file path as the shared plan.
...
- Use `Agent(subagent_type="oh-my-gemini:explore", model="flash")` for brownfield codebase exploration (Phase 1)
- Use Gemini built-in team mode for 3 parallel tracer lanes (Phase 3)
...
- Use `Write` tool to save trace result and final spec to `.omg/specs/`
...
[Phase 5] Spec ready. User selects ralplan → autopilot.
  → omg-plan --consensus --direct runs on the spec
...
- [ ] Phase 3 saves trace result to `.omg/specs/deep-dive-trace-{slug}.md` with per-lane critical unknowns
...
- [ ] Final spec saved to `.omg/specs/deep-dive-{slug}.md` in standard deep-interview format
- [ ] Final spec contains "Trace Findings" section
- [ ] Phase 5 execution bridge passes spec_path explicitly to downstream skills
- [ ] Phase 5 "Ralplan → Autopilot" option explicitly invokes autopilot after omg-plan consensus completes
...
Optional settings in `.gemini/settings.json`:
...
Deep-dive's output (`.omg/specs/deep-dive-{slug}.md`) feeds into the standard omg pipeline:

```
/deep-dive "problem"
  → Trace (3 parallel lanes) + Interview (Socratic Q&A)
  → Spec: .omg/specs/deep-dive-{slug}.md

  → /omg-plan --consensus --direct (spec as input)
    → Planner/Architect/Critic consensus
    → Plan: .omg/plans/ralplan-*.md
