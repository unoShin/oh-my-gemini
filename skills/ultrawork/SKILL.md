...
3. **Route to correct tiers**:
   - Simple lookups/definitions: LOW tier (Flash)
   - Standard implementation: MEDIUM tier (Pro)
   - Complex analysis/refactoring: HIGH tier (Ultra)
...
<Tool_Usage>
- Use `Task(subagent_type="oh-my-gemini:executor", model="flash", ...)` for simple changes
- Use `Task(subagent_type="oh-my-gemini:executor", model="pro", ...)` for standard work
- Use `Task(subagent_type="oh-my-gemini:executor", model="ultra", ...)` for complex work
...
```
Task(subagent_type="oh-my-gemini:executor", model="flash", prompt="Add missing type export for Config interface")
Task(subagent_type="oh-my-gemini:executor", model="pro", prompt="Implement the /api/users endpoint with validation")
Task(subagent_type="oh-my-gemini:executor", model="pro", prompt="Add integration tests for the auth middleware")
```
...
```
Task(subagent_type="oh-my-gemini:executor", model="pro", prompt="npm install && npm run build", run_in_background=true)
Task(subagent_type="oh-my-gemini:executor", model="flash", prompt="Update the README with new API endpoints")
```
...
```
Task(subagent_type="oh-my-gemini:executor", model="ultra", prompt="Add a missing semicolon")
```
Why bad: Ultra is expensive overkill for a trivial fix. Use executor with Flash instead.
</Bad>
</Examples>
