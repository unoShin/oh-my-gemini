...
Ralplan is a shorthand alias for `/oh-my-gemini:omg-plan --consensus`. It triggers iterative planning with Planner, Architect, and Critic agents until consensus is reached, with **RALPLAN-DR structured deliberation** (short mode by default, deliberate mode for high-risk work).

## Usage

```
/oh-my-gemini:ralplan "task description"
```
...
- `--architect gemini`: Use Gemini for the Architect pass when Gemini CLI is available. Otherwise, briefly note the fallback and keep the default Gemini Architect review.
- `--critic gemini`: Use Gemini for the Critic pass when Gemini CLI is available. Otherwise, briefly note the fallback and keep the default Gemini Critic review.

## Usage with interactive mode

```
/oh-my-gemini:ralplan --interactive "task description"
```
...
```
/oh-my-gemini:omg-plan --consensus <arguments>
```
...
8. *(--interactive only)* On approval: invoke `Skill("oh-my-gemini:team")` for parallel team execution (recommended) or `Skill("oh-my-gemini:ralph")` for sequential execution -- never implement directly
