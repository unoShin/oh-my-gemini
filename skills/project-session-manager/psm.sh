...
Examples:
  psm review omg#123
  psm fix unoShin/oh-my-gemini#42
  psm feature omg add-webhooks
  psm list
...
cmd_review() {
    ...
    if [[ "$no_tmux" != "true" ]]; then
        ...
        if [[ "$tmux_status" == "error" ]]; then
            ...
            # Launch Gemini Code
            if [[ "$no_gemini" != "true" ]]; then
                log_info "Launching Gemini Code..."
                psm_launch_gemini "$session_name"
            fi
        fi
    fi
