...
MCP servers extend Gemini Code with additional tools (web search, GitHub, etc.).
...
Agent teams are an experimental Gemini Code feature that lets you spawn N coordinated agents working on a shared task list with inter-agent messaging. **Teams are disabled by default** and require enabling via `settings.json`.

Reference: https://code.gemini.com/docs/en/agent-teams

Use AskUserQuestion:

**Question:** "Would you like to enable agent teams? Teams let you spawn coordinated agents (e.g., `/team 3:executor 'fix all errors'`). This is an experimental Gemini Code feature."
...
**CRITICAL**: Agent teams require `GEMINI_CODE_EXPERIMENTAL_AGENT_TEAMS` to be set in `~/.gemini/settings.json`. This must be done carefully to preserve existing user settings.
...
Then use the Read tool to read `~/.gemini/settings.json` (if it exists). Use the Edit tool to merge the teams configuration while preserving ALL existing settings.
...
if [ -f "$SETTINGS_FILE" ]; then
  TEMP_FILE=$(mktemp)
  jq '.env = (.env // {} | . + {"GEMINI_CODE_EXPERIMENTAL_AGENT_TEAMS": "1"})' "$SETTINGS_FILE" > "$TEMP_FILE" && mv "$TEMP_FILE" "$SETTINGS_FILE"
  echo "Added GEMINI_CODE_EXPERIMENTAL_AGENT_TEAMS to existing settings.json"
else
...
Store the team configuration in `~/.gemini/.omg-config.json`:
...
**Note:** Teammates do not have a separate model default. Each teammate is a full Gemini Code session that inherits your configured model. Subagents spawned by teammates can use any model tier.
...
if jq -e '.env.GEMINI_CODE_EXPERIMENTAL_AGENT_TEAMS' "$SETTINGS_FILE" > /dev/null 2>&1; then
...
Skip this step. Agent teams will remain disabled. User can enable later by adding to `~/.gemini/settings.json`:
```json
{
  "env": {
    "GEMINI_CODE_EXPERIMENTAL_AGENT_TEAMS": "1"
...
Or by running `/oh-my-gemini:omg-setup --force` and choosing to enable teams.

## Save Progress

```bash
CONFIG_TYPE=$(jq -r '.configType // "unknown"' ".omg/state/setup-state.json" 2>/dev/null || echo "unknown")
bash "${GEMINI_PLUGIN_ROOT}/scripts/setup-progress.sh" save 6 "$CONFIG_TYPE"
```
