...
- Cancel with `/oh-my-gemini:cancel` at any time; progress is preserved for resume
</Execution_Policy>
...
6. **Phase 5 - Cleanup**: Delete all state files on successful completion
   ...
   - Run `/oh-my-gemini:cancel` for clean exit
</Steps>

<Tool_Usage>
- Use `Task(subagent_type="oh-my-gemini:architect", ...)` for Phase 4 architecture validation
- Use `Task(subagent_type="oh-my-gemini:security-reviewer", ...)` for Phase 4 security review
- Use `Task(subagent_type="oh-my-gemini:code-reviewer", ...)` for Phase 4 quality review
- Agents form their own analysis first, then spawn Gemini Task agents for cross-validation
...
If autopilot was cancelled or failed, run `/oh-my-gemini:autopilot` again to resume from where it stopped.
