...
```
/oh-my-gemini:sciomg <goal>                    # Standard research with user checkpoints
/oh-my-gemini:sciomg AUTO: <goal>              # Fully autonomous until complete
/oh-my-gemini:sciomg status                    # Check current research session status
/oh-my-gemini:sciomg resume                    # Resume interrupted research session
/oh-my-gemini:sciomg list                      # List all research sessions
/oh-my-gemini:sciomg report <session-id>       # Generate report for session
```

### Quick Examples

```
/oh-my-gemini:sciomg What are the performance characteristics of different sorting algorithms?
/oh-my-gemini:sciomg AUTO: Analyze authentication patterns in this codebase
/oh-my-gemini:sciomg How does the error handling work across the API layer?
```
...
Task(subagent_type="oh-my-gemini:scientist", model="flash", prompt="[RESEARCH_STAGE:1] Investigate...")

// Stage 2 - Standard analysis
Task(subagent_type="oh-my-gemini:scientist", model="pro", prompt="[RESEARCH_STAGE:2] Analyze...")

// Stage 3 - Complex reasoning
Task(subagent_type="oh-my-gemini:scientist", model="ultra", prompt="[RESEARCH_STAGE:3] Deep analysis of...")
```
...
|-----------------|-------|-------|---------|
| Data gathering | `scientist` (model=flash) | flash | File enumeration, pattern counting, simple lookups |
| Standard analysis | `scientist` | pro | Code analysis, pattern detection, documentation review |
| Complex reasoning | `scientist` | ultra | Architecture analysis, cross-cutting concerns, hypothesis validation |
...
Task(subagent_type="oh-my-gemini:scientist", model="pro", prompt="
...
4. **Cancellation:** `/oh-my-gemini:cancel` or "stop", "cancel"

### AUTO Mode Example

```
/oh-my-gemini:sciomg AUTO: Comprehensive security analysis of the authentication system
...
Task(subagent_type="oh-my-gemini:scientist", model="flash", prompt="[STAGE:1] Analyze src/api/...")
Task(subagent_type="oh-my-gemini:scientist", model="flash", prompt="[STAGE:2] Analyze src/utils/...")
Task(subagent_type="oh-my-gemini:scientist", model="flash", prompt="[STAGE:3] Analyze src/components/...")
```
...
Task(subagent_type="oh-my-gemini:scientist", model="pro", prompt="[HYPOTHESIS:A] Test if caching improves...")
Task(subagent_type="oh-my-gemini:scientist", model="pro", prompt="[HYPOTHESIS:B] Test if batching reduces...")
Task(subagent_type="oh-my-gemini:scientist", model="pro", prompt="[HYPOTHESIS:C] Test if lazy loading helps...")
```
...
Task(subagent_type="oh-my-gemini:scientist", model="ultra", prompt="
[CROSS_VALIDATION]
...
| Command | Action |
|---------|--------|
| `/oh-my-gemini:sciomg status` | Show current session progress |
| `/oh-my-gemini:sciomg resume` | Resume most recent interrupted session |
| `/oh-my-gemini:sciomg resume <session-id>` | Resume specific session |
| `/oh-my-gemini:sciomg list` | List all sessions with status |
| `/oh-my-gemini:sciomg report <session-id>` | Generate/regenerate report |
| `/oh-my-gemini:sciomg cancel` | Cancel current session (preserves state) |
