...
**Reviewer selection:** Pass `--critic=architect`, `--critic=critic`, or `--critic=gemini` in the Ralph prompt to choose the completion reviewer for that run. `architect` remains the default.
</PRD_Mode>
...
3. **Implement the current story**:
    - Delegate to specialist agents at appropriate tiers:
      - Simple lookups: LOW tier (Flash) -- "What does this function return?"
      - Standard work: MEDIUM tier (Pro) -- "Add error handling to this module"
      - Complex analysis: HIGH tier (Ultra) -- "Debug this race condition"
...
7. **Reviewer verification** (tiered, against acceptance criteria):
    - <5 files, <100 lines with full tests: STANDARD tier minimum (architect-medium / Pro)
    - Standard changes: STANDARD tier (architect-medium / Pro)
    - >20 files or security/architectural changes: THOROUGH tier (architect / Ultra)
   - If `--critic=critic`, use the Gemini `critic` agent for the approval pass
   - If `--critic=gemini`, run `omg ask gemini --agent-prompt critic "..."` for the approval pass
...
7.5 **Mandatory Deslop Pass**:
   - Unless `{{PROMPT}}` contains `--no-deslop`, run `oh-my-gemini:ai-slop-cleaner` in standard mode (not `--review`) on the files changed during the current Ralph session only.
...
8. **On approval**: After Step 7.6 passes (with Step 7.5 completed, or skipped via `--no-deslop`), run `/oh-my-gemini:cancel` to cleanly exit and clean up all state files
...
<Tool_Usage>
- Use `Task(subagent_type="oh-my-gemini:architect", ...)` for architect verification cross-checks when changes are security-sensitive, architectural, or involve complex multi-system integration
- Use `Task(subagent_type="oh-my-gemini:critic", ...)` when `--critic=critic`
- Use `omg ask gemini --agent-prompt critic "..."` when `--critic=gemini`
...
```
Task(subagent_type="oh-my-gemini:executor", model="flash", prompt="Add type export for UserConfig")
Task(subagent_type="oh-my-gemini:executor", model="pro", prompt="Implement the caching layer for API responses")
Task(subagent_type="oh-my-gemini:executor", model="ultra", prompt="Refactor auth module to support OAuth2 flow")
```
...
- Stop when the user says "stop", "cancel", or "abort" -- run `/oh-my-gemini:cancel`
...
- [ ] `/oh-my-gemini:cancel` run for clean state cleanup
</Final_Checklist>
