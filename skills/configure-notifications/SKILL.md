...
Set up OMG notification integrations so you're alerted when sessions end, need input, or complete background tasks.
...
Set up Telegram notifications so OMG can message you when sessions end, need input, or complete background tasks.

### How This Skill Works

This is an interactive, natural-language configuration skill. Walk the user through setup by asking questions with AskUserQuestion. Write the result to `~/.gemini/.omg-config.json`.
...
2. Send /newbot
3. Choose a name (e.g., "My OMG Notifier")
4. Choose a username (e.g., "my_omg_bot")
...
1. **Session end (Recommended)** - When a Gemini session finishes
2. **Input needed** - When Gemini is waiting for your response (great for long-running tasks)
...
RESPONSE=$(curl -s -w "\n%{http_code}" \
  ...
  -d "text=OMG test notification - Telegram is configured!")
...
Config saved to: ~/.gemini/.omg-config.json

You can also set these via environment variables:
  OMG_TELEGRAM_BOT_TOKEN=123456789:ABCdefGHI...
  OMG_TELEGRAM_CHAT_ID=123456789

To reconfigure: /oh-my-gemini:configure-notifications telegram
To configure Discord: /oh-my-gemini:configure-notifications discord
To configure Slack: /oh-my-gemini:configure-notifications slack
```
...
```bash
export OMG_TELEGRAM_BOT_TOKEN="123456789:ABCdefGHIjklMNOpqrsTUVwxyz"
...
Set up Discord notifications so OMG can ping you when sessions end, need input, or complete background tasks.
...
**Options:**
1. **OMG (default)** - Display as "OMG"
2. **Gemini Code** - Display as "Gemini Code"
...
curl -s -o /dev/null -w "%{http_code}" \
  ...
  -d "{\"content\": \"${MENTION:+$MENTION\\n}OMG test notification - Discord is configured!\"}" \
  "$WEBHOOK_URL"
```
...
You can also set these via environment variables:
  OMG_DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/...
  OMG_DISCORD_MENTION=<@1465264645320474637>

To reconfigure: /oh-my-gemini:configure-notifications discord
To configure Telegram: /oh-my-gemini:configure-notifications telegram
To configure Slack: /oh-my-gemini:configure-notifications slack
```
...
export OMG_DISCORD_MENTION="<@1465264645320474637>"  # optional
```

**Bot API method:**
```bash
export OMG_DISCORD_NOTIFIER_BOT_TOKEN="your-bot-token"
export OMG_DISCORD_NOTIFIER_CHANNEL="your-channel-id"
export OMG_DISCORD_MENTION="<@1465264645320474637>"  # optional
```
...
Set up Slack notifications so OMG can message you when sessions end, need input, or complete background tasks.
...
3. Name your app (e.g., "OMG Notifier") and select your workspace
...
curl -s -o /dev/null -w "%{http_code}" \
  -H "Content-Type: application/json" \
  -d "{\"text\": \"${MENTION_PREFIX}OMG test notification - Slack is configured!\"}" \
  "$WEBHOOK_URL"
```
...
You can also set these via environment variables:
  OMG_SLACK_WEBHOOK_URL=https://hooks.slack.com/services/...
  OMG_SLACK_MENTION=<@U1234567890>

To reconfigure: /oh-my-gemini:configure-notifications slack
To configure Discord: /oh-my-gemini:configure-notifications discord
To configure Telegram: /oh-my-gemini:configure-notifications telegram
```
...
```bash
export OMG_SLACK_WEBHOOK_URL="https://hooks.slack.com/services/T00/B00/xxx"
export OMG_SLACK_MENTION="<@U1234567890>"  # optional
```
...
- `omg --telegram` — Activates Telegram notifications (sets `OMG_TELEGRAM=1`)
- `omg --discord` — Activates Discord notifications (sets `OMG_DISCORD=1`)
- `omg --slack` — Activates Slack notifications (sets `OMG_SLACK=1`)
- `omg --webhook` — Activates webhook notifications (sets `OMG_WEBHOOK=1`)
- `omg --openclaw` — Activates OpenClaw gateway integration (sets `OMG_OPENCLAW=1`)
...
**Examples:**
- `omg --telegram --discord` — Telegram + Discord active
- `omg --telegram --slack --webhook` — Telegram + Slack + Webhook active
- `omg --telegram --openclaw` — Telegram + OpenClaw active
- `omg` — No notifications sent (all platforms require explicit activation)

---

## Hook Event Templates

Customize notification messages per event and per platform using `omg_config.hook.json`.
...
Check if `~/.gemini/omg_config.hook.json` exists. If it does, show the current configuration. If not, explain what it does.
...
Config file: ~/.gemini/omg_config.hook.json
```
...
**Options:**
1. **session-end** - When a Gemini session finishes (most common)
2. **ask-user-question** - When Gemini is waiting for input
3. **session-idle** - When Gemini finishes and waits for input
...
RAW FIELDS:
  ...
  {{activeMode}}     - Active OMG mode name
...
Read or create `~/.gemini/omg_config.hook.json` and merge the new settings:
...
- `/oh-my-gemini:configure-openclaw` — Configure OpenClaw gateway integration
...
If `~/.gemini/omg_config.openclaw.json` exists, detect and offer migration:

**Step 1: Detect Legacy Config**
```bash
LEGACY_CONFIG="$HOME/.gemini/omg_config.openclaw.json"
...
If migrate:
- Read `omg_config.openclaw.json`
- Transform to custom integration format
- Save to `.omg-config.json`
- Backup legacy to `omg_config.openclaw.json.bak`
...
curl -X POST \
  ...
  -d '{"event":"test","instruction":"OMG test notification","timestamp":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' \
  "$WEBHOOK_URL"
```
...
| `{{tmuxSession}}` | tmux session name | `gemini:my-project` |
