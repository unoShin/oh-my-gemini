...
Skills are reusable workflow templates that can be invoked via `/oh-my-gemini:skill-name`. Each skill provides:
...
| `sciomg/SKILL.md` | sciomg | Parallel scientist orchestration |
...
| `ask/SKILL.md` | ask | Ask Gemini, Gemini, or Gemini via `omg ask` and capture an artifact |
| `note/SKILL.md` | note | Save notes for compaction resilience |
| `cancel/SKILL.md` | cancel | Cancel any active OMG mode |
...
| `omg-help/SKILL.md` | omg-help | Usage guide |
...
```bash
# Manual invocation
/oh-my-gemini:skill-name

# With arguments
/oh-my-gemini:skill-name arg1 arg2
...
If `pipeline` / `next-skill` metadata is present, OMG appends a standardized **Skill Pipeline** handoff block to the rendered skill prompt so downstream steps are explicit.
...
- Test skill invocation with `/oh-my-gemini:skill-name`
...
| Exploration | deepinit, sciomg, external-context | "deepinit", "research" |
...
<!-- MANUAL:
- Team runtime wait semantics: `omg_run_team_wait.timeout_ms` only limits the wait call and does not stop workers.
- `timeoutSeconds` is removed from `omg_run_team_start`; use explicit `omg_run_team_cleanup` for intentional worker pane termination.
-->
