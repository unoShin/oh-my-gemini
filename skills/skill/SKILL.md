...
**Built-in skill note:** Built-in skills are bundled with oh-my-gemini and are discoverable/readable, but not removed or edited through `/skill remove` or `/skill edit`.
...
5. **Ask for scope:**
   - `user` → `~/.gemini/skills/omg-learned/<name>/SKILL.md`
...
✓ Created skill at ~/.gemini/skills/omg-learned/custom-logger/SKILL.md
...
1. **Search for skill** in both scopes:
   - `~/.gemini/skills/omg-learned/<name>/SKILL.md`
   - `.omg/skills/<name>/SKILL.md`
2. **If found:**
   - Display skill info (name, description, scope)
   - **Ask for confirmation:** "Delete '<name>' skill from <scope>? (yes/no)"
3. **If confirmed:**
   - Delete entire skill directory (e.g., `~/.gemini/skills/omg-learned/<name>/`)
...
Assistant: Found skill 'old-logger' in user scope:
  ...
  Scope: user (~/.gemini/skills/omg-learned/)
...
```
Skill: custom-logger
Scope: user (~/.gemini/skills/omg-learned/custom-logger/)
Description: Enhanced logging with structured output
Triggers: log, logger, logging
Argument Hint: <level> [message]
Quality: 95% (if available)
Usage Count: 42 times (if available)
File Path: /home/user/.gemini/skills/omg-learned/custom-logger/SKILL.md
...
Description: Enhanced logging with structured output
Triggers: log, logger, logging
File: ~/.gemini/skills/omg-learned/custom-logger/SKILL.md
...
USER_SKILLS_DIR="$HOME/.gemini/skills/omg-learned"
...
echo "=== USER-LEVEL SKILLS (~/.gemini/skills/omg-learned/) ==="
if [ -d "$HOME/.gemini/skills/omg-learned" ]; then
  USER_COUNT=$(find "$HOME/.gemini/skills/omg-learned" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $USER_COUNT"

  if [ $USER_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find "$HOME/.gemini/skills/omg-learned" -name "*.md" -type f -exec sh -c '
      FILE="$1"
...
Then ask for scope:
- **User-level** (~/.gemini/skills/omg-learned/) - Available across all projects
...
- `/oh-my-gemini:skill list` - Show detailed skill inventory
- `/oh-my-gemini:skill add` - Start skill creation (invoke learner)
- `/oh-my-gemini:skill scan` - Scan both skill directories
...
- `/oh-my-gemini:learner` - Extract a skill from current conversation
- `/oh-my-gemini:note` - Save quick notes (less formal than skills)
- `/oh-my-gemini:deepinit` - Generate AGENTS.md codebase hierarchy
...
```
> /oh-my-gemini:skill list

Checking skill directories...
✓ User skills directory exists: ~/.gemini/skills/omg-learned/
...
- Run `/oh-my-gemini:skill list` periodically to review your skill library
