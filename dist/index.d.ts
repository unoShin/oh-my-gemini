 ...
 * Oh-My-GeminiCode
 *
 * A multi-agent orchestration system for the Gemini Agent SDK.
 * Inspired by oh-my-opencode, reimagined for Gemini Code.
 *
 * Main features:
 * - OMG: Primary orchestrator that delegates to specialized subagents
 * - Parallel execution: Background agents run concurrently
 * - LSP/AST tools: IDE-like capabilities for agents
 * - Context management: Auto-injection from AGENTS.md/GEMINI.md
...
import { getAgentDefinitions, omgSystemPrompt } from './agents/definitions.js';
...
export { loadConfig, getAgentDefinitions, omgSystemPrompt };
...
export { omgToolsServer, omgToolNames, getOmgToolNames } from './mcp/omg-tools-server.js';
...
export { install, isInstalled, getInstallInfo, isGeminiInstalled, GEMINI_CONFIG_DIR as INSTALLER_GEMINI_CONFIG_DIR, AGENTS_DIR, COMMANDS_DIR, VERSION as INSTALLER_VERSION, type InstallResult, type InstallOptions } from './installer/index.js';
/**
 * Options for creating a OMG session
 */
export interface OmgOptions {
...
export interface OmgSession {
    /** The query options to pass to Gemini Agent SDK */
    queryOptions: {
        options: {
            systemPrompt: string;
 ...
 * to run a query with the Gemini Agent SDK.
 *
 * @example
 * ```typescript
 * import { createOmgSession } from 'oh-my-gemini';
 * import { query } from '@google/gemini-agent-sdk';
 *
 * const session = createOmgSession();
 *
 * // Use with Gemini Agent SDK
 ...
 * ```
 */
export declare function createOmgSession(options?: OmgOptions): OmgSession;
/**
 ...
 */
export declare function getOmgSystemPrompt(options?: {
