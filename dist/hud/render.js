...
export async function render(context, config) {
    ...
    // Profile name (from GEMINI_CONFIG_DIR)
    if (enabledElements.profile && context.profileName) {
        gitElements.push(bold(`profile:${context.profileName}`));
    }
    // [OMG#X.Y.Z] label with optional update notification
    if (enabledElements.omgLabel) {
        const versionTag = context.omgVersion ? `#${context.omgVersion}` : "";
        if (context.updateAvailable) {
            elements.push(bold(`[OMG${versionTag}] -> ${context.updateAvailable} omg update`));
        }
        else {
            elements.push(bold(`[OMG${versionTag}]`));
        }
    }
