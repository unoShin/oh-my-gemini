...
import { buildAutoresearchSetupSlashCommand, checkTmuxAvailable, guidedAutoresearchSetup, guidedAutoresearchSetupInference, initAutoresearchMission, parseInitArgs, prepareAutoresearchSetupGeminiHome, runAutoresearchNoviceBridge, spawnAutoresearchSetupTmux, spawnAutoresearchTmux, } from '../autoresearch-guided.js';
    ...
    it('uses explicit cwd, login-shell wrapping, and verifies startup before logging success', () => {
        ...
        vi.mocked(execFileSync).mockImplementation((cmd, args, opts) => {
            ...
            if (cmd === 'tmux' && Array.isArray(args) && args[0] === 'new-session') {
                expect(args.slice(0, 6)).toEqual(['new-session', '-d', '-s', 'omg-autoresearch-demo', '-c', '/repo']);
                expect(args[6]).toBe('wrapped:' + `${process.execPath} ${process.cwd()}/bin/omg.js autoresearch /repo/missions/demo`);
                return Buffer.from('');
            }
            throw new Error(`unexpected call: ${String(cmd)}`);
        });
        spawnAutoresearchTmux('/repo/missions/demo', 'demo');
        expect(buildTmuxShellCommandMock).toHaveBeenCalledWith(process.execPath, [expect.stringMatching(/bin\/omg\.js$/), 'autoresearch', '/repo/missions/demo']);
        expect(wrapWithLoginShellMock).toHaveBeenCalledWith(`${process.execPath} ${process.cwd()}/bin/omg.js autoresearch /repo/missions/demo`);
        expect(logSpy).toHaveBeenCalledWith('\nAutoresearch launched in background tmux session.');
        expect(logSpy).toHaveBeenCalledWith('  Attach:   tmux attach -t omg-autoresearch-demo');
    });
});
describe('prepareAutoresearchSetupGeminiHome', () => {
    it('creates a temp GEMINI_HOME with autoNudge disabled and symlinked skills when available', async () => {
        vi.mocked(execFileSync).mockReset();
        const repo = await initRepo();
        const originalGeminiHome = process.env.GEMINI_HOME;
        try {
            const baseGeminiHome = join(repo, 'base-gemini-home');
            await mkdir(join(baseGeminiHome, 'skills'), { recursive: true });
            await writeFile(join(baseGeminiHome, 'skills', 'marker.txt'), 'ok\n', 'utf-8');
            process.env.GEMINI_HOME = baseGeminiHome;
            const tempGeminiHome = prepareAutoresearchSetupGeminiHome(repo, 'setup-session');
            const configText = await readFile(join(tempGeminiHome, '.omx-config.json'), 'utf-8');
            expect(JSON.parse(configText)).toEqual({ autoNudge: { enabled: false } });
            expect(await readFile(join(tempGeminiHome, 'skills', 'marker.txt'), 'utf-8')).toBe('ok\n');
        }
        finally {
...
describe('spawnAutoresearchSetupTmux', () => {
    ...
    it('launches a detached gemini setup session and seeds deep-interview autoresearch mode', async () => {
        tmuxAvailableMock.mockReturnValue(true);
        const repo = await initRepo();
        let hasSessionCalls = 0;
        try {
            vi.mocked(execFileSync).mockImplementation((cmd, args) => {
                if (cmd === 'tmux' && Array.isArray(args) && args[0] === 'new-session') {
                    expect(args.slice(0, 9)).toEqual([
                        'new-session', '-d', '-P', '-F', '#{pane_id}', '-s', 'omg-autoresearch-setup-kf12oi', '-c', repo,
                    ]);
                    expect(typeof args[9]).toBe('string');
                    expect(String(args[9])).toContain('wrapped:env');
                    expect(String(args[9])).toContain(`GEMINI_HOME=${repo}/.omx/tmp/omg-autoresearch-setup-kf12oi/gemini-home`);
                    expect(String(args[9])).toContain('gemini');
                    expect(String(args[9])).toContain('--dangerously-skip-permissions');
                    return '%42\n';
                }
                if (cmd === 'tmux' && Array.isArray(args) && args[0] === 'has-session') {
                    hasSessionCalls += 1;
                    expect(args).toEqual(['has-session', '-t', 'omg-autoresearch-setup-kf12oi']);
                    return Buffer.from('');
                }
            ...
            expect(buildTmuxShellCommandMock).toHaveBeenCalledWith('env', [`GEMINI_HOME=${repo}/.omx/tmp/omg-autoresearch-setup-kf12oi/gemini-home`, 'gemini', '--dangerously-skip-permissions']);
            expect(wrapWithLoginShellMock).toHaveBeenCalledWith(`env GEMINI_HOME=${repo}/.omx/tmp/omg-autoresearch-setup-kf12oi/gemini-home gemini --dangerously-skip-permissions`);
            ...
            expect(logSpy).toHaveBeenCalledWith('\nAutoresearch setup launched in background Gemini session.');
            expect(logSpy).toHaveBeenCalledWith('  Attach:   tmux attach -t omg-autoresearch-setup-kf12oi');
