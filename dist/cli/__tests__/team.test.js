...
describe('team cli', () => {
    ...
    beforeEach(() => {
        ...
        process.env.OMG_RUNTIME_CLI_PATH = '/tmp/runtime-cli.cjs';
    ...
    afterEach(() => {
        delete process.env.OMG_JOBS_DIR;
        delete process.env.OMG_RUNTIME_CLI_PATH;
    ...
    it('startTeamJob starts runtime-cli and persists running job', async () => {
        ...
        expect(result.jobId).toMatch(/^omg-[a-z0-9]{1,16}$/);
    ...
    it('teamCommand start --json outputs valid JSON envelope', async () => {
        ...
        await teamCommand(['start', '--agent', 'gemini', '--task', 'review auth flow', '--json']);
        expect(mocks.spawn).toHaveBeenCalledTimes(1);
        expect(write).toHaveBeenCalledTimes(1);
        expect(end).toHaveBeenCalledTimes(1);
        // Verify stdin payload sent to runtime-cli
        const stdinPayload = JSON.parse(write.mock.calls[0][0]);
        expect(stdinPayload.agentTypes).toEqual(['gemini']);
        ...
        expect(output.jobId).toMatch(/^omg-[a-z0-9]{1,16}$/);
    ...
    it('teamCommand start forwards --new-window to runtime-cli payload', async () => {
        ...
        const { teamCommand } = await import('../team.js');
        await teamCommand(['start', '--agent', 'gemini', '--task', 'review auth flow', '--new-window', '--json']);
        const stdinPayload = JSON.parse(write.mock.calls[0][0]);
    ...
    it('teamCommand start without --json outputs non-JSON', async () => {
        ...
        const { teamCommand } = await import('../team.js');
        await teamCommand(['start', '--agent', 'gemini', '--task', 'do stuff']);
        expect(logSpy).toHaveBeenCalledTimes(1);
    ...
    it('team status deduplicates workerPaneIds from duplicate worker config rows', async () => {
        ...
        const cwd = mkdtempSync(join(tmpdir(), 'omg-team-cli-v2-status-dedup-'));
        const root = join(cwd, '.omg', 'state', 'team', 'demo-team');
        mkdirSync(root, { recursive: true });
        writeFileSync(join(root, 'config.json'), JSON.stringify({
            name: 'demo-team',
    ...
    it('team status supports team-name target via runtime snapshot', async () => {
        ...
        mocks.resumeTeam.mockResolvedValue({
            ...
            leaderPaneId: '%0',
            config: { teamName: 'demo-team', workerCount: 1, agentTypes: ['gemini'], tasks: [], cwd: '/tmp/demo' },
    ...
    it('team resume invokes runtime resumeTeam', async () => {
        ...
        mocks.resumeTeam.mockResolvedValue({
            ...
            leaderPaneId: '%0',
            config: { teamName: 'alpha-team', workerCount: 1, agentTypes: ['gemini'], tasks: [], cwd: '/tmp/demo' },
            workerNames: ['worker-1'],
            workerPaneIds: ['%1'],
    ...
    it('team shutdown supports --force and calls runtime shutdown', async () => {
        ...
        mocks.resumeTeam.mockResolvedValue({
            ...
            leaderPaneId: '%0',
            config: { teamName: 'beta-team', workerCount: 1, agentTypes: ['gemini'], tasks: [], cwd: '/tmp/demo' },
            workerNames: ['worker-1'],
            workerPaneIds: ['%1'],
            activeWorkers: new Map(),
            cwd: '/tmp/demo',
    ...
    it('legacy shorthand start alias supports optional ralph token', async () => {
        ...
        const { teamCommand } = await import('../team.js');
        await teamCommand(['ralph', '2:gemini', 'ship', 'feature', '--json']);
        expect(write).toHaveBeenCalledTimes(1);
        const payload = JSON.parse(write.mock.calls[0][0]);
        expect(payload.agentTypes).toEqual(['gemini', 'gemini']);
