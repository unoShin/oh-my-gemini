...
describe('teamCommand api operations', () => {
    ...
    it('blocks team start when running inside worker context', async () => {
        const previousWorker = process.env.OMG_TEAM_WORKER;
        try {
            process.env.OMG_TEAM_WORKER = 'demo-team/worker-1';
            const logs = await captureLog(() => teamCommand(['1:executor', 'do work']));
            expect(logs[0]).toContain('omg team [N:agent-type[:role]]');
            expect(process.exitCode).toBe(1);
        }
        finally {
            process.env.OMG_TEAM_WORKER = previousWorker;
            process.exitCode = 0;
        }
    });
    it('allows nested team spawn only when parent governance enables it', async () => {
        wd = await mkdtemp(join(tmpdir(), 'omg-team-governance-'));
        previousCwd = process.cwd();
        process.chdir(wd);
...
describe('parseTeamArgs comma-separated multi-type specs', () => {
    it('parses 1:gemini,1:gemini into heterogeneous agentTypes', () => {
        const parsed = parseTeamArgs(['1:gemini,1:gemini', 'do the task']);
        expect(parsed.workerCount).toBe(2);
        expect(parsed.agentTypes).toEqual(['gemini', 'gemini']);
        expect(parsed.workerSpecs).toEqual([{ agentType: 'gemini' }, { agentType: 'gemini' }]);
        expect(parsed.task).toBe('do the task');
    });
    it('parses 2:gemini,1:gemini:architect with mixed counts and roles', () => {
        const parsed = parseTeamArgs(['2:gemini,1:gemini:architect', 'design system']);
        expect(parsed.workerCount).toBe(3);
        expect(parsed.agentTypes).toEqual(['gemini', 'gemini', 'gemini']);
        expect(parsed.workerSpecs).toEqual([
            { agentType: 'gemini' },
            { agentType: 'gemini' },
            { agentType: 'gemini', role: 'architect' },
        ]);
    ...
    it('sets role when all segments share the same role', () => {
        const parsed = parseTeamArgs(['1:gemini:executor,2:gemini:executor', 'run tasks']);
        expect(parsed.workerCount).toBe(3);
        expect(parsed.agentTypes).toEqual(['gemini', 'gemini', 'gemini']);
        expect(parsed.workerSpecs).toEqual([
    ...
    it('still parses single-type spec 3:gemini into uniform agentTypes', () => {
        const parsed = parseTeamArgs(['3:gemini', 'fix tests']);
        expect(parsed.workerCount).toBe(3);
        expect(parsed.agentTypes).toEqual(['gemini', 'gemini', 'gemini']);
        expect(parsed.task).toBe('fix tests');
    });
    it('defaults to 3 gemini workers when no spec is given', () => {
    ...
    it('parses single spec with role correctly', () => {
        const parsed = parseTeamArgs(['2:gemini:architect', 'design auth']);
        expect(parsed.workerCount).toBe(2);
        expect(parsed.agentTypes).toEqual(['gemini', 'gemini']);
        expect(parsed.workerSpecs).toEqual([
            { agentType: 'gemini', role: 'architect' },
    ...
    it('supports --json and --new-window flags with comma-separated specs', () => {
        const parsed = parseTeamArgs(['1:gemini,1:gemini', '--new-window', '--json', 'compare']);
        expect(parsed.workerCount).toBe(2);
        expect(parsed.agentTypes).toEqual(['gemini', 'gemini']);
    ...
    it('throws on total count exceeding maximum', () => {
        expect(() => parseTeamArgs(['15:gemini,10:gemini', 'big task'])).toThrow('exceeds maximum');
    });
});
describe('buildStartupTasks', () => {
    it('adds owner-aware fanout for explicit per-worker roles', () => {
        const parsed = parseTeamArgs(['1:gemini:architect,1:gemini:writer', 'draft launch plan']);
    ...
    it('keeps simple fanout unchanged when no explicit roles are provided', () => {
        const parsed = parseTeamArgs(['2:gemini', 'fix tests']);
        expect(buildStartupTasks(parsed)).toEqual([
