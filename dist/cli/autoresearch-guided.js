...
export async function runAutoresearchNoviceBridge(repoRoot, seedInputs = {}, io = createQuestionIO()) {
    ...
    try {
        while (true) {
            ...
            const evaluatorIntent = await promptWithDefault(io, '\nHow should OMG judge success? Describe it in plain language', topic);
...
export async function guidedAutoresearchSetupInference(repoRoot, deps = {}) {
    ...
    try {
        ...
        const explicitEvaluator = await askQuestion(rl, '\nOptional evaluator command (leave blank and OMG will infer one if confidence is high)\n> ');
...
export function spawnAutoresearchTmux(missionDir, slug) {
    ...
    const omgPath = resolve(join(__dirname, '..', '..', 'bin', 'omg.js'));
    const command = buildTmuxShellCommand(process.execPath, [omgPath, 'autoresearch', missionDir]);
...
export function prepareAutoresearchSetupGeminiHome(repoRoot, sessionName) {
    const baseGeminiHome = process.env.GEMINI_HOME?.trim() || join(homedir(), '.gemini');
    const tempGeminiHome = join(repoRoot, '.omx', 'tmp', sessionName, 'gemini-home');
    mkdirSync(tempGeminiHome, { recursive: true });
    for (const dirName of ['skills', 'commands']) {
        const sourceDir = join(baseGeminiHome, dirName);
        if (existsSync(sourceDir)) {
            ensureSymlink(sourceDir, join(tempGeminiHome, dirName));
        }
    }
    writeFileSync(join(tempGeminiHome, '.omx-config.json'), `${JSON.stringify({ autoNudge: { enabled: false } }, null, 2)}\n`, 'utf-8');
    return tempGeminiHome;
}
...
export function spawnAutoresearchSetupTmux(repoRoot) {
    ...
    const sessionName = `omg-autoresearch-setup-${Date.now().toString(36)}`;
    const geminiHome = prepareAutoresearchSetupGeminiHome(repoRoot, sessionName);
    const geminiCommand = buildTmuxShellCommand('env', [`GEMINI_HOME=${geminiHome}`, 'gemini', GEMINI_BYPASS_FLAG]);
    const wrappedGeminiCommand = wrapWithLoginShell(geminiCommand);
    const paneId = execFileSync('tmux', ['new-session', '-d', '-P', '-F', '#{pane_id}', '-s', sessionName, '-c', repoRoot, wrappedGeminiCommand], { encoding: 'utf-8' }).trim();
    assertTmuxSessionAvailable(sessionName);
    if (paneId) {
    ...
    console.log(`  GEMINI_HOME: ${quoteShellArg(geminiHome)}`);
    console.log(`  Attach:   tmux attach -t ${sessionName}`);
}
