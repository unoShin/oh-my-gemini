...
import { resolveLaunchPolicy, buildTmuxSessionName, buildTmuxShellCommand, wrapWithLoginShell, isGeminiAvailable, resolveGeminiPath, } from './tmux-utils.js';
// Flag mapping
 ...
 */
export function normalizeGeminiLaunchArgs(args) {
    const normalized = [];
 ...
 */
export function runGemini(cwd, args, sessionId) {
 ...
 */
function runGeminiInsideTmux(cwd, args) {
    try {
 ...
 */
function runGeminiOutsideTmux(cwd, args, _sessionId) {
 ...
 */
function runGeminiDirect(cwd, args) {
    try {
        execFileSync(resolveGeminiPath(), args, { cwd, stdio: 'inherit' });
    }
    catch (error) {
