...
import { resolveStatePath, ensureOmgDir, validateWorkingDirectory, resolveSessionStatePath, ensureSessionStateDir, listSessionIds, validateSessionId, getOmgRoot, } from '../lib/worktree-paths.js';
...
function pruneMissionBoardTeams(root, teamNames) {
    const missionStatePath = join(getOmgRoot(root), 'state', 'mission-state.json');
...
function cleanupTeamRuntimeState(root, teamNames) {
    const teamStateRoot = join(getOmgRoot(root), 'state', 'team');
...
function getLegacyStateFileCandidates(mode, root) {
    ...
    const candidates = [
        ...
        join(getOmgRoot(root), `${normalizedName}.json`),
    ];
