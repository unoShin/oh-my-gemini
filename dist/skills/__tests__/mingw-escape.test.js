...
describe('MINGW64 escape safety: no "!" in node -e inline scripts (issue #729)', () => {
    ...
    describe('skills/omg-setup (SKILL.md + phases)', () => {
        const setupDir = join(REPO_ROOT, 'skills', 'omg-setup');
        const filesToScan = [
        ...
        it('has no "!" in any node -e script body (MINGW64 safe)', () => {
            const violations = findBangViolations(allScripts, 'omg-setup/*');
            if (violations.length > 0) {
                expect.fail('Found "!" in node -e scripts (breaks MINGW64/Git Bash):\n' +
                    violations.map(v => `  • ${v}`).join('\n'));
            }
    ...
    describe('specific regressions (issue #729)', () => {
        ...
        it('hud SKILL.md keeps Unix statusLine guidance portable while preserving Windows-safe paths', () => {
            const content = readFileSync(join(REPO_ROOT, 'skills', 'hud', 'SKILL.md'), 'utf-8');
            expect(content).toContain('"command": "node $HOME/.gemini/hud/omg-hud.mjs"');
            expect(content).toContain('"command": "node C:/Users/username/.gemini/hud/omg-hud.mjs"');
            expect(content).not.toContain('"command": "node /home/username/.gemini/hud/omg-hud.mjs"');
            expect(content).not.toContain('The command must use an absolute path, not `~`');
        });
        it("omg-setup version-detect script uses v==='' not !v", () => {
            const setupDir = join(REPO_ROOT, 'skills', 'omg-setup');
            const files = [
                join(setupDir, 'SKILL.md'),
                ...readdirSync(join(setupDir, 'phases')).map(f => join(setupDir, 'phases', f)),
        ...
        it('omg-setup extracts GEMINI.md version from OMG marker', () => {
            const setupDir = join(REPO_ROOT, 'skills', 'omg-setup');
            const files = [
                join(setupDir, 'SKILL.md'),
                ...readdirSync(join(setupDir, 'phases')).map(f => join(setupDir, 'phases', f)),
                join(REPO_ROOT, 'scripts', 'setup-gemini-md.sh'),
            ].filter(f => f.endsWith('.md') || f.endsWith('.sh'));
            const combined = files.map(f => readFileSync(f, 'utf-8')).join('\n');
            expect(combined).toContain("grep -m1 'OMG:VERSION:'");
            expect(combined).not.toContain('grep -m1 "^# oh-my-gemini"');
        });
        ...
        it('omg-setup phase 2 delegates HUD setup instead of inlining statusLine formatting', () => {
            const content = readFileSync(join(REPO_ROOT, 'skills', 'omg-setup', 'phases', '02-configure.md'), 'utf-8');
            expect(content).toContain('Use the Skill tool to invoke: `hud` with args: `setup`');
            expect(content).toContain('Configure `statusLine` in `~/.gemini/settings.json`');
            expect(content).not.toContain('Read `~/.gemini/settings.json`, then update/add the `statusLine` field.');
