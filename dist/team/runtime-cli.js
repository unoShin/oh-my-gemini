...
export async function writeResultArtifact(output, finishedAt, jobId = process.env.OMG_JOB_ID, omgJobsDir = process.env.OMG_JOBS_DIR) {
    if (!jobId || !omgJobsDir)
        return;
    const resultPath = join(omgJobsDir, `${jobId}-result.json`);
...
async function main() {
    ...
    // Persist pane IDs so MCP server can clean up explicitly via omg_run_team_cleanup.
