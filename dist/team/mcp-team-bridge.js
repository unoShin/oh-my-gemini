 ...
 * against tmux-based CLI workers (Gemini CLI, Gemini CLI) directly.
 * This file is retained for the tmux bridge daemon functionality.
 *
 * MCP Team Bridge Daemon
 *
 * Core bridge process that runs in a tmux session alongside a Gemini/Gemini CLI.
...
/** Model name validation regex (matches gemini-core.ts pattern) */
...
function validateProvider(provider) {
    if (provider !== "gemini" && provider !== "gemini") {
        throw new Error(`Invalid provider: ${provider}. Must be 'gemini' or 'gemini'`);
    }
}
...
/** Maximum accumulated size for parseGeminiOutput (1MB) */
const MAX_GEMINI_OUTPUT_SIZE = 1024 * 1024;
/** Parse Gemini JSONL output to extract text responses */
