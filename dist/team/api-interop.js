...
function readTeamStateRootFromEnv(env = process.env) {
    const candidate = typeof env.OMG_TEAM_STATE_ROOT === 'string' && env.OMG_TEAM_STATE_ROOT.trim() !== ''
        ? env.OMG_TEAM_STATE_ROOT.trim()
...
export function resolveTeamApiCliCommand(env = process.env) {
    const hasOmgContext = ((typeof env.OMG_TEAM_WORKER === 'string' && env.OMG_TEAM_WORKER.trim() !== '')
        || (typeof env.OMG_TEAM_STATE_ROOT === 'string' && env.OMG_TEAM_STATE_ROOT.trim() !== ''));
    if (hasOmgContext)
        return 'omg team api';
