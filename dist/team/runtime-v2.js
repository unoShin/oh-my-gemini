 ...
 * Runtime selection:
 * - Default: v2 enabled
 * - Opt-out: set OMG_RUNTIME_V2=0|false|no|off to force legacy v1
...
import { buildWorkerArgv, resolveValidatedBinaryPath, getWorkerEnv as getModelWorkerEnv, isPromptModeAgent, getPromptModeArgs, resolveGeminiWorkerModel, } from './model-contract.js';
...
function buildV2TaskInstruction(teamName, workerName, task, taskId) {
    const claimTaskCommand = formatOmgCliInvocation(`team api claim-task --input '${JSON.stringify({ team_name: teamName, task_id: taskId, worker: workerName })}' --json`, {});
    const completeTaskCommand = formatOmgCliInvocation(`team api transition-task-status --input '${JSON.stringify({ team_name: teamName, task_id: taskId, from: 'in_progress', to: 'completed', claim_token: '<claim_token>' })}' --json`);
    const failTaskCommand = formatOmgCliInvocation(`team api transition-task-status --input '${JSON.stringify({ team_name: teamName, task_id: taskId, from: 'in_progress', to: 'failed', claim_token: '<claim_token>' })}' --json`);
    return [
...
async function spawnV2Worker(opts) {
    ...
    // For Gemini agents on Bedrock/Vertex, resolve the provider-specific model.
    const modelForAgent = (() => {
        if (opts.agentType === 'gemini') {
            return process.env.OMG_EXTERNAL_MODELS_DEFAULT_ANTHROPIC_MODEL
                || process.env.OMG_GEMINI_DEFAULT_MODEL
                || resolveGeminiWorkerModel();
        }
        return undefined;
    })();
    ...
    // For prompt-mode agents (gemini, gemini), pass instruction via CLI flag
    if (usePromptMode) {
    ...
    if (unownedTaskIndices.length > 0) {
        ...
        const allocationWorkers = workerNames.map((name, i) => ({
            ...
            role: config.workerRoles?.[i]
                ?? (agentTypes[i % agentTypes.length] ?? agentTypes[0] ?? 'gemini'),
            currentLoad: 0,
        }));
