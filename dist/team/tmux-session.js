 ...
 *   node dist/team/bridge-entry.js --config /tmp/omg-bridge-{worker}.json
 */
...
export async function createTeamSession(teamName, workerCount, cwd, options = {}) {
    ...
    if (!inTmux) {
        ...
        // pane/window that OMG can split directly.
 ...
 * Worker startup: env OMG_TEAM_WORKER={teamName}/workerName shell -lc "exec agentCmd"
 */
...
export async function waitForPaneReady(paneId, opts = {}) {
    ...
    console.warn(`[tmux-session] waitForPaneReady: pane ${paneId} timed out after ${timeoutMs}ms ` +
        `(set OMG_SHELL_READY_TIMEOUT_MS to tune)`);
    return false;
}
...
export function shouldAttemptAdaptiveRetry(args) {
    if (process.env.OMG_TEAM_AUTO_INTERRUPT_RETRY === '0')
        return false;
...
/**
 * Inject a status message into the leader Gemini pane.
 * The message is typed into the leader's input, triggering a new conversation turn.
 * Prefixes with [OMG_TMUX_INJECT] marker to distinguish from user input.
 * Returns false on error (does not throw).
 */
export async function injectToLeaderPane(sessionName, leaderPaneId, message) {
    ...
    // If the leader is running a blocking tool (e.g. omg_run_team_wait shows
