...
describe('team api dispatch-aware messaging', () => {
    ...
    it('uses OMG_TEAM_STATE_ROOT placeholder in mailbox triggers for worktree-backed workers', async () => {
        ...
        const requests = await listDispatchRequests(teamName, cwd, { kind: 'mailbox', to_worker: 'worker-1' });
        expect(requests).toHaveLength(1);
        expect(requests[0]?.trigger_message).toContain('$OMG_TEAM_STATE_ROOT/team/dispatch-team/mailbox/worker-1.json');
