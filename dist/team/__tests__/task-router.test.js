...
describe('task-router', () => {
    ...
    function registerWorker(name, provider = 'gemini', status = 'polling') {
        registerMcpWorker(teamName, name, provider, provider === 'gemini' ? 'gpt-5.3-gemini' : 'gemini-3-pro', `${teamName}-${name}`, testDir, testDir);
    ...
    describe('routeTasks', () => {
        ...
        it('routes to gemini worker for code review capabilities', () => {
        ...
        it('excludes quarantined workers', () => {
            registerWorker('gemini-1', 'gemini', 'quarantined');
            registerWorker('gemini-2', 'gemini');
            const tasks = [makeTask('t1', 'Review code')];
            const decisions = routeTasks(teamName, testDir, tasks, {
                t1: ['code-review'],
            });
            expect(decisions).toHaveLength(1);
            expect(decisions[0].assignedTo).toBe('gemini-2');
        });
        ...
        it('uses general capability as fallback', () => {
            ...
            const decisions = routeTasks(teamName, testDir, tasks);
            // Gemini doesn't have 'general' capability, so no match
