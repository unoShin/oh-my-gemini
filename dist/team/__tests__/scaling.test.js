...
describe('scaleUp duplicate worker guard', () => {
    ...
    it('refuses to spawn a duplicate worker identity when next_worker_index collides', async () => {
        cwd = await mkdtemp(join(tmpdir(), 'omg-scaling-duplicate-'));
        ...
        const result = await scaleUp(teamName, 1, 'gemini', [{ subject: 'demo', description: 'demo task' }], cwd, { OMG_TEAM_SCALING_ENABLED: '1' });
