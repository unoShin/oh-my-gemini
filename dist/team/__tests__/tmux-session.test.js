...
describe('sessionName', () => {
    it('builds correct session name', () => {
        expect(sessionName('myteam', 'gemini1')).toBe('omg-team-myteam-gemini1');
    });
...
describe('buildWorkerStartCommand', () => {
    ...
    it('accepts absolute Windows launchBinary paths with spaces', () => {
        ...
        expect(() => buildWorkerStartCommand({
            teamName: 't',
            workerName: 'w',
            envVars: { OMG_TEAM_WORKER: 't/w' },
            launchBinary: 'C:\\Program Files\\OpenAI\\Gemini\\gemini.exe',
    ...
    it('uses exec \"$@\" for launchBinary with non-fish shells', () => {
        ...
        expect(cmd).toContain("'--' 'gemini' '--full-auto'");
    });
    ...
    it('does not double-escape env vars in launchBinary mode (issue #1415)', () => {
        ...
        const cmd = buildWorkerStartCommand({
            ...
            workerName: 'w',
            envVars: {
                ANTHROPIC_MODEL: 'us.anthropic.gemini-pro-4-6-v1[1m]',
                GEMINI_CODE_USE_BEDROCK: '1',
            },
            launchBinary: '/usr/local/bin/gemini',
            launchArgs: ['--dangerously-skip-permissions'],
            cwd: '/tmp'
        });
        // env assignments must appear WITHOUT extra wrapping quotes.
        // Correct:   ANTHROPIC_MODEL='us.anthropic.gemini-pro-4-6-v1[1m]'
        // Wrong:     'ANTHROPIC_MODEL='"'"'us.anthropic...'"'"''  (double-escaped)
        expect(cmd).toContain("ANTHROPIC_MODEL='us.anthropic.gemini-pro-4-6-v1[1m]'");
        expect(cmd).toContain("GEMINI_CODE_USE_BEDROCK='1'");
        // The env keyword and other args should still be shell-escaped
        expect(cmd).toMatch(/^'env'/);
        expect(cmd).toContain("'/usr/local/bin/gemini'");
    ...
    it('env vars with special characters survive single escaping correctly', () => {
        ...
        const cmd = buildWorkerStartCommand({
            ...
            workerName: 'w',
            envVars: {
                OMG_TEAM_WORKER: 'my-team/worker-1',
                ANTHROPIC_DEFAULT_PRO_MODEL: 'global.anthropic.gemini-pro-4-6[1m]',
            },
            launchBinary: '/usr/local/bin/gemini',
            launchArgs: [],
            cwd: '/tmp'
        ...
        expect(cmd).toContain("OMG_TEAM_WORKER='my-team/worker-1'");
        expect(cmd).toContain("ANTHROPIC_DEFAULT_PRO_MODEL='global.anthropic.gemini-pro-4-6[1m]'");
    });
    it('rejects relative launchBinary containing spaces', () => {
        ...
        expect(() => buildWorkerStartCommand({
            ...
            workerName: 'w',
            envVars: {},
            launchBinary: 'Program Files/gemini',
            cwd: '/tmp'
    ...
    it('rejects dangerous shell metacharacters in launchBinary', () => {
        ...
        expect(() => buildWorkerStartCommand({
            ...
            workerName: 'w',
            envVars: {},
            launchBinary: '/usr/bin/gemini;touch /tmp/pwn',
            cwd: '/tmp'
...
describe('shouldAttemptAdaptiveRetry', () => {
    it('only enables adaptive retry for busy panes with visible unsent message', () => {
        delete process.env.OMG_TEAM_AUTO_INTERRUPT_RETRY;
        ...
        expect(shouldAttemptAdaptiveRetry({
            paneBusy: true,
            latestCapture: '❯ check-inbox\ngpt-5.3-gemini high · 80% left',
            message: 'check-inbox',
            paneInCopyMode: false,
            retriesAttempted: 0,
        })).toBe(true);
    });
    it('respects OMG_TEAM_AUTO_INTERRUPT_RETRY=0', () => {
        process.env.OMG_TEAM_AUTO_INTERRUPT_RETRY = '0';
        expect(shouldAttemptAdaptiveRetry({
            paneBusy: true,
            latestCapture: '❯ check-inbox',
            message: 'check-inbox',
...
describe('sendToWorker implementation guards', () => {
    ...
    it('supports env-gated adaptive interrupt retry', () => {
        expect(source).toContain('OMG_TEAM_AUTO_INTERRUPT_RETRY');
