...
describe('worker-bootstrap', () => {
    ...
    describe('generateWorkerOverlay', () => {
        ...
        it('supports state-root placeholders for worktree-backed trigger paths', () => {
            expect(generateTriggerMessage('test-team', 'worker-1', '$OMG_TEAM_STATE_ROOT'))
                .toContain('$OMG_TEAM_STATE_ROOT/team/test-team/workers/worker-1/inbox.md');
            expect(generateTriggerMessage('test-team', 'worker-1', '$OMG_TEAM_STATE_ROOT'))
                .toContain('work now');
            expect(generateMailboxTriggerMessage('test-team', 'worker-1', 2, '$OMG_TEAM_STATE_ROOT'))
                .toContain('$OMG_TEAM_STATE_ROOT/team/test-team/mailbox/worker-1.json');
            expect(generateMailboxTriggerMessage('test-team', 'worker-1', 2, '$OMG_TEAM_STATE_ROOT'))
        ...
        it('renders plugin-safe CLI lifecycle examples when omg is unavailable in plugin installs', () => {
            process.env.GEMINI_PLUGIN_ROOT = '/plugin-root';
            process.env.PATH = '';
            const overlay = generateWorkerOverlay(baseParams);
            expect(overlay).toContain('node "$GEMINI_PLUGIN_ROOT"/bridge/cli.cjs team api read-task');
            expect(overlay).toContain('node "$GEMINI_PLUGIN_ROOT"/bridge/cli.cjs team api claim-task');
            expect(overlay).toContain('node "$GEMINI_PLUGIN_ROOT"/bridge/cli.cjs team api transition-task-status');
        });
    });
    describe('getWorkerEnv', () => {
        it('returns correct env vars', () => {
            const env = getWorkerEnv('my-team', 'worker-2', 'gemini');
            expect(env.OMG_TEAM_WORKER).toBe('my-team/worker-2');
