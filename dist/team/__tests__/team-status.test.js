...
afterEach(() => {
    ...
    // Clean up outbox files written to ~/.gemini/teams/ by appendOutbox
    rmSync(join(getGeminiConfigDir(), 'teams', TEST_TEAM), { recursive: true, force: true });
});
...
function makeWorker(name, provider = 'gemini') {
    return {
...
function makeHeartbeat(workerName, provider = 'gemini', ageMs = 0) {
    return {
        workerName,
        teamName: TEST_TEAM,
        provider,
...
describe('getTeamStatus', () => {
    ...
    it('aggregates worker status with heartbeats and tasks', () => {
        const w1 = makeWorker('w1', 'gemini');
        const w2 = makeWorker('w2', 'gemini');
        writeWorkerRegistry([w1, w2]);
        // Write heartbeats (fresh)
        writeHeartbeatFile(makeHeartbeat('w1', 'gemini', 1000));
        ...
        expect(sw1.provider).toBe('gemini');
    ...
    it('detects dead workers via heartbeat age', () => {
        ...
        writeHeartbeatFile(makeHeartbeat('w1', 'gemini', 60000));
        const status = getTeamStatus(TEST_TEAM, WORK_DIR);
        const sw1 = status.workers.find(w => w.workerName === 'w1');
    ...
    it('respects custom heartbeatMaxAgeMs', () => {
        ...
        writeHeartbeatFile(makeHeartbeat('w1', 'gemini', 10000));
