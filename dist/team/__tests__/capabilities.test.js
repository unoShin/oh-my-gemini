...
describe('capabilities', () => {
    describe('getDefaultCapabilities', () => {
        it('returns capabilities for gemini-native', () => {
            const caps = getDefaultCapabilities('gemini-native');
        ...
        it('returns a copy, not a reference', () => {
            const caps1 = getDefaultCapabilities('gemini-native');
            const caps2 = getDefaultCapabilities('gemini-native');
            caps1.push('research');
    ...
    describe('scoreWorkerFitness', () => {
        it('returns 1.0 for exact match', () => {
            const worker = makeMember('w1', 'mcp-gemini', ['code-review', 'security-review']);
            const score = scoreWorkerFitness(worker, ['code-review', 'security-review']);
            expect(score).toBe(1.0);
        });
        it('returns 0.5 for partial match', () => {
            const worker = makeMember('w1', 'mcp-gemini', ['code-review']);
        ...
        it('gives partial credit for general capability', () => {
            const worker = makeMember('w1', 'gemini-native', ['general']);
        ...
        it('returns 1.0 when no capabilities required', () => {
            const worker = makeMember('w1', 'gemini-native', ['code-edit']);
    ...
    describe('rankWorkersForTask', () => {
        ...
        it('respects custom capabilities over defaults', () => {
            const w1 = makeMember('custom', 'gemini-native', ['security-review', 'architecture']);
            const w2 = makeMember('default', 'mcp-gemini', ['code-review']);
