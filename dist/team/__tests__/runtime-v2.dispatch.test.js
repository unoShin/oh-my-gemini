...
const modelContractMocks = vi.hoisted(() => ({
    buildWorkerArgv: vi.fn(() => ['/usr/bin/gemini']),
    resolveValidatedBinaryPath: vi.fn(() => '/usr/bin/gemini'),
    getWorkerEnv: vi.fn(() => ({ OMG_TEAM_WORKER: 'dispatch-team/worker-1' })),
...
describe('runtime v2 startup inbox dispatch', () => {
    ...
    beforeEach(() => {
        ...
        modelContractMocks.buildWorkerArgv.mockImplementation((agentType) => [`/usr/bin/${agentType ?? 'gemini'}`]);
        modelContractMocks.resolveValidatedBinaryPath.mockImplementation((agentType) => `/usr/bin/${agentType ?? 'gemini'}`);
        modelContractMocks.getWorkerEnv.mockImplementation((...args) => {
            ...
            return { OMG_TEAM_WORKER: `${teamName}/${workerName}` };
        });
    ...
    it('writes durable inbox dispatch evidence when startup worker notification succeeds', async () => {
        ...
        expect(mocks.spawnWorkerInPane).toHaveBeenCalledWith('dispatch-session', '%2', expect.objectContaining({
            envVars: expect.objectContaining({
                OMG_TEAM_WORKER: 'dispatch-team/worker-1',
                OMG_TEAM_STATE_ROOT: join(cwd, '.omg', 'state', 'team', 'dispatch-team'),
                OMG_TEAM_LEADER_CWD: cwd,
            }),
        }));
    });
    it('uses owner-aware startup allocation when task owners are provided', async () => {
        cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-v2-owner-startup-'));
        const { startTeamV2 } = await import('../runtime-v2.js');
        const runtime = await startTeamV2({
            teamName: 'dispatch-team',
            workerCount: 2,
        ...
        const spawnedWorkers = mocks.spawnWorkerInPane.mock.calls.map((call) => call[2]?.envVars?.OMG_TEAM_WORKER);
    ...
    it('preserves explicit worker roles in runtime config during startup fanout', async () => {
        cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-v2-worker-roles-'));
        const { startTeamV2 } = await import('../runtime-v2.js');
        const runtime = await startTeamV2({
            teamName: 'dispatch-team',
            workerCount: 2,
    ...
    it('requires Gemini startup evidence beyond the initial notify and retries once before failing', async () => {
        cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-v2-gemini-evidence-missing-'));
        const { startTeamV2 } = await import('../runtime-v2.js');
        const runtime = await startTeamV2({
            teamName: 'dispatch-team',
            workerCount: 1,
            agentTypes: ['gemini'],
            tasks: [{ subject: 'Dispatch test', description: 'Verify Gemini startup evidence gate' }],
            cwd,
        });
        expect(runtime.config.workers[0]?.pane_id).toBe('%2');
        expect(runtime.config.workers[0]?.assigned_tasks).toEqual([]);
    ...
    it('does not treat ACK-only mailbox replies as Gemini startup evidence', async () => {
        cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-v2-gemini-evidence-ack-'));
        ...
        const runtime = await startTeamV2({
            ...
            workerCount: 1,
            agentTypes: ['gemini'],
            tasks: [{ subject: 'Dispatch test', description: 'Verify Gemini mailbox ack evidence' }],
            cwd,
        });
        expect(runtime.config.workers[0]?.assigned_tasks).toEqual([]);
        expect(mocks.sendToWorker).toHaveBeenCalledTimes(2);
    });
    it('accepts Gemini startup once the worker claims the task', async () => {
        cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-v2-gemini-evidence-claim-'));
        mocks.sendToWorker.mockImplementation(async () => {
        ...
        const runtime = await startTeamV2({
            ...
            workerCount: 1,
            agentTypes: ['gemini'],
            tasks: [{ subject: 'Dispatch test', description: 'Verify Gemini claim evidence' }],
            cwd,
        });
        expect(runtime.config.workers[0]?.assigned_tasks).toEqual(['1']);
        expect(mocks.sendToWorker).toHaveBeenCalledTimes(1);
    });
    it('accepts Gemini startup once worker status shows task progress', async () => {
        cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-v2-gemini-evidence-status-'));
        mocks.sendToWorker.mockImplementation(async () => {
        ...
        const runtime = await startTeamV2({
            ...
            workerCount: 1,
            agentTypes: ['gemini'],
            tasks: [{ subject: 'Dispatch test', description: 'Verify Gemini status evidence' }],
            cwd,
        });
        expect(runtime.config.workers[0]?.assigned_tasks).toEqual(['1']);
        expect(mocks.sendToWorker).toHaveBeenCalledTimes(1);
    });
    it('passes the full lifecycle instruction to gemini prompt-mode workers and waits for claim evidence', async () => {
        cwd = await mkdtemp(join(tmpdir(), 'omg-runtime-v2-gemini-prompt-'));
        ...
        const runtime = await startTeamV2({
            ...
            workerCount: 1,
            agentTypes: ['gemini'],
            tasks: [{ subject: 'Dispatch test', description: 'Verify gemini lifecycle prompt mode' }],
            cwd,
        });
        expect(modelContractMocks.getPromptModeArgs).toHaveBeenCalledWith('gemini', expect.stringContaining('team api claim-task'));
        expect(modelContractMocks.getPromptModeArgs).toHaveBeenCalledWith('gemini', expect.stringContaining('transition-task-status'));
        expect(mocks.spawnWorkerInPane).toHaveBeenCalledWith('dispatch-session', '%2', expect.objectContaining({
