...
describe('message-router', () => {
    ...
    afterEach(() => {
        ...
        try {
            const inboxDir = join(homedir(), '.gemini', 'teams', teamName, 'inbox');
            rmSync(inboxDir, { recursive: true, force: true });
        }
        catch { /* ignore */ }
    });
    function registerWorker(name, agentType = 'mcp-gemini') {
        const provider = agentType === 'mcp-gemini' ? 'gemini' : 'gemini';
        registerMcpWorker(teamName, name, provider, 'gpt-5.3-gemini', `${teamName}-${name}`, testDir, testDir);
    ...
    describe('routeMessage', () => {
        it('routes to MCP worker via inbox', () => {
            ...
            const result = routeMessage(teamName, 'gemini-1', 'Hello worker', testDir);
            expect(result.method).toBe('inbox');
            expect(result.details).toContain('inbox');
            // Verify inbox file was written
            const inboxPath = join(homedir(), '.gemini', 'teams', teamName, 'inbox', 'gemini-1.jsonl');
    ...
    describe('broadcastToTeam', () => {
        it('broadcasts to all MCP workers', () => {
            ...
            const inbox1 = join(homedir(), '.gemini', 'teams', teamName, 'inbox', 'worker1.jsonl');
            const inbox2 = join(homedir(), '.gemini', 'teams', teamName, 'inbox', 'worker2.jsonl');
