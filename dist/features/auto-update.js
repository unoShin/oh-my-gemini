 ...
 * Auto-Update System
 *
 * Provides version checking and auto-update functionality for oh-my-gemini.
 *
 * Features:
...
import { install as installOmg, HOOKS_DIR, isProjectScopedPlugin, isRunningAsPlugin, getInstalledOmgPluginRoots, getRuntimePackageRoot, } from '../installer/index.js';
import { getConfigDir } from '../utils/config-dir.js';
import { purgeStalePluginCacheVersions } from '../utils/paths.js';
/** GitHub repository information */
export const REPO_OWNER = 'unoShin';
export const REPO_NAME = 'oh-my-gemini';
...
/**
 * Best-effort sync of the Gemini Code marketplace clone.
 * The marketplace clone at ~/.gemini/plugins/marketplaces/omg/ is used by
 * Gemini Code to populate the plugin cache. If it's stale, `/plugin install`
...
function syncActivePluginCache() {
    const activeRoots = getInstalledOmgPluginRoots().filter(root => existsSync(root));
    if (activeRoots.length === 0) {
        return { synced: false, errors: [] };
    }
    const result = copyPluginSyncPayload(getRuntimePackageRoot(), activeRoots);
    if (result.synced) {
        console.log('[omg update] Synced plugin cache');
    }
    return result;
}
export function shouldBlockStandaloneUpdateInCurrentSession() {
    ...
    const entrypoint = process.env.GEMINI_CODE_ENTRYPOINT?.trim();
    if (entrypoint) {
        return true;
    }
    const sessionId = process.env.GEMINI_SESSION_ID?.trim() || process.env.GEMINICODE_SESSION_ID?.trim();
    if (sessionId) {
        return true;
    }
    return false;
...
export function syncPluginCache(verbose = false) {
    const pluginCacheRoot = join(getConfigDir(), 'plugins', 'cache', 'omg', 'oh-my-gemini');
    ...
    try {
        ...
        const sourceRoot = join(npmRoot, 'oh-my-gemini');
        ...
        if (result.errors.length > 0) {
            for (const error of result.errors) {
                console.warn(`[omg update] Plugin cache sync warning: ${error}`);
            }
        }
        if (result.synced) {
            console.log('[omg update] Plugin cache synced');
        }
        ...
        const message = error instanceof Error ? error.message : String(error);
        if (verbose) {
            console.warn(`[omg update] Plugin cache sync warning: ${message}`);
        }
        else {
            console.warn('[omg update] Plugin cache sync warning:', message);
        }
...
/** Installation paths (respects GEMINI_CONFIG_DIR env var) */
export const GEMINI_CONFIG_DIR = getConfigDir();
export const VERSION_FILE = join(GEMINI_CONFIG_DIR, '.omg-version.json');
export const CONFIG_FILE = join(GEMINI_CONFIG_DIR, '.omg-config.json');
/**
 * Read the OMG configuration
 */
export function getOMGConfig() {
...
export function isSilentAutoUpdateEnabled() {
    return getOMGConfig().silentAutoUpdate;
}
/**
...
export function isAutoUpgradePromptEnabled() {
    return getOMGConfig().autoUpgradePrompt !== false;
}
/**
 * Check if team feature is enabled
 * Returns false by default - requires explicit opt-in
 * Checks ~/.gemini/settings.json first, then env var fallback
 */
export function isTeamEnabled() {
    try {
        ...
        if (existsSync(settingsPath)) {
            ...
            const val = settings.env?.GEMINI_CODE_EXPERIMENTAL_AGENT_TEAMS;
            if (val === '1' || val === 'true') {
                return true;
            }
        }
    ...
    const envVal = process.env.GEMINI_CODE_EXPERIMENTAL_AGENT_TEAMS;
...
export function getInstalledVersion() {
    if (!existsSync(VERSION_FILE)) {
        ...
        try {
            ...
            const result = execSync('npm list -g oh-my-gemini --json', {
                encoding: 'utf-8',
                timeout: 5000,
                stdio: 'pipe'
            });
            const data = JSON.parse(result);
            if (data.dependencies?.['oh-my-gemini']?.version) {
                return {
                    version: data.dependencies['oh-my-gemini'].version,
...
export async function fetchLatestRelease() {
    const response = await fetch(`${GITHUB_API_URL}/releases/latest`, {
        headers: {
            ...
            'User-Agent': 'oh-my-gemini-updater'
        }
    });
...
export function reconcileUpdateRuntime(options) {
    ...
    try {
        const installResult = installOmg({
            force: true,
    ...
    try {
        ...
        if (pluginSyncResult.errors.length > 0 && options?.verbose) {
            for (const err of pluginSyncResult.errors) {
                console.warn(`[omg] Plugin cache sync warning: ${err}`);
            }
        }
    }
    catch (error) {
        if (options?.verbose) {
            const message = error instanceof Error ? error.message : String(error);
            console.warn(`[omg] Plugin cache sync warning: ${message}`);
        }
    }
...
function getFirstResolvedBinaryPath(output) {
    const resolved = output
        ...
        .find(Boolean);
    if (!resolved) {
        throw new Error('Unable to resolve omg binary path for update reconciliation');
    }
    return resolved;
}
function resolveOmgBinaryPath() {
...
export async function performUpdate(options) {
    ...
    try {
        // Block npm update only from active Gemini Code/plugin sessions.
        // Standalone terminals may inherit GEMINI_PLUGIN_ROOT and should still update.
        if (shouldBlockStandaloneUpdateInCurrentSession() && !options?.standalone) {
            return {
                success: false,
                previousVersion,
                newVersion: 'unknown',
                message: 'Running inside an active Gemini Code plugin session. Use "/plugin install oh-my-gemini" to update, or pass --standalone to force npm update.',
            };
        }
        ...
        try {
            execSync('npm install -g oh-my-gemini@latest', {
                encoding: 'utf-8',
                stdio: options?.verbose ? 'inherit' : 'pipe',
                timeout: 120000, // 2 minute timeout for npm
                ...(process.platform === 'win32' ? { windowsHide: true } : {})
            });
            // Sync Gemini Code marketplace clone so plugin cache picks up new version (#506)
            const marketplaceSync = syncMarketplaceClone(options?.verbose ?? false);
            if (!marketplaceSync.ok && options?.verbose) {
                console.warn(`[omg update] ${marketplaceSync.message}`);
            }
            ...
            // with the NEW code. Otherwise, installOmg() runs OLD logic against NEW files.
            if (!process.env.OMG_UPDATE_RECONCILE) {
                // Set flag to prevent infinite loop
                process.env.OMG_UPDATE_RECONCILE = '1';
                // Find the omg binary path
                const omgPath = resolveOmgBinaryPath();
                // Re-exec with reconcile subcommand
                try {
                    execFileSync(omgPath, ['update-reconcile', ...(options?.clean ? ['--skip-grace-period'] : [])], {
                        encoding: 'utf-8',
                        stdio: options?.verbose ? 'inherit' : 'pipe',
                        timeout: 60000,
                        env: { ...process.env, OMG_UPDATE_RECONCILE: '1' },
            ...
            throw new Error('Auto-update via npm failed. Please run manually:\n' +
                '  npm install -g oh-my-gemini@latest\n' +
                'Or use: /plugin install oh-my-gemini\n' +
...
export function formatUpdateNotification(checkResult) {
    if (!checkResult.updateAvailable) {
        return `oh-my-gemini is up to date (v${checkResult.currentVersion ?? 'unknown'})`;
    }
    const lines = [
        '╔═══════════════════════════════════════════════════════════╗',
        '║           oh-my-gemini Update Available!              ║',
        '╚═══════════════════════════════════════════════════════════╝',
        '',
        ...
        '  Or reinstall via: /plugin install oh-my-gemini',
        ''
    ];
...
const SILENT_UPDATE_STATE_FILE = join(GEMINI_CONFIG_DIR, '.omg-silent-update.json');
/**
...
export async function silentAutoUpdate(config = {}) {
    const { checkIntervalHours = 24, autoApply = true, logFile = join(GEMINI_CONFIG_DIR, '.omg-update.log'), maxRetries = 3 } = config;
