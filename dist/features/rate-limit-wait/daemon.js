 ...
 * Gemini Code sessions when rate limits reset.
 *
...
const DEFAULT_CONFIG = {
    ...
    verbose: false,
    stateFilePath: getGlobalOmgStatePath('rate-limit-daemon.json'),
    pidFilePath: getGlobalOmgStatePath('rate-limit-daemon.pid'),
    logFilePath: getGlobalOmgStatePath('rate-limit-daemon.log'),
};
...
export function startDaemon(config) {
    ...
    import('${modulePath}').then(async ({ pollLoopWithConfigFile }) => {
      await pollLoopWithConfigFile(process.env.OMG_DAEMON_CONFIG_FILE);
  ...
  `;
    try {
        ...
        const daemonEnv = {
            ...
            OMG_DAEMON_CONFIG_FILE: configPath,
        };
