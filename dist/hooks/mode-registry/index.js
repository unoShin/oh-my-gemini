...
import { listSessionIds, resolveSessionStatePath, getSessionStateDir, getOmgRoot, } from "../../lib/worktree-paths.js";
...
export function getStateDir(cwd) {
    return join(getOmgRoot(cwd), "state");
}
/**
...
/**
 * Get the global state file path (in ~/.gemini/) for modes that support it
        ...
        for (const exclusiveMode of EXCLUSIVE_MODES) {
            if (exclusiveMode !== mode &&
                ...
                return {
                    ...
                    message: `Cannot start ${MODE_CONFIGS[mode].name} while ${config.name} is active. Cancel ${config.name} first with /oh-my-gemini:cancel.`,
                };
            }
        }
    }
 ...
 * - Global state file if applicable (~/.gemini/{mode}-state.json)
 *
