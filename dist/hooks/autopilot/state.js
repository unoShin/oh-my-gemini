...
import { resolveStatePath, resolveSessionStatePath, getOmgRoot, } from "../../lib/worktree-paths.js";
...
export function ensureAutopilotDir(directory) {
    const autopilotDir = join(getOmgRoot(directory), SPEC_DIR);
...
export function getSpecPath(directory) {
    return join(getOmgRoot(directory), SPEC_DIR, "spec.md");
}
/**
...
/**
 * Get a prompt for Gemini to execute the transition
 */
...
Task(subagent_type="oh-my-gemini:architect", model="ultra",
  prompt="FUNCTIONAL COMPLETENESS REVIEW: Verify all requirements from spec are implemented")

Task(subagent_type="oh-my-gemini:security-reviewer", model="ultra",
  prompt="SECURITY REVIEW: Check for vulnerabilities, injection risks, auth issues")

Task(subagent_type="oh-my-gemini:code-reviewer", model="ultra",
