 ...
 * and respects the OMG_SKIP_HOOKS env kill-switch.
 */
...
describe('processHook - Routing Matrix', () => {
    ...
    describe('HookType routing', () => {
        ...
        it('should activate ralph and linked ultrawork when Skill tool invokes ralph', async () => {
            ...
            try {
                ...
                const input = {
                    sessionId,
                    toolName: 'Skill',
                    toolInput: { skill: 'oh-my-gemini:ralph' },
                    directory: tempDir,
                };
                const result = await processHook('post-tool-use', input);
                expect(result.continue).toBe(true);
        ...
        it('activates ralplan state when Skill tool invokes ralplan directly', async () => {
            ...
            try {
                ...
                const result = await processHook('pre-tool-use', {
                    sessionId,
                    toolName: 'Skill',
                    toolInput: { skill: 'oh-my-gemini:ralplan' },
                    directory: tempDir,
                });
                expect(result.continue).toBe(true);
        ...
        it('activates ralplan state when Skill tool invokes omg-plan in consensus mode', async () => {
    ...
    describe('input normalization', () => {
        it('should normalize snake_case tool_name to camelCase toolName', async () => {
            // Send snake_case input (as Gemini Code would)
            const rawInput = {
    ...
    // --------------------------------------------------------------------------
    // OMG_SKIP_HOOKS environment variable
    // --------------------------------------------------------------------------
    describe('OMG_SKIP_HOOKS kill-switch', () => {
        it('should skip a specific hook type when listed', async () => {
            process.env.OMG_SKIP_HOOKS = 'keyword-detector';
            const input = {
                sessionId: 'test-session',
        ...
        it('should skip multiple comma-separated hooks', async () => {
            process.env.OMG_SKIP_HOOKS = 'keyword-detector,pre-tool-use,post-tool-use';
            const input = {
                sessionId: 'test-session',
                toolName: 'Bash',
                toolInput: { command: 'ls' },
        ...
        it('should handle whitespace around hook names', async () => {
            process.env.OMG_SKIP_HOOKS = ' keyword-detector , pre-tool-use ';
            const input = {
                sessionId: 'test-session',
                prompt: 'ultrawork',
                directory: '/tmp/test-routing',
        ...
        it('should process normally with empty OMG_SKIP_HOOKS', async () => {
            process.env.OMG_SKIP_HOOKS = '';
            const input = {
                sessionId: 'test-session',
                prompt: 'hello world',
                directory: '/tmp/test-routing',
    ...
    // --------------------------------------------------------------------------
    // DISABLE_OMG env kill-switch
    // --------------------------------------------------------------------------
    describe('DISABLE_OMG kill-switch', () => {
        it('should return continue:true for all hooks when DISABLE_OMG=1', async () => {
            process.env.DISABLE_OMG = '1';
            const input = {
                sessionId: 'test-session',
                prompt: 'ultrawork this',
                directory: '/tmp/test-routing',
        ...
        it('should return continue:true when DISABLE_OMG=true', async () => {
            process.env.DISABLE_OMG = 'true';
            const input = {
                sessionId: 'test-session',
                prompt: 'test',
                directory: '/tmp/test-routing',
        ...
        it('should process normally when DISABLE_OMG=false', async () => {
            process.env.DISABLE_OMG = 'false';
            const input = {
                sessionId: 'test-session',
                prompt: 'hello world',
                directory: '/tmp/test-routing',
        ...
        it('DISABLE_OMG takes precedence over OMG_SKIP_HOOKS', async () => {
            process.env.DISABLE_OMG = '1';
            process.env.OMG_SKIP_HOOKS = 'keyword-detector';
            const input = {
                sessionId: 'test-session',
    ...
    describe('camelCase validation after normalization', () => {
        ...
        it('snake_case input should be normalized and pass validation', async () => {
            const spy = vi.spyOn(console, 'error').mockImplementation(() => { });
            // Raw snake_case input as Gemini Code would send
            const rawInput = {
    ...
    //
    // processHook() normalizes Gemini Code's snake_case payload (session_id,
    ...
    describe('Regression #858 — snake_case fields reach handlers after normalization', () => {
        ...
        it('setup-maintenance: hook type routing overrides conflicting trigger input', async () => {
            ...
            try {
                ...
                expect(context).toContain('OMG maintenance completed:');
                expect(context).not.toContain('OMG initialized:');
            }
            finally {
                rmSync(tempDir, { recursive: true, force: true });
            }
        ...
        it('subagent start/stop: normalized optional fields survive routing lifecycle', async () => {
            ...
            try {
                ...
                expect(agent?.model).toBe('gpt-5.3-gemini-spark');
