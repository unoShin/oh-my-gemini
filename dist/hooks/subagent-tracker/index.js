...
function acquireLock(directory) {
    const lockPath = join(getOmgRoot(directory), "state", "subagent-tracker.lock");
    const lockDir = join(getOmgRoot(directory), "state");
...
export function getAgentDashboard(directory) {
    ...
    for (const agent of running) {
        ...
        const shortType = agent.agent_type.replace("oh-my-gemini:", "");
...
export function getAgentObservatory(directory) {
    ...
    for (const intervention of interventions.slice(0, 3)) {
        const shortType = intervention.agent_type.replace("oh-my-gemini:", "");
        ...
        if (agents.length > 1) {
            ...
            for (let i = 1; i < agents.length; i++) {
                interventions.push({
                    ...
                    reason: `File conflict on ${file} with ${agents[0].type.replace("oh-my-gemini:", "")}`,
                    suggested_action: "warn",
                    auto_execute: false,
                });
            }
...
export function detectFileConflicts(directory) {
    ...
    for (const agent of running) {
        for (const file of agent.file_ownership || []) {
            ...
            fileToAgents
                .get(file)
                .push(agent.agent_type.replace("oh-my-gemini:", ""));
        }
    }
    const conflicts = [];
    for (const [file, agents] of fileToAgents) {
