...
describe('skill-state', () => {
    ...
    describe('getSkillProtection', () => {
        ...
        it('defaults to none for unknown/non-OMG skills', () => {
            expect(getSkillProtection('unknown-skill')).toBe('none');
            expect(getSkillProtection('my-custom-skill')).toBe('none');
        });
        it('strips oh-my-gemini: prefix', () => {
            expect(getSkillProtection('oh-my-gemini:plan')).toBe('medium');
            expect(getSkillProtection('oh-my-gemini:ralph')).toBe('none');
        });
        it('is case-insensitive', () => {
            expect(getSkillProtection('SKILL')).toBe('light');
            expect(getSkillProtection('Plan')).toBe('medium');
        });
        it('returns none for project custom skills with same name as OMG skills (issue #1581)', () => {
            // rawSkillName without oh-my-gemini: prefix → project custom skill
        ...
        it('returns protection for OMG skills when rawSkillName has prefix', () => {
            expect(getSkillProtection('plan', 'oh-my-gemini:plan')).toBe('medium');
            expect(getSkillProtection('deepinit', 'oh-my-gemini:deepinit')).toBe('heavy');
        });
        it('returns none for other plugin skills with rawSkillName', () => {
            // ouroboros:plan, gemini-mem:make-plan etc. should not get OMG protection
            expect(getSkillProtection('plan', 'ouroboros:plan')).toBe('none');
            expect(getSkillProtection('make-plan', 'gemini-mem:make-plan')).toBe('none');
        });
    ...
    describe('writeSkillActiveState', () => {
        ...
        it('strips namespace prefix from skill name', () => {
            const state = writeSkillActiveState(tempDir, 'oh-my-gemini:plan', 'session-1');
            expect(state.skill_name).toBe('plan');
        });
        it('does not write state for project custom skills with same name as OMG skills (issue #1581)', () => {
            // rawSkillName='plan' (no prefix) → project custom skill → no state
            const state = writeSkillActiveState(tempDir, 'plan', 'session-1', 'plan');
            expect(state).toBeNull();
            expect(readSkillActiveState(tempDir, 'session-1')).toBeNull();
        });
        it('writes state for OMG skills when rawSkillName has prefix', () => {
            const state = writeSkillActiveState(tempDir, 'plan', 'session-1', 'oh-my-gemini:plan');
            expect(state).not.toBeNull();
            expect(state.skill_name).toBe('plan');
            expect(state.max_reinforcements).toBe(5);
        });
