 ...
 *
 * Fix for: https://github.com/unoShin/oh-my-gemini/issues/1033
 */
 ...
 *
 * IMPORTANT: When adding a new OMG skill, register it here with the
...
const SKILL_PROTECTION = {
    ...
    learner: 'medium',
    'omg-setup': 'medium',
    setup: 'medium', // alias for omg-setup
    'mcp-setup': 'medium',
 ...
 * @param rawSkillName - The original skill name as invoked (e.g., 'oh-my-gemini:plan'
 *   or 'plan'). When provided, only skills invoked with the 'oh-my-gemini:' prefix
 *   are eligible for protection. This prevents project custom skills (e.g., a user's
 *   `.gemini/skills/plan/`) from being confused with OMG built-in skills of the same name.
...
export function getSkillProtection(skillName, rawSkillName) {
    ...
    if (rawSkillName != null && !rawSkillName.toLowerCase().startsWith('oh-my-gemini:')) {
        return 'none';
    }
 ...
 *   OMG built-in skills from project custom skills. See getSkillProtection().
 */
