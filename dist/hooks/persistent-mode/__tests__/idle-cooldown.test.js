...
const { TEST_HOME } = vi.hoisted(() => ({
    TEST_HOME: process.env.HOME || '/tmp/omg-test-home',
}));
vi.mock('os', async () => {
...
function getConfigPaths() {
    return getGlobalOmgConfigCandidates('config.json');
}
describe('getIdleNotificationCooldownSeconds', () => {
    ...
    const originalOmgHome = process.env.OMG_HOME;
    afterEach(() => {
        ...
        if (originalOmgHome === undefined) {
            delete process.env.OMG_HOME;
        }
        else {
            process.env.OMG_HOME = originalOmgHome;
        }
    });
