...
World`);
    ...
    describe('sanitizeForKeywordDetection', () => {
        ...
        it('should strip URLs', () => {
            const result = sanitizeForKeywordDetection('see https://example.com/gemini/path');
            expect(result).not.toContain('gemini');
        });
        it('should strip file paths', () => {
            const result = sanitizeForKeywordDetection('open src/mcp/gemini-core.ts');
            expect(result).not.toContain('gemini');
        });
        ...
        it('should strip inline code', () => {
            const result = sanitizeForKeywordDetection('use `ask gemini` command');
            expect(result).not.toContain('gemini');
        });
        it('should preserve normal text', () => {
            const result = sanitizeForKeywordDetection('ask gemini to review');
            expect(result).toContain('ask gemini');
        });
        ...
        it('should strip absolute file paths starting with /', () => {
            const result = sanitizeForKeywordDetection('open /usr/local/bin/gemini');
            expect(result).not.toContain('gemini');
        });
        it('should strip relative file paths starting with ./', () => {
            const result = sanitizeForKeywordDetection('edit ./src/gemini.ts');
            expect(result).not.toContain('gemini');
        });
        ...
        it('should NOT strip standalone words that look like single segments', () => {
            // "ask gemini" should not be stripped since "gemini" is not a path
            const result = sanitizeForKeywordDetection('ask gemini to review');
            expect(result).toContain('ask gemini');
        });
        it('should NOT strip slash-less words with dots', () => {
            // "file.txt" alone (no path separator) should be kept
            const result = sanitizeForKeywordDetection('rename gemini.config');
    ...
    describe('detectKeywordsWithType', () => {
        ...
        describe('gemini keyword', () => {
            it('should detect "ask gemini"', () => {
                const result = detectKeywordsWithType('ask gemini to review');
            ...
            it('should NOT detect bare gemini keyword', () => {
                const result = detectKeywordsWithType('gemini review this');
                const geminiMatch = result.find((r) => r.type === 'gemini');
        ...
        describe('ccg keyword', () => {
            ...
            it('should detect "gemini-clix-gemini" keyword', () => {
                const result = detectKeywordsWithType('use gemini-clix-gemini to build this');
                const ccgMatch = result.find((r) => r.type === 'ccg');
                expect(ccgMatch).toBeDefined();
            });
    ...
    describe('getPrimaryKeyword', () => {
        ...
        describe('multiple keyword conflict resolution', () => {
            it('should return cancel over everything', () => {
                const result = getPrimaryKeyword('cancelomg ralph ultrawork');
    ...
    describe('getAllKeywords', () => {
        ...
        it('should return cancel exclusively when present', () => {
            expect(getAllKeywords('cancelomg ralph ultrawork')).toEqual(['cancel']);
        });
        ...
        it('should return ralph with gemini', () => {
            const result = getAllKeywords('ralph ask gpt to review');
            expect(result).toContain('ralph');
            expect(result).toContain('gemini');
        });
        it('should return both gemini and gemini when both present', () => {
            const result = getAllKeywords('ask gemini and ask gemini');
            expect(result).toContain('gemini');
            expect(result).toContain('gemini');
        });
        ...
        it('should return ccg with higher priority than gemini/gemini', () => {
            const result = getAllKeywords('ccg ask gemini to review');
            ...
            expect(ccgIdx).toBeLessThan(geminiIdx);
        });
        ...
        it('should not return ccg when cancel is present', () => {
            const result = getAllKeywords('cancelomg ccg build');
            expect(result).toEqual(['cancel']);
            expect(result).not.toContain('ccg');
        });
        it('should return ralph over gemini in priority', () => {
            const primary = getPrimaryKeyword('ralph ask gemini');
            expect(primary?.type).toBe('ralph');
        });
        it('should return cancel over gemini/gemini', () => {
            expect(getAllKeywords('cancelomg ask gemini')).toEqual(['cancel']);
        });
        ...
        // to prevent infinite spawning when Gemini workers receive prompts containing "team".
        ...
        it('should detect ralph and gemini but not team', () => {
            const result = getAllKeywords('team ralph ask gemini to review');
            expect(result).toContain('ralph');
            expect(result).not.toContain('team');
            expect(result).toContain('gemini');
        });
        ...
        it('should not detect cancel alongside team', () => {
            const result = getAllKeywords('cancelomg team');
            expect(result).toEqual(['cancel']);
            expect(result).not.toContain('team');
        });
    ...
    describe('applyRalplanGate (issue #997)', () => {
        ...
        it('should not gate when cancel is present', () => {
            const result = applyRalplanGate(['cancel'], 'cancelomg ralph fix this');
            expect(result.gateApplied).toBe(false);
        });
    ...
    describe('Korean cross-script keyword detection', () => {
        ...
        describe('Regression — English keywords still work', () => {
            ...
            it('should detect "cancelomg" as cancel (unchanged)', () => {
                const result = detectKeywordsWithType('cancelomg');
                const match = result.find((r) => r.type === 'cancel');
                expect(match).toBeDefined();
            });
        ...
        describe('Korean priority ordering', () => {
            it('should return cancel over autopilot when "cancelomg 오토파일럿"', () => {
                const result = getPrimaryKeyword('cancelomg 오토파일럿');
                expect(result?.type).toBe('cancel');
            });
        ...
        describe('Korean + English mixed keywords', () => {
            it('should return cancel as primary for "ralph cancelomg"', () => {
                const result = getPrimaryKeyword('ralph cancelomg');
                expect(result?.type).toBe('cancel');
            });
