 ...
 * GEMINI_PLUGIN_ROOT segments so that
 * Gemini Code UI bug #17088 (false "hook error" labels on MSYS2/Git Bash)
 * is avoided.
 */
...
describe('patchHooksJsonForWindows', () => {
    ...
    it('replaces sh+find-node.sh with the run.cjs wrapper for a simple script', () => {
        const original = makeHooksJson([
            'sh "${GEMINI_PLUGIN_ROOT}/scripts/find-node.sh" "${GEMINI_PLUGIN_ROOT}/scripts/keyword-detector.mjs"',
        ]);
        ...
        expect(cmd).toBe('node "$GEMINI_PLUGIN_ROOT"/scripts/run.cjs "$GEMINI_PLUGIN_ROOT"/scripts/keyword-detector.mjs');
    });
    it('preserves trailing arguments (e.g. subagent-tracker start)', () => {
        const original = makeHooksJson([
            'sh "${GEMINI_PLUGIN_ROOT}/scripts/find-node.sh" "${GEMINI_PLUGIN_ROOT}/scripts/subagent-tracker.mjs" start',
        ]);
        writeFileSync(hooksJsonPath, JSON.stringify(original, null, 2));
        patchHooksJsonForWindows(pluginRoot);
        const patched = JSON.parse(readFileSync(hooksJsonPath, 'utf-8'));
        const cmd = patched.hooks.UserPromptSubmit[0].hooks[0].command;
        expect(cmd).toBe('node "$GEMINI_PLUGIN_ROOT"/scripts/run.cjs "$GEMINI_PLUGIN_ROOT"/scripts/subagent-tracker.mjs start');
    });
    it('is idempotent — already-patched commands are not double-modified', () => {
        const already = makeHooksJson([
            'node "$GEMINI_PLUGIN_ROOT"/scripts/run.cjs "$GEMINI_PLUGIN_ROOT"/scripts/keyword-detector.mjs',
        ]);
    ...
    it('patches all hooks across multiple event types', () => {
                ...
                UserPromptSubmit: [
                    {
                        ...
                        hooks: [
                            {
                                type: 'command',
                                command: 'sh "${GEMINI_PLUGIN_ROOT}/scripts/find-node.sh" "${GEMINI_PLUGIN_ROOT}/scripts/keyword-detector.mjs"',
                            },
                        ],
                    },
                ],
                SessionStart: [
                    {
                        ...
                        hooks: [
                            {
                                type: 'command',
                                command: 'sh "${GEMINI_PLUGIN_ROOT}/scripts/find-node.sh" "${GEMINI_PLUGIN_ROOT}/scripts/session-start.mjs"',
                            },
                        ],
                    },
                ],
        ...
        patchHooksJsonForWindows(pluginRoot);
        const patched = JSON.parse(readFileSync(hooksJsonPath, 'utf-8'));
        expect(patched.hooks.UserPromptSubmit[0].hooks[0].command).toBe('node "$GEMINI_PLUGIN_ROOT"/scripts/run.cjs "$GEMINI_PLUGIN_ROOT"/scripts/keyword-detector.mjs');
        expect(patched.hooks.SessionStart[0].hooks[0].command).toBe('node "$GEMINI_PLUGIN_ROOT"/scripts/run.cjs "$GEMINI_PLUGIN_ROOT"/scripts/session-start.mjs');
    });
