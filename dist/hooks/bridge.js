 ...
 * #!/bin/bash
 * INPUT=$(cat)
 * echo "$INPUT" | node ~/.gemini/omg/hook-bridge.mjs --hook=keyword-detector
 * ```
 */
import { pathToFileURL } from "url";
import { existsSync, mkdirSync, readFileSync, renameSync, unlinkSync, writeFileSync, } from "fs";
import { dirname, join } from "path";
import { resolveToWorktreeRoot, getOmgRoot } from "../lib/worktree-paths.js";
...
import { processOrchestratorPreTool, processOrchestratorPostTool, } from "./omg-orchestrator/index.js";
...
import { compactOmgStartupGuidance, loadConfig } from "../config/loader.js";
...
function getModeStatePaths(directory, modeName, sessionId) {
    const stateDir = join(getOmgRoot(directory), "state");
...
function isConsensusPlanningSkillInvocation(skillName, toolInput) {
    ...
    if (skillName !== "omg-plan" && skillName !== "plan") {
        return false;
    }
...
function workerBashBlockReason(command) {
    ...
    if (WORKER_BLOCKED_TEAM_CLI_PATTERN.test(command)) {
        return `Team worker cannot run team orchestration commands. Use only \`${formatOmgCliInvocation("team api ... --json")}\`.`;
    }
...
async function processKeywordDetector(input) {
    ...
    if (process.env.OMG_TEAM_WORKER) {
    ...
    for (const keywordType of keywords) {
        switch (keywordType) {
            ...
            case "deep-interview":
                ...
                break;
            case "gemini": {
                const teamStartCommand = formatOmgCliInvocation(`team start --agent ${keywordType} --count N --task "<task from user message>"`);
                messages.push(`[MAGIC KEYWORD: team]\n` +
                    `User intent: delegate to ${keywordType} CLI workers via ${formatOmgCliInvocation('team')}.\n` +
...
async function processPersistentMode(input) {
    ...
    if (!teamState ||
        ...
        // No persistent mode and no active team — Gemini is truly idle.
            ...
            if (!isAbort && !isContextLimit) {
                ...
                if (shouldSendIdleNotification(stateDir, sessionId)) {
                    ...
                    import("../notifications/index.js")
                        ...
                        sessionId,
                        projectPath: directory,
                        profileName: process.env.OMG_NOTIFY_PROFILE,
...
`);
    ...
    if (existsSync(agentsMdPath)) {
        try {
            let agentsContent = compactOmgStartupGuidance(readFileSync(agentsMdPath, "utf-8")).trim();
            if (agentsContent) {
...
`);
    ...
    // This prevents the LLM from following the static GEMINI.md instruction
    // "Pass model on Task calls: flash, pro, ultra" which produces invalid
...
The GEMINI.md instruction "Pass model on Task calls: flash, pro, ultra" does NOT apply here.
...
/**
 * @internal Object wrapper for OpenClaw gateway dispatch.
 * (Deprecated for v0.1.0 Minimal Release)
 */
export const _openclaw = {
    wake: (event, context) => {
        // OpenClaw disabled in v5.0.0 Minimal
    },
};
/**
...
function processPreToolUse(input) {
    ...
    // Gemini Code's hook protocol does not support modifiedInput, so we cannot
    // silently strip the model. Instead, deny the call so Gemini retries without
    ...
    if (isDelegationToolName(input.toolName)) {
        ...
        if (inputModel) {
            ...
            if (config.routing?.forceInherit) {
                ...
                // Gemini Code supports for blocking a specific tool call with
    ...
    if (input.toolName === "Task") {
        ...
        if (originalTaskInput?.run_in_background === true) {
            ...
            if (permissionFallback.shouldFallback) {
                const reason = `[BACKGROUND PERMISSIONS] ${subagentType || "This background agent"} may need ${permissionFallback.missingTools.join(", ")} permissions, but background agents cannot request interactive approval. Re-run without \`run_in_background=true\` or pre-approve ${permissionFallback.missingTools.join(", ")} in Gemini Code settings.`;
                return {
                    continue: false,
                    reason,
                    message: reason,
    ...
    if (input.toolName === "Bash") {
        ...
        if (nextBashInput.run_in_background === true) {
            ...
            if (permissionFallback.shouldFallback) {
                const reason = "[BACKGROUND PERMISSIONS] This Bash command is not auto-approved for background execution. Re-run without `run_in_background=true` or pre-approve the command in Gemini Code settings.";
                return {
                    continue: false,
                    reason,
                    message: reason,
    ...
    // Pass rawSkillName so writeSkillActiveState can distinguish OMG built-in
            ...
            if (runningCount >= maxBgTasks) {
                return {
                    ...
                    reason: `Background process limit reached (${runningCount}/${maxBgTasks}). ` +
                        ...
                        `Limit is configurable via permissions.maxBackgroundTasks in config or OMG_MAX_BACKGROUND_TASKS env var.`,
                };
            }
        }
    }
 ...
 * Used to distinguish OMG built-in skills (prefixed with 'oh-my-gemini:')
...
async function processPostToolUse(input) {
    ...
    // (e.g., ralplan consensus handoff into Skill("oh-my-gemini:ralph")).
...
/**
 * Cached parsed OMG_SKIP_HOOKS for performance (env vars don't change during process lifetime)
 */
let _cachedSkipHooks = null;
function getSkipHooks() {
    if (_cachedSkipHooks === null) {
        _cachedSkipHooks =
            process.env.OMG_SKIP_HOOKS?.split(",")
                .map((s) => s.trim())
                .filter(Boolean) ?? [];
    }
...
export async function processHook(hookType, rawInput) {
    ...
    if (process.env.DISABLE_OMG === "1" || process.env.DISABLE_OMG === "true") {
        return { continue: true };
    }
    ...
    // Normalize snake_case fields from Gemini Code to camelCase
