 ...
 * Ralph Hook
 *
 * Self-referential work loop that continues until cancelled via /oh-my-gemini:cancel.
...
import { resolveSessionStatePath, getOmgRoot, } from "../../lib/worktree-paths.js";
...
export function isUltraQAActive(directory, sessionId) {
    ...
    const stateFile = join(omgDir, "state", "ultraqa-state.json");
    try {
...
export const RALPH_CRITIC_MODES = ['architect', 'critic', 'gemini'];
...
export function createRalphLoopHook(directory) {
    const startLoop = (sessionId, prompt, options) => {
        ...
        if (isUltraQAActive(directory, sessionId)) {
            console.error("Cannot start Ralph Loop while UltraQA is active. Cancel UltraQA first with /oh-my-gemini:cancel.");
            return false;
        }
