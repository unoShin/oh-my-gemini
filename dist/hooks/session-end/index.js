...
import { getOMGConfig } from '../../features/auto-update.js';
...
import { resolveToWorktreeRoot, getOmgRoot, validateSessionId, isValidTranscriptPath, resolveSessionStatePath } from '../../lib/worktree-paths.js';
...
function getAgentCounts(directory) {
    const trackingPath = path.join(getOmgRoot(directory), 'state', 'subagent-tracking.json');
...
export function cleanupTransientState(directory) {
    ...
    const trackingPath = path.join(omgDir, 'state', 'subagent-tracking.json');
    if (fs.existsSync(trackingPath)) {
        try {
    ...
    const checkpointsDir = path.join(omgDir, 'checkpoints');
...
export function cleanupMissionState(directory, sessionId) {
    const missionStatePath = path.join(getOmgRoot(directory), 'state', 'mission-state.json');
...
async function findSessionOwnedTeams(directory, sessionId) {
    ...
    const teamRoot = path.join(getOmgRoot(directory), 'state', 'team');
...
export function exportSessionSummary(directory, metrics) {
    const sessionsDir = path.join(getOmgRoot(directory), 'sessions');
...
export async function processSessionEnd(input) {
    ...
    // from the repo root, even when Gemini Code is running from a subdirectory (issue #891).
    ...
    // Best-effort cleanup for tmux-backed team workers owned by this Gemini Code
    ...
    const profileName = process.env.OMG_NOTIFY_PROFILE;
