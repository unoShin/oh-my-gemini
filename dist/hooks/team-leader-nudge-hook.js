...
const INJECT_MARKER = '[OMG_TMUX_INJECT]';
...
function resolveNudgeCooldownMs() {
    const raw = safeString(process.env.OMG_TEAM_LEADER_NUDGE_COOLDOWN_MS || '');
    const parsed = asNumber(raw);
...
function resolveMaxNudgeCount() {
    const raw = safeString(process.env.OMG_TEAM_LEADER_MAX_NUDGE_COUNT || '');
    const parsed = asNumber(raw);
...
export async function maybeNudgeLeader(params) {
    ...
    // Send nudge
    const message = `[OMG] Leader nudge (${guidance.nextAction}): ${guidance.message} ${INJECT_MARKER}`;
