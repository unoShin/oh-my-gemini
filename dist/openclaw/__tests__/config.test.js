...
vi.mock("../../utils/paths.js", () => ({
    getGeminiConfigDir: vi.fn(() => "/home/user/.gemini"),
}));
...
describe("getOpenClawConfig", () => {
    ...
    it("returns null when OMG_OPENCLAW is not set", () => {
        vi.stubEnv("OMG_OPENCLAW", "");
        expect(getOpenClawConfig()).toBeNull();
    });
    it("returns null when OMG_OPENCLAW is not '1'", () => {
    ...
    it("returns valid config when file exists and OMG_OPENCLAW=1", () => {
        vi.stubEnv("OMG_OPENCLAW", "1");
    ...
    it("respects OMG_OPENCLAW_CONFIG env var for custom config path", () => {
        vi.stubEnv("OMG_OPENCLAW", "1");
        vi.stubEnv("OMG_OPENCLAW_CONFIG", "/custom/path/config.json");
