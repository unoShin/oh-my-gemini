...
function extractOmgVersionMarker(content) {
...
function trimGeminiUserContent(content) {
 ...
 */
export function isOmgStatusLine(statusLine) {
    if (!statusLine)
        return false;
 ...
 */
export function isOmgHook(command) {
...
export function getInstalledOmgPluginRoots() {
...
export function hasPluginProvidedAgentFiles() {
    return getInstalledOmgPluginRoots().some(pluginRoot => directoryHasMarkdownFiles(join(pluginRoot, 'agents')));
}
/**
 ...
 */
export function extractOmgVersionFromGeminiMd(content) {
 ...
 */
export function mergeGeminiMd(existingContent, omgContent, version) {
    const START_MARKER = '<!-- OMG:START -->';
    const END_MARKER = '<!-- OMG:END -->';
    const USER_CUSTOMIZATIONS = '<!-- User customizations -->';
    const OMG_BLOCK_PATTERN = new RegExp(`^${escapeRegex(START_MARKER)}\\r?\\n[\\s\\S]*?^${escapeRegex(END_MARKER)}(?:\\r?\\n)?`, 'gm');
    ...
    const preservedUserContent = trimGeminiUserContent(stripGeneratedUserCustomizationHeaders(strippedExistingContent));
                ...
                // These were written by the old installer; hooks are now delivered via the plugin's hooks.json.
                {
                    ...
                    for (const [eventType, groups] of Object.entries(existingHooks)) {
                        ...
                        const filtered = groupList.filter(group => {
                            const isLegacy = group.hooks.every(h => h.type === 'command' && h.command.includes('/.gemini/hooks/'));
                            if (isLegacy)
                                legacyRemoved++;
                            return !isLegacy;
                        });
