...
describe('job-state-db', () => {
    ...
    describe('upsertJob', () => {
        ...
        it('should update an existing job', () => {
            ...
            const fetched = getJob('gemini', 'abcd1234');
    ...
    describe('getJob', () => {
        ...
        it('should return a job by provider and jobId', () => {
            const job = createTestJob();
            upsertJob(job);
            const result = getJob('gemini', 'abcd1234');
            expect(result).not.toBeNull();
            expect(result.provider).toBe('gemini');
            expect(result.jobId).toBe('abcd1234');
            expect(result.model).toBe('gpt-5.3-gemini');
            expect(result.agentRole).toBe('architect');
        });
        it('should return null for non-existent job', () => {
            expect(getJob('gemini', 'nonexist')).toBeNull();
        });
        ...
        it('should return null when db is not initialized', () => {
            closeJobDb();
            expect(getJob('gemini', 'abcd1234')).toBeNull();
        });
    });
    describe('getJobsByStatus', () => {
        ...
        it('should filter by status for all providers', () => {
            upsertJob(createTestJob({ provider: 'gemini', jobId: 'c1', status: 'completed' }));
            upsertJob(createTestJob({ provider: 'gemini', jobId: 'g1', status: 'completed' }));
            upsertJob(createTestJob({ provider: 'gemini', jobId: 'c2', status: 'failed' }));
        ...
        it('should filter by provider and status', () => {
            upsertJob(createTestJob({ provider: 'gemini', jobId: 'c1', status: 'completed' }));
            upsertJob(createTestJob({ provider: 'gemini', jobId: 'g1', status: 'completed' }));
            const geminiCompleted = getJobsByStatus('gemini', 'completed');
            expect(geminiCompleted).toHaveLength(1);
            expect(geminiCompleted[0].provider).toBe('gemini');
        });
    ...
    describe('getActiveJobs', () => {
        ...
        it('should filter by provider', () => {
            upsertJob(createTestJob({ provider: 'gemini', jobId: 'c1', status: 'running' }));
            upsertJob(createTestJob({ provider: 'gemini', jobId: 'g1', status: 'running' }));
            const geminiJobs = getActiveJobs('gemini');
            expect(geminiJobs).toHaveLength(1);
            expect(geminiJobs[0].provider).toBe('gemini');
        });
    ...
    describe('getRecentJobs', () => {
        ...
        it('should filter by provider', () => {
            const recentTime = new Date().toISOString();
            upsertJob(createTestJob({ provider: 'gemini', jobId: 'c1', spawnedAt: recentTime }));
            upsertJob(createTestJob({ provider: 'gemini', jobId: 'g1', spawnedAt: recentTime }));
            const geminiRecent = getRecentJobs('gemini', 60 * 60 * 1000);
            expect(geminiRecent).toHaveLength(1);
            expect(geminiRecent[0].provider).toBe('gemini');
        });
    ...
    describe('updateJobStatus', () => {
        ...
        it('should update specific fields', () => {
            ...
            updateJobStatus('gemini', 'abcd1234', {
                status: 'completed',
                completedAt: '2024-01-01T00:00:00Z',
            });
            const result = getJob('gemini', 'abcd1234');
        ...
        it('should return true even if no fields to update', () => {
            upsertJob(createTestJob());
            expect(updateJobStatus('gemini', 'abcd1234', {})).toBe(true);
        });
        it('should update pid field', () => {
            upsertJob(createTestJob({ pid: 12345 }));
            updateJobStatus('gemini', 'abcd1234', { pid: 99999 });
            const result = getJob('gemini', 'abcd1234');
            expect(result.pid).toBe(99999);
        });
        it('should update error field', () => {
            upsertJob(createTestJob());
            updateJobStatus('gemini', 'abcd1234', { error: 'test error message' });
            const result = getJob('gemini', 'abcd1234');
        ...
        it('should update killedByUser field', () => {
            upsertJob(createTestJob());
            updateJobStatus('gemini', 'abcd1234', { killedByUser: true });
            const result = getJob('gemini', 'abcd1234');
            expect(result.killedByUser).toBe(true);
        });
        ...
        it('should return false when db is not initialized', () => {
            closeJobDb();
            expect(updateJobStatus('gemini', 'abcd1234', { status: 'completed' })).toBe(false);
        });
    });
    describe('deleteJob', () => {
        beforeEach(async () => {
        ...
        it('should delete a job', () => {
            upsertJob(createTestJob());
            expect(deleteJob('gemini', 'abcd1234')).toBe(true);
            expect(getJob('gemini', 'abcd1234')).toBeNull();
        });
        it('should succeed even if job does not exist', () => {
            expect(deleteJob('gemini', 'nonexist')).toBe(true);
        });
        it('should only delete the specified provider job', () => {
            upsertJob(createTestJob({ provider: 'gemini', jobId: 'aaaa1111' }));
            upsertJob(createTestJob({ provider: 'gemini', jobId: 'aaaa1111' }));
            deleteJob('gemini', 'aaaa1111');
            expect(getJob('gemini', 'aaaa1111')).toBeNull();
            expect(getJob('gemini', 'aaaa1111')).not.toBeNull();
        });
        it('should return false when db is not initialized', () => {
            closeJobDb();
            expect(deleteJob('gemini', 'abcd1234')).toBe(false);
        });
    });
    describe('migrateFromJsonFiles', () => {
        ...
        it('should import valid status JSON files', () => {
            ...
            writeFileSync(join(PROMPTS_DIR, 'gemini-status-test-migrated1.json'), JSON.stringify(job));
            const result = migrateFromJsonFiles(PROMPTS_DIR);
            expect(result.imported).toBe(1);
            expect(result.errors).toBe(0);
            const fetched = getJob('gemini', 'migrated1');
            expect(fetched).not.toBeNull();
            expect(fetched.jobId).toBe('migrated1');
        });
        it('should skip malformed files', () => {
            writeFileSync(join(PROMPTS_DIR, 'gemini-status-bad-file.json'), 'not valid json');
            const result = migrateFromJsonFiles(PROMPTS_DIR);
        ...
        it('should import multiple files in a transaction', () => {
            ...
            writeFileSync(join(PROMPTS_DIR, 'gemini-status-test-job1.json'), JSON.stringify(job1));
            writeFileSync(join(PROMPTS_DIR, 'gemini-status-test-job2.json'), JSON.stringify(job2));
            const result = migrateFromJsonFiles(PROMPTS_DIR);
            expect(result.imported).toBe(2);
            expect(result.errors).toBe(0);
            expect(getJob('gemini', 'job1')).not.toBeNull();
        ...
        it('should skip files missing required fields', () => {
            ...
            writeFileSync(join(PROMPTS_DIR, 'gemini-status-invalid.json'), JSON.stringify(invalidJob));
            const result = migrateFromJsonFiles(PROMPTS_DIR);
            expect(result.imported).toBe(0);
            expect(result.errors).toBe(1);
        });
    ...
    describe('cleanupOldJobs', () => {
        ...
        it('should remove old terminal jobs', () => {
            ...
            expect(getJob('gemini', 'new1')).not.toBeNull();
            expect(getJob('gemini', 'active1')).not.toBeNull();
            expect(getJob('gemini', 'old1')).toBeNull();
            expect(getJob('gemini', 'old2')).toBeNull();
        });
        it('should not remove active jobs regardless of age', () => {
            ...
            expect(getJob('gemini', 'active1')).not.toBeNull();
            expect(getJob('gemini', 'active2')).not.toBeNull();
        });
        it('should remove timeout status jobs', () => {
            ...
            expect(getJob('gemini', 'timeout1')).toBeNull();
        });
        it('should use default max age of 24 hours', () => {
            ...
            expect(cleaned).toBe(1);
            expect(getJob('gemini', 'old1')).toBeNull();
            expect(getJob('gemini', 'recent1')).not.toBeNull();
        });
    ...
    describe('getJobSummaryForPreCompact', () => {
        ...
        it('should show both gemini and gemini jobs', () => {
            upsertJob(createTestJob({ provider: 'gemini', jobId: 'c1', status: 'running' }));
            upsertJob(createTestJob({ provider: 'gemini', jobId: 'g1', status: 'running' }));
            const summary = getJobSummaryForPreCompact();
