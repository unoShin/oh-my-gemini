...
describe('Notepad Module', () => {
    ...
    describe('initNotepad', () => {
        it('should create notepad.md with correct structure', () => {
            ...
            expect(content).toContain('Auto-managed by OMG');
        });
        it('should create .omg directory if not exists', () => {
            const omgDir = join(testDir, '.omg');
            expect(existsSync(omgDir)).toBe(false);
            initNotepad(testDir);
            expect(existsSync(omgDir)).toBe(true);
        });
