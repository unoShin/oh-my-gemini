...
import { readRalphStateForHud, readUltraworkStateForHud, readAutopilotStateForHud, isAnyModeActive, getActiveSkills, } from '../../hud/omg-state.js';
...
describe('hud omg state session scoping', () => {
    const tempDirs = [];
    afterEach(() => {
    ...
    function createWorktree() {
        const dir = mkdtempSync(join(tmpdir(), 'omg-hud-state-'));
        tempDirs.push(dir);
        return dir;
    }
    it('keeps backward-compatible newest-session fallback when sessionId is omitted', () => {
        const worktree = createWorktree();
        const omgRoot = join(worktree, '.omg');
        const older = Date.now() - 60_000;
        const newer = Date.now();
        writeJson(join(omgRoot, 'state', 'sessions', 'session-a', 'ralph-state.json'), {
            active: true,
            iteration: 1,
            max_iterations: 5,
            current_story_id: 'story-a',
        }, older);
        writeJson(join(omgRoot, 'state', 'sessions', 'session-b', 'ralph-state.json'), {
            active: true,
            iteration: 4,
            max_iterations: 7,
    ...
    it('does not leak to other sessions or fallback files when a session-scoped file is missing', () => {
        const worktree = createWorktree();
        const omgRoot = join(worktree, '.omg');
        writeJson(join(omgRoot, 'state', 'sessions', 'session-b', 'autopilot-state.json'), {
            active: true,
            phase: 'execution',
            iteration: 3,
            max_iterations: 10,
        ...
        writeJson(join(omgRoot, 'state', 'autopilot-state.json'), {
            active: true,
            phase: 'qa',
            iteration: 8,
            max_iterations: 10,
    ...
    it('applies session scoping to combined mode helpers', () => {
        ...
        writeJson(join(omgRoot, 'state', 'sessions', 'session-b', 'ultrawork-state.json'), {
            active: true,
...
//# sourceMappingURL=omg-state.test.js.map