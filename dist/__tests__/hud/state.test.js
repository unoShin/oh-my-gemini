...
describe("readHudConfig", () => {
    ...
    describe("priority order", () => {
        ...
        it("reads from settings.json omgHud key first", () => {
            mockExistsSync.mockImplementation((path) => {
                const s = String(path);
                return /[\\/]Users[\\/]testuser[\\/]\.gemini[\\/]settings\.json$/.test(s);
            });
        ...
        it("falls back to legacy hud-config.json when settings.json has no omgHud", () => {
            mockExistsSync.mockImplementation((path) => {
                const s = String(path);
                return (/[\\/]Users[\\/]testuser[\\/]\.gemini[\\/]settings\.json$/.test(s) ||
                    /[\\/]Users[\\/]testuser[\\/]\.gemini[\\/]\.omg[\\/]hud-config\.json$/.test(s));
            });
            mockReadFileSync.mockImplementation((path) => {
                const s = String(path);
                if (/[\\/]Users[\\/]testuser[\\/]\.gemini[\\/]settings\.json$/.test(s)) {
                    return JSON.stringify({ someOtherKey: true });
                }
                if (/[\\/]Users[\\/]testuser[\\/]\.gemini[\\/]\.omg[\\/]hud-config\.json$/.test(s)) {
    ...
    describe("merging with defaults", () => {
        ...
        it("merges partial config with defaults", () => {
            ...
            expect(config.elements.omgLabel).toBe(DEFAULT_HUD_CONFIG.elements.omgLabel);
...
describe("writeHudConfig", () => {
    ...
    it("preserves unrelated settings.json keys while writing omgHud", () => {
        ...
        expect(written.omgHud.elements.gitRepo).toBe(true);
    });
    it("merges legacy hud-config defaults into the written omgHud payload", () => {
        mockExistsSync.mockImplementation((path) => {
            const s = String(path);
        ...
        const [, raw] = mockAtomicWriteFileSync.mock.calls[0];
        const written = JSON.parse(raw);
        expect(written.omgHud.elements.cwd).toBe(true);
        expect(written.omgHud.elements.gitBranch).toBe(true);
        expect(written.omgHud.wrapMode).toBe("truncate");
    });
});
