...
describe('renderSkills', () => {
    ...
    describe('namespaced skill names', () => {
        it('displays only last segment for namespaced skills (oh-my-gemini:plan)', () => {
            const lastSkill = { name: 'oh-my-gemini:plan', timestamp: new Date() };
            const result = renderSkills(inactiveUltrawork, inactiveRalph, lastSkill);
            expect(result).toContain('skill:plan');
            expect(result).not.toContain('oh-my-gemini');
        });
        it('displays only last segment for namespaced skills with args', () => {
            const lastSkill = { name: 'oh-my-gemini:autopilot', args: 'build app', timestamp: new Date() };
            const result = renderSkills(inactiveUltrawork, inactiveRalph, lastSkill);
