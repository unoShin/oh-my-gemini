...
import { getPluginCacheBase, getGeminiConfigDir } from '../utils/paths.js';
/**
...
describe('HUD Windows Compatibility', () => {
    ...
    describe('Cross-Platform Plugin Cache Path (#670)', () => {
        it('getPluginCacheBase should return path with correct segments', () => {
            ...
            expect(normalized).toContain('plugins/cache/omg/oh-my-gemini');
        });
        it('getPluginCacheBase should use platform-native separators', () => {
            const cachePath = getPluginCacheBase();
            // On Windows: backslashes, on Unix: forward slashes
            expect(cachePath).toContain(`plugins${sep}cache${sep}omg${sep}oh-my-gemini`);
        });
        it('getPluginCacheBase should be under gemini config dir', () => {
            const cachePath = getPluginCacheBase();
        ...
        it('plugin-setup.mjs should respect GEMINI_CONFIG_DIR for plugin cache base', () => {
            const setupPath = join(packageRoot, 'scripts', 'plugin-setup.mjs');
            const content = readFileSync(setupPath, 'utf-8');
            // Should use GEMINI_CONFIG_DIR env var for cross-platform compat (#897)
            expect(content).toContain('process.env.GEMINI_CONFIG_DIR');
        ...
        it('omg-doctor skill should use cross-platform Node.js commands', () => {
            ...
            expect(content).not.toMatch(/ls ~\/\.gemini\/plugins\/cache/);
            ...
            expect(content).toContain("p.join(d,'plugins','cache','omg','oh-my-gemini')");
            expect(content).not.toContain('ls ~/.gemini/GEMINI-*.md');
            expect(content).toContain("find \"$HOME/.gemini\" -maxdepth 1 -type f -name 'GEMINI-*.md' -print 2>/dev/null");
        });
        ...
        it('hud skill should normalize statusLine command paths to forward slashes', () => {
            ...
            expect(content).not.toContain('"command": "node C:\\Users\\username\\.gemini\\hud\\omg-hud.mjs"');
        });
        it('usage-api should use path.join with separate segments', () => {
            ...
            expect(content).toContain("'plugins', 'oh-my-gemini', '.usage-cache.json'");
        });
    });
});
