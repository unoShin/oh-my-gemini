...
describe('GiteaProvider', () => {
    ...
    describe('viewIssue', () => {
        ...
        it('includes Authorization header in REST fallback when GITEA_TOKEN is set', () => {
            ...
            expect(mockExecFileSync).toHaveBeenNthCalledWith(2, 'curl', ['-sS', '-H', 'Authorization: token test-token', 'https://gitea.example.com/api/v1/repos/user/repo/issues/42'], expect.any(Object));
            expect(result).toEqual({
                title: 'Auth Issue',
                body: 'With auth',
        ...
        it('omits Authorization header in REST fallback when GITEA_TOKEN is not set', () => {
            ...
            expect(mockExecFileSync).toHaveBeenNthCalledWith(2, 'curl', ['-sS', 'https://gitea.example.com/api/v1/repos/user/repo/issues/1'], expect.any(Object));
        });
    });
    describe('checkAuth', () => {
