...
describe('Ralph PRD-Mandatory', () => {
    ...
    describe('detectCriticModeFlag', () => {
        ...
        it('detects --critic gemini', () => {
            expect(detectCriticModeFlag('ralph --critic gemini fix this')).toBe('gemini');
        });
    ...
    describe('stripCriticModeFlag', () => {
        ...
        it('removes --critic gemini', () => {
            expect(stripCriticModeFlag('ralph --critic gemini fix this')).toBe('ralph fix this');
        });
    });
    // ==========================================================================
    ...
    describe('story-aware architect verification', () => {
        ...
        it('should support gemini verification prompts', () => {
            const prompt = getArchitectVerificationPrompt({
                ...baseVerificationState,
                critic_mode: 'gemini',
            });
            expect(prompt).toContain('[GEMINI CRITIC VERIFICATION REQUIRED');
            expect(prompt).toContain('omg ask gemini --agent-prompt critic');
            expect(prompt).toContain('<ralph-approved critic="gemini">VERIFIED_COMPLETE</ralph-approved>');
        });
        it('detects generic Ralph approval markers', () => {
            expect(detectArchitectApproval('<ralph-approved critic="gemini">VERIFIED_COMPLETE</ralph-approved>')).toBe(true);
        });
        it('detects gemini-style rejection language', () => {
            const result = detectArchitectRejection('Gemini reviewer found issues: Missing tests.');
    ...
    describe('integration: PRD-driven verification', () => {
        ...
        it('stores selected critic mode in Ralph state', () => {
            const hook = createRalphLoopHook(testDir);
            hook.startLoop(undefined, 'Implement caching', { criticMode: 'gemini' });
            const state = readRalphState(testDir);
            expect(state?.critic_mode).toBe('gemini');
        });
