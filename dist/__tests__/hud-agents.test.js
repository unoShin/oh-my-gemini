...
describe('Agents Element', () => {
    ...
    describe('renderAgentsCoded (codes format)', () => {
        ...
        it('should show single-character codes for known agents', () => {
            const agents = [
                createAgent('oh-my-gemini:architect', 'ultra'),
            ];
            const result = renderAgentsCoded(agents);
            // Architect with ultra should be uppercase A in magenta
            expect(result).toContain('agents:');
            expect(result).toContain('A');
        });
        it('should use lowercase for pro/flash tiers', () => {
            const agents = [
                createAgent('oh-my-gemini:explore', 'flash'),
            ];
            const result = renderAgentsCoded(agents);
        ...
        it('should handle multiple agents', () => {
            const now = Date.now();
            const agents = [
                createAgent('oh-my-gemini:architect', 'ultra', new Date(now - 2000)),
                createAgent('oh-my-gemini:explore', 'flash', new Date(now - 1000)),
                createAgent('oh-my-gemini:executor', 'pro', new Date(now)),
            ];
            const result = renderAgentsCoded(agents);
        ...
        it('should handle agents without model info', () => {
            const agents = [createAgent('oh-my-gemini:architect')];
            const result = renderAgentsCoded(agents);
            expect(result).toContain('A');
        });
        it('should use first letter for unknown agent types', () => {
            const agents = [
                createAgent('oh-my-gemini:unknown-agent', 'pro'),
            ];
            const result = renderAgentsCoded(agents);
    ...
    describe('renderAgentsCodedWithDuration (codes-duration format)', () => {
        ...
        it('should not show duration for very recent agents', () => {
            const agents = [
                createAgent('oh-my-gemini:architect', 'ultra', new Date()),
            ];
        ...
        it('should show seconds for agents running 10-59s', () => {
            const agents = [
                createAgent('oh-my-gemini:architect', 'ultra', new Date(Date.now() - 30000)), // 30 seconds ago
            ];
            const result = renderAgentsCodedWithDuration(agents);
        ...
        it('should show minutes for agents running 1-9 min', () => {
            const agents = [
                createAgent('oh-my-gemini:architect', 'ultra', new Date(Date.now() - 180000)), // 3 minutes ago
            ];
            const result = renderAgentsCodedWithDuration(agents);
            const stripped = result.replace(/\x1b\[[0-9;]*m/g, '');
        ...
        it('should show alert for agents running 10+ min', () => {
            const agents = [
                createAgent('oh-my-gemini:architect', 'ultra', new Date(Date.now() - 600000)), // 10 minutes ago
            ];
            const result = renderAgentsCodedWithDuration(agents);
            const stripped = result.replace(/\x1b\[[0-9;]*m/g, '');
    ...
    describe('renderAgentsDetailed (detailed format)', () => {
        ...
        it('should abbreviate common long names', () => {
            const agents = [
                createAgent('oh-my-gemini:executor', 'pro'),
            ];
            const result = renderAgentsDetailed(agents);
        ...
        it('should include duration for long-running agents', () => {
            const agents = [
                createAgent('oh-my-gemini:architect', 'ultra', new Date(Date.now() - 120000)), // 2 minutes
            ];
            const result = renderAgentsDetailed(agents);
    ...
    describe('renderAgentsByFormat (format router)', () => {
        const now = Date.now();
        const agents = [
            createAgent('oh-my-gemini:architect', 'ultra', new Date(now - 1000)),
            createAgent('oh-my-gemini:explore', 'flash', new Date(now)),
        ];
    ...
    describe('Agent type codes', () => {
        const testCases = [
            // Build/Analysis Lane
            { type: 'architect', model: 'ultra', expected: 'A' },
            { type: 'explore', model: 'flash', expected: 'e' },
            { type: 'executor', model: 'pro', expected: 'x' },
            { type: 'deep-executor', model: 'ultra', expected: 'D' }, // deprecated: falls back to first char
            { type: 'debugger', model: 'pro', expected: 'g' },
            { type: 'verifier', model: 'pro', expected: 'v' },
            // Review Lane
            { type: 'style-reviewer', model: 'flash', expected: 'y' },
            { type: 'quality-reviewer', model: 'pro', expected: 'q' }, // deprecated: falls back to first char
            { type: 'api-reviewer', model: 'pro', expected: 'i' },
            { type: 'security-reviewer', model: 'pro', expected: 'k' },
            { type: 'performance-reviewer', model: 'pro', expected: 'o' },
            { type: 'code-reviewer', model: 'ultra', expected: 'R' },
            // Domain Specialists
            { type: 'dependency-expert', model: 'pro', expected: 'l' },
            { type: 'test-engineer', model: 'pro', expected: 't' },
            { type: 'build-fixer', model: 'pro', expected: 'b' }, // deprecated: falls back to first char
            { type: 'designer', model: 'pro', expected: 'd' },
            { type: 'writer', model: 'flash', expected: 'w' },
            { type: 'qa-tester', model: 'pro', expected: 'q' },
            { type: 'scientist', model: 'pro', expected: 's' },
            { type: 'git-master', model: 'pro', expected: 'm' },
            // Product Lane
            { type: 'product-manager', model: 'pro', expected: 'pm' },
            { type: 'ux-researcher', model: 'pro', expected: 'u' },
            { type: 'information-architect', model: 'pro', expected: 'ia' },
            { type: 'product-analyst', model: 'pro', expected: 'a' },
            { type: 'quality-strategist', model: 'pro', expected: 'qs' },
            // Coordination
            { type: 'critic', model: 'ultra', expected: 'C' },
            { type: 'analyst', model: 'ultra', expected: 'T' },
            { type: 'planner', model: 'ultra', expected: 'P' },
            { type: 'vision', model: 'pro', expected: 'v' },
            // Multi-char codes with ultra tier (first char uppercase)
            { type: 'quality-reviewer', model: 'ultra', expected: 'Q' }, // deprecated: falls back to first char uppercase
            { type: 'quality-strategist', model: 'ultra', expected: 'Qs' },
            { type: 'product-manager', model: 'ultra', expected: 'Pm' },
            { type: 'information-architect', model: 'ultra', expected: 'Ia' },
            // Domain Specialists
            { type: 'document-specialist', model: 'pro', expected: 'd' },
            // Backward Compatibility
            { type: 'researcher', model: 'pro', expected: 'r' },
        ];
        testCases.forEach(({ type, model, expected }) => {
            it(`should render ${type} (${model}) as '${expected}'`, () => {
                const agents = [
                    createAgent(`oh-my-gemini:${type}`, model),
                ];
                const result = renderAgentsCoded(agents);
                const stripped = result.replace(/\x1b\[[0-9;]*m/g, '');
    ...
    describe('Model tier color coding', () => {
        it('should use magenta for ultra tier', () => {
            const agents = [
                createAgent('oh-my-gemini:architect', 'ultra'),
            ];
            const result = renderAgentsCoded(agents);
        ...
        it('should use yellow for pro tier', () => {
            const agents = [
                createAgent('oh-my-gemini:executor', 'pro'),
            ];
            const result = renderAgentsCoded(agents);
        ...
        it('should use green for flash tier', () => {
            const agents = [
                createAgent('oh-my-gemini:explore', 'flash'),
            ];
            const result = renderAgentsCoded(agents);
        ...
        it('should use cyan for unknown model', () => {
            const agents = [
                createAgent('oh-my-gemini:architect'),
            ];
            const result = renderAgentsCoded(agents);
    ...
    describe('renderAgentsMultiLine (multiline format)', () => {
        ...
        it('should return empty for completed agents only', () => {
            const agents = [
                { ...createAgent('oh-my-gemini:architect'), status: 'completed' },
            ];
        ...
        it('should limit to maxLines and show overflow indicator', () => {
            const agents = [
                ...
                createAgent('oh-my-gemini:explore', 'flash'),
                createAgent('oh-my-gemini:executor', 'pro'),
                createAgent('oh-my-gemini:document-specialist', 'flash'),
            ];
        ...
        it('should include duration for long-running agents', () => {
            const agents = [
                createAgent('oh-my-gemini:architect', 'ultra', new Date(Date.now() - 120000) // 2 minutes ago
                ),
            ];
            const result = renderAgentsMultiLine(agents);
            expect(result.detailLines).toHaveLength(1);
        ...
        it('should handle agents without descriptions', () => {
            const agents = [createAgent('oh-my-gemini:architect', 'ultra')];
            const result = renderAgentsMultiLine(agents);
            expect(result.detailLines).toHaveLength(1);
            expect(result.detailLines[0]).toContain('...');
        });
