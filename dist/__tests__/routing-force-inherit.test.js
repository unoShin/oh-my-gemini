...
describe('routing.forceInherit (issue #1135)', () => {
    let originalEnv;
    beforeEach(() => {
        originalEnv = process.env.OMG_ROUTING_FORCE_INHERIT;
        vi.clearAllMocks();
    });
    afterEach(() => {
        if (originalEnv === undefined) {
            ...
            process.env.OMG_ROUTING_FORCE_INHERIT = originalEnv;
        }
    });
    describe('routeTask with forceInherit', () => {
        it('returns inherit model type when forceInherit is true', () => {
            const result = routeTask({ taskPrompt: 'Find all files', agentType: 'explore' }, { enabled: true, defaultTier: 'MEDIUM', forceInherit: true, escalationEnabled: false, maxEscalations: 0, tierModels: { LOW: 'flash', MEDIUM: 'pro', HIGH: 'ultra' } });
        ...
        it('bypasses agent-specific overrides when forceInherit is true', () => {
            const result = routeTask({ taskPrompt: 'Design system architecture', agentType: 'architect' }, {
                ...
                maxEscalations: 0,
                tierModels: { LOW: 'flash', MEDIUM: 'pro', HIGH: 'ultra' },
                agentOverrides: {
        ...
        it('bypasses complexity-based routing when forceInherit is true', () => {
            const result = routeTask({
                ...
                agentType: 'executor',
            }, { enabled: true, defaultTier: 'MEDIUM', forceInherit: true, escalationEnabled: false, maxEscalations: 0, tierModels: { LOW: 'flash', MEDIUM: 'pro', HIGH: 'ultra' } });
            expect(result.model).toBe('inherit');
            expect(result.modelType).toBe('inherit');
        });
        it('routes normally when forceInherit is false', () => {
            const result = routeTask({ taskPrompt: 'Find all files', agentType: 'explore' }, { enabled: true, defaultTier: 'MEDIUM', forceInherit: false, escalationEnabled: false, maxEscalations: 0, tierModels: { LOW: 'flash', MEDIUM: 'pro', HIGH: 'ultra' } });
            expect(result.model).not.toBe('inherit');
        });
        it('routes normally when forceInherit is undefined', () => {
            const result = routeTask({ taskPrompt: 'Find all files', agentType: 'explore' }, { enabled: true, defaultTier: 'MEDIUM', escalationEnabled: false, maxEscalations: 0, tierModels: { LOW: 'flash', MEDIUM: 'pro', HIGH: 'ultra' } });
            expect(result.model).not.toBe('inherit');
        });
    });
    describe('getModelForTask with forceInherit', () => {
        it('returns inherit for all agent types when forceInherit is true', () => {
            const config = { enabled: true, defaultTier: 'MEDIUM', forceInherit: true, escalationEnabled: false, maxEscalations: 0, tierModels: { LOW: 'flash', MEDIUM: 'pro', HIGH: 'ultra' } };
    ...
    describe('processPreToolUse with forceInherit', () => {
        it('strips model from Task calls when forceInherit is true', () => {
            ...
            expect(modified.subagent_type).toBe('oh-my-gemini:executor');
        });
        ...
        it('does not strip model when forceInherit is false', () => {
            ...
            expect(modified.model).toBe('flash');
        });
