...
/**
 * Regression tests for run.cjs graceful fallback when GEMINI_PLUGIN_ROOT
...
describe('run.cjs — graceful fallback for stale plugin paths', () => {
    ...
    beforeEach(() => {
        ...
        fakeCacheBase = join(tmpDir, 'plugins', 'cache', 'omg', 'oh-my-gemini');
    ...
    it('exits 0 when target script does not exist (stale GEMINI_PLUGIN_ROOT)', () => {
        ...
        const result = runCjs(staleTarget, {
            GEMINI_PLUGIN_ROOT: staleVersion,
        });
    ...
    it('resolves target through symlinked version directory', () => {
        ...
        const result = runCjs(target, {
            GEMINI_PLUGIN_ROOT: symlinkVersion,
        });
        expect(result.status).toBe(0);
    ...
    it('runs target normally when path is valid (fast path)', () => {
        ...
        const result = runCjs(target, {
            GEMINI_PLUGIN_ROOT: versionDir,
        });
        expect(result.status).toBe(0);
        expect(result.stdout).toContain('direct-ok');
    });
    it('exits 0 when no GEMINI_PLUGIN_ROOT is set and target is missing', () => {
