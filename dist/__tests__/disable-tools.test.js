/**
 * Tests for OMG_DISABLE_TOOLS env var support
 *
...
import { parseDisabledGroups, DISABLE_TOOLS_GROUP_MAP } from '../mcp/omg-tools-server.js';
import { TOOL_CATEGORIES } from '../constants/index.js';
describe('OMG_DISABLE_TOOLS', () => {
    let savedEnv;
    beforeEach(() => {
        savedEnv = process.env.OMG_DISABLE_TOOLS;
        delete process.env.OMG_DISABLE_TOOLS;
    });
    afterEach(() => {
        if (savedEnv !== undefined) {
            process.env.OMG_DISABLE_TOOLS = savedEnv;
        }
        else {
            delete process.env.OMG_DISABLE_TOOLS;
        }
    ...
    describe('parseDisabledGroups()', () => {
        ...
        describe('multiple groups', () => {
            ...
            it('disables all issue-722 specified groups', () => {
                const result = parseDisabledGroups('lsp,ast,python-repl,gemini,gemini,trace,state,notepad,project-memory');
                expect(result.has(TOOL_CATEGORIES.LSP)).toBe(true);
                expect(result.has(TOOL_CATEGORIES.AST)).toBe(true);
                expect(result.has(TOOL_CATEGORIES.PYTHON)).toBe(true);
                expect(result.has(TOOL_CATEGORIES.GEMINI)).toBe(true);
        ...
        describe('robustness', () => {
            ...
            it('reads from process.env.OMG_DISABLE_TOOLS when no argument given', () => {
                process.env.OMG_DISABLE_TOOLS = 'lsp,ast';
                const result = parseDisabledGroups();
                expect(result.has(TOOL_CATEGORIES.LSP)).toBe(true);
                expect(result.has(TOOL_CATEGORIES.AST)).toBe(true);
            });
            it('explicit argument takes precedence over env var', () => {
                process.env.OMG_DISABLE_TOOLS = 'lsp';
                const result = parseDisabledGroups('ast');
                expect(result.has(TOOL_CATEGORIES.AST)).toBe(true);
    ...
    describe('DISABLE_TOOLS_GROUP_MAP', () => {
        it('contains all issue-722 specified group names', () => {
            const requiredGroups = ['lsp', 'ast', 'python-repl', 'gemini', 'gemini', 'trace', 'state', 'notepad', 'project-memory', 'interop'];
