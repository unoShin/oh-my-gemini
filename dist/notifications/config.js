...
const CONFIG_FILE = join(getGeminiConfigDir(), ".omg-config.json");
...
export function buildConfigFromEnv() {
    ...
    // Telegram (support both OMG_TELEGRAM_BOT_TOKEN and OMG_TELEGRAM_NOTIFIER_BOT_TOKEN)
    const telegramToken = process.env.OMG_TELEGRAM_BOT_TOKEN ||
        process.env.OMG_TELEGRAM_NOTIFIER_BOT_TOKEN;
    const telegramChatId = process.env.OMG_TELEGRAM_CHAT_ID ||
    ...
    if (slackWebhook) {
        config.slack = {
            ...
            mention: validateSlackMention(process.env.OMG_SLACK_MENTION),
        };
        hasAnyPlatform = true;
    }
    // Slack Bot (app token + bot token + channel)
    const slackBotToken = process.env.OMG_SLACK_BOT_TOKEN;
    const slackBotChannel = process.env.OMG_SLACK_BOT_CHANNEL;
    if (slackBotToken && slackBotChannel) {
        config["slack-bot"] = {
            enabled: true,
            appToken: process.env.OMG_SLACK_APP_TOKEN,
...
function applyEnvMerge(config) {
    ...
    const envSlackMention = validateSlackMention(process.env.OMG_SLACK_MENTION);
    if (envSlackMention) {
...
export function getVerbosity(config) {
    const envValue = process.env.OMG_NOTIFY_VERBOSITY;
...
export function getTmuxTailLines(config) {
    const envValue = Number.parseInt(process.env.OMG_NOTIFY_TMUX_TAIL_LINES ?? "", 10);
...
export function getNotificationConfig(profileName) {
    ...
    const effectiveProfile = profileName || process.env.OMG_NOTIFY_PROFILE;
 ...
 *   --telegram  -> OMG_TELEGRAM=1
 *   --discord   -> OMG_DISCORD=1
 *   --slack     -> OMG_SLACK=1
 *   --webhook   -> OMG_WEBHOOK=1
 */
function isPlatformActivated(platform) {
    if (platform === "telegram")
        return process.env.OMG_TELEGRAM === "1";
    if (platform === "discord" || platform === "discord-bot")
        return process.env.OMG_DISCORD === "1";
    if (platform === "slack" || platform === "slack-bot")
        return process.env.OMG_SLACK === "1";
    if (platform === "webhook")
        return process.env.OMG_WEBHOOK === "1";
    return false;
}
/**
...
export function getReplyConfig() {
    ...
    if (!enabled)
        return null;
    const authorizedDiscordUserIds = parseDiscordUserIds(process.env.OMG_REPLY_DISCORD_USER_IDS, replyRaw?.authorizedDiscordUserIds);
    ...
    const authorizedSlackUserIds = parseSlackUserIds(process.env.OMG_REPLY_SLACK_USER_IDS, replyRaw?.authorizedSlackUserIds);
    return {
        enabled: true,
        pollIntervalMs: parseIntSafe(process.env.OMG_REPLY_POLL_INTERVAL_MS) ?? replyRaw?.pollIntervalMs ?? 3000,
        maxMessageLength: replyRaw?.maxMessageLength ?? 500,
        rateLimitPerMinute: parseIntSafe(process.env.OMG_REPLY_RATE_LIMIT) ?? replyRaw?.rateLimitPerMinute ?? 10,
...
const LEGACY_OPENCLAW_CONFIG = join(getGeminiConfigDir(), "omg_config.openclaw.json");
/**
