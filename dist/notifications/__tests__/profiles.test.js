...
describe("getNotificationConfig - named profiles", () => {
    ...
    it("returns named profile when OMG_NOTIFY_PROFILE env var is set", () => {
        vi.stubEnv("OMG_NOTIFY_PROFILE", "ops");
        vi.mocked(existsSync).mockReturnValue(true);
        vi.mocked(readFileSync).mockReturnValue(JSON.stringify({
            notifications: {
                enabled: true,
    ...
    it("profileName argument takes precedence over OMG_NOTIFY_PROFILE env var", () => {
        vi.stubEnv("OMG_NOTIFY_PROFILE", "env-profile");
        vi.mocked(existsSync).mockReturnValue(true);
        vi.mocked(readFileSync).mockReturnValue(JSON.stringify({
            notificationProfiles: {
                "env-profile": {
    ...
    it("falls back to default when profile env var set but no profiles exist", () => {
        const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => { });
        vi.stubEnv("OMG_NOTIFY_PROFILE", "missing");
        vi.mocked(existsSync).mockReturnValue(true);
        vi.mocked(readFileSync).mockReturnValue(JSON.stringify({
            notifications: {
                enabled: true,
    ...
    it("merges env platforms into profile config", () => {
        vi.stubEnv("OMG_TELEGRAM_BOT_TOKEN", "env-tg-token");
        vi.stubEnv("OMG_TELEGRAM_CHAT_ID", "env-tg-chat");
        vi.mocked(existsSync).mockReturnValue(true);
        vi.mocked(readFileSync).mockReturnValue(JSON.stringify({
            notificationProfiles: {
                work: {
