...
describe("reply config", () => {
    beforeEach(() => {
        ...
        delete process.env.OMG_REPLY_ENABLED;
        delete process.env.OMG_REPLY_POLL_INTERVAL_MS;
        delete process.env.OMG_REPLY_RATE_LIMIT;
        delete process.env.OMG_REPLY_DISCORD_USER_IDS;
        delete process.env.OMG_REPLY_INCLUDE_PREFIX;
        ...
        delete process.env.OMG_TELEGRAM_NOTIFIER_BOT_TOKEN;
        delete process.env.OMG_TELEGRAM_CHAT_ID;
        delete process.env.OMG_TELEGRAM_NOTIFIER_CHAT_ID;
        delete process.env.OMG_TELEGRAM_NOTIFIER_UID;
    ...
    it("applies environment overrides for reply settings and discord user IDs", async () => {
        process.env.OMG_REPLY_POLL_INTERVAL_MS = "5000";
        process.env.OMG_REPLY_RATE_LIMIT = "20";
        process.env.OMG_REPLY_INCLUDE_PREFIX = "false";
        process.env.OMG_REPLY_DISCORD_USER_IDS = `${VALID_DISCORD_USER_ID},invalid-id`;
        mockConfigFile({
            notifications: {
                enabled: true,
                "discord-bot": {
    ...
    it("returns discordMention from env var OMG_DISCORD_MENTION", async () => {
        process.env.OMG_DISCORD_NOTIFIER_BOT_TOKEN = "env-token";
        process.env.OMG_DISCORD_NOTIFIER_CHANNEL = "env-channel";
        process.env.OMG_DISCORD_MENTION = "<@987654321098765432>";
        mockConfigFile(null);
        const { getNotificationConfig, getReplyListenerPlatformConfig } = await import("../config.js");
        const notifConfig = getNotificationConfig();
        const runtime = getReplyListenerPlatformConfig(notifConfig);
