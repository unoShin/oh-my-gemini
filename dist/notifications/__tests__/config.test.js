...
describe("buildConfigFromEnv", () => {
    ...
    beforeEach(() => {
        vi.stubEnv("OMG_DISCORD_NOTIFIER_BOT_TOKEN", "");
        vi.stubEnv("OMG_DISCORD_NOTIFIER_CHANNEL", "");
        vi.stubEnv("OMG_DISCORD_WEBHOOK_URL", "");
        vi.stubEnv("OMG_DISCORD_MENTION", "");
        vi.stubEnv("OMG_TELEGRAM_BOT_TOKEN", "");
        vi.stubEnv("OMG_TELEGRAM_NOTIFIER_BOT_TOKEN", "");
        vi.stubEnv("OMG_TELEGRAM_CHAT_ID", "");
        vi.stubEnv("OMG_TELEGRAM_NOTIFIER_CHAT_ID", "");
        vi.stubEnv("OMG_TELEGRAM_NOTIFIER_UID", "");
        vi.stubEnv("OMG_SLACK_WEBHOOK_URL", "");
        vi.stubEnv("OMG_SLACK_MENTION", "");
    });
    afterEach(() => {
        vi.unstubAllEnvs();
    });
    ...
    it("builds discord-bot config from env vars", () => {
        vi.stubEnv("OMG_DISCORD_NOTIFIER_BOT_TOKEN", "test-token");
        vi.stubEnv("OMG_DISCORD_NOTIFIER_CHANNEL", "123456");
    ...
    it("includes validated mention in discord-bot config", () => {
        vi.stubEnv("OMG_DISCORD_NOTIFIER_BOT_TOKEN", "test-token");
        vi.stubEnv("OMG_DISCORD_NOTIFIER_CHANNEL", "123456");
        vi.stubEnv("OMG_DISCORD_MENTION", "<@12345678901234567>");
        const config = buildConfigFromEnv();
    ...
    it("rejects invalid mention in env var", () => {
        vi.stubEnv("OMG_DISCORD_NOTIFIER_BOT_TOKEN", "test-token");
        vi.stubEnv("OMG_DISCORD_NOTIFIER_CHANNEL", "123456");
        vi.stubEnv("OMG_DISCORD_MENTION", "@everyone");
        const config = buildConfigFromEnv();
        expect(config["discord-bot"].mention).toBeUndefined();
    });
    it("builds discord webhook config from env var", () => {
        vi.stubEnv("OMG_DISCORD_WEBHOOK_URL", "https://discord.com/api/webhooks/test");
        const config = buildConfigFromEnv();
    ...
    it("builds telegram config from env vars", () => {
        vi.stubEnv("OMG_TELEGRAM_BOT_TOKEN", "123:abc");
        vi.stubEnv("OMG_TELEGRAM_CHAT_ID", "999");
        const config = buildConfigFromEnv();
    ...
    it("builds slack config from env var", () => {
        vi.stubEnv("OMG_SLACK_WEBHOOK_URL", "https://hooks.slack.com/services/test");
        const config = buildConfigFromEnv();
    ...
    it("uses OMG_TELEGRAM_NOTIFIER_BOT_TOKEN as fallback", () => {
        vi.stubEnv("OMG_TELEGRAM_NOTIFIER_BOT_TOKEN", "123:fallback");
        vi.stubEnv("OMG_TELEGRAM_CHAT_ID", "999");
        const config = buildConfigFromEnv();
    ...
    it("uses OMG_TELEGRAM_NOTIFIER_UID as fallback for chat ID", () => {
        vi.stubEnv("OMG_TELEGRAM_BOT_TOKEN", "123:abc");
        vi.stubEnv("OMG_TELEGRAM_NOTIFIER_UID", "uid-999");
        const config = buildConfigFromEnv();
...
describe("getNotificationConfig - deep merge", () => {
    ...
    it("env provides discord-bot when file config has only discord webhook", () => {
        ...
        vi.stubEnv("OMG_DISCORD_NOTIFIER_BOT_TOKEN", "env-bot-token");
        vi.stubEnv("OMG_DISCORD_NOTIFIER_CHANNEL", "env-channel");
    ...
    it("env provides telegram when file config has only discord", () => {
        vi.stubEnv("OMG_TELEGRAM_BOT_TOKEN", "123:tg-token");
        vi.stubEnv("OMG_TELEGRAM_CHAT_ID", "tg-chat");
        const envConfig = buildConfigFromEnv();
    ...
    it("builds config with multiple platforms from env", () => {
        vi.stubEnv("OMG_DISCORD_NOTIFIER_BOT_TOKEN", "bot-token");
        vi.stubEnv("OMG_DISCORD_NOTIFIER_CHANNEL", "channel-123");
        vi.stubEnv("OMG_TELEGRAM_BOT_TOKEN", "456:tg");
        vi.stubEnv("OMG_TELEGRAM_CHAT_ID", "chat-789");
        vi.stubEnv("OMG_SLACK_WEBHOOK_URL", "https://hooks.slack.com/services/test");
        const config = buildConfigFromEnv();
        expect(config).not.toBeNull();
        expect(config.enabled).toBe(true);
