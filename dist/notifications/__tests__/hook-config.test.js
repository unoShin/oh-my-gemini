/**
 * Tests for hook notification config reader (omg_config.hook.json).
 *
 * Covers:
 * - File missing → null
 * - File disabled → null
 ...
 * - OMG_HOOK_CONFIG env var override
 */
...
const TEST_CONFIG_PATH = join(TEST_DIR, "omg_config.hook.json");
...
describe("hook-config reader", () => {
    beforeEach(() => {
        ...
        vi.stubEnv("OMG_HOOK_CONFIG", TEST_CONFIG_PATH);
    });
    afterEach(() => {
        vi.unstubAllEnvs();
        resetHookConfigCache();
    ...
    it("returns null when file does not exist", () => {
        vi.stubEnv("OMG_HOOK_CONFIG", join(TEST_DIR, "nonexistent.json"));
    ...
    it("OMG_HOOK_CONFIG env var overrides default path", () => {
        ...
        vi.stubEnv("OMG_HOOK_CONFIG", altPath);
        resetHookConfigCache();
        const config = getHookConfig();
