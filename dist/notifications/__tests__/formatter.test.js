...
describe("formatSessionIdle", () => {
    ...
    it("should include idle header and waiting message", () => {
        ...
        expect(result).toContain("Gemini has finished and is waiting for input.");
    });
...
describe("formatNotification routing", () => {
    ...
    it("should route agent-call correctly", () => {
        const result = formatNotification({
            ...
            event: "agent-call",
            agentName: "executor",
            agentType: "oh-my-gemini:executor",
        });
...
describe("formatAgentCall", () => {
    ...
    it("should include agent type when provided", () => {
        ...
        expect(result).toContain("**Type:** `oh-my-gemini:executor`");
    });
...
describe("parseTmuxTail noise filters", () => {
    ...
    it("drops OMG HUD versioned status lines", () => {
        expect(parseTmuxTail("[OMG#4.4.5] | thinking | session:510m | ctx:61% | 🔧57")).toBe("");
    });
    it("drops unversioned OMG HUD lines", () => {
        expect(parseTmuxTail("[OMG] | session:5m")).toBe("");
    });
    ...
    it("returns empty string when all lines are noise types", () => {
        const input = [
            "────────────────────────",
            "[OMG#4.4.5] | thinking | session:510m",
