...
// Mock getGeminiConfigDir to return a predictable path
...
describe("getNotificationConfig - file + env deep merge", () => {
    ...
    it("returns env-only config when no file exists", () => {
        vi.stubEnv("OMG_DISCORD_NOTIFIER_BOT_TOKEN", "env-token");
    ...
    it("merges env discord-bot into file config that lacks it", () => {
        ...
        vi.stubEnv("OMG_DISCORD_NOTIFIER_CHANNEL", "env-channel-id");
        const config = getNotificationConfig();
        expect(config).not.toBeNull();
    ...
    it("merges env telegram into file config that only has discord", () => {
        ...
        vi.stubEnv("OMG_TELEGRAM_BOT_TOKEN", "123:tg-env");
        vi.stubEnv("OMG_TELEGRAM_CHAT_ID", "tg-chat-env");
        const config = getNotificationConfig();
        expect(config).not.toBeNull();
    ...
    it("allows OMG_NOTIFY_TMUX_TAIL_LINES to override file config", () => {
        vi.mocked(existsSync).mockReturnValue(true);
        vi.mocked(readFileSync).mockReturnValue(JSON.stringify({
            notifications: {
                enabled: true,
        ...
        vi.stubEnv("OMG_NOTIFY_TMUX_TAIL_LINES", "34");
        const config = getNotificationConfig();
        expect(config).not.toBeNull();
        expect(config.tmuxTailLines).toBe(21);
    ...
    it("file mention takes precedence over env mention", () => {
        ...
        vi.stubEnv("OMG_DISCORD_MENTION", "<@11111111111111111>");
        const config = getNotificationConfig();
    ...
    it("merges env slack into file config that lacks it", () => {
        ...
        vi.stubEnv("OMG_SLACK_WEBHOOK_URL", "https://hooks.slack.com/services/env-slack");
        const config = getNotificationConfig();
        expect(config).not.toBeNull();
        // File discord preserved
        expect(config.discord.webhookUrl).toBe("https://discord.com/api/webhooks/file-webhook");
    ...
    it("file slack webhookUrl takes precedence over env", () => {
        ...
        vi.stubEnv("OMG_SLACK_WEBHOOK_URL", "https://hooks.slack.com/services/env-url");
        const config = getNotificationConfig();
    ...
    it("file slack mention takes precedence over env slack mention", () => {
        ...
        vi.stubEnv("OMG_SLACK_MENTION", "<@U9999999999>");
        const config = getNotificationConfig();
