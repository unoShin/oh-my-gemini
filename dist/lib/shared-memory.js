...
import { getOmgRoot } from './worktree-paths.js';
...
export function isSharedMemoryEnabled() {
    try {
        const configPath = join(process.env.HOME || process.env.USERPROFILE || '', '.gemini', CONFIG_FILE_NAME);
...
function getNamespaceDir(namespace, worktreeRoot) {
    ...
    return join(omgRoot, SHARED_MEMORY_DIR, namespace);
}
...
export function cleanupExpired(namespace, worktreeRoot) {
    const omgRoot = getOmgRoot(worktreeRoot);
    const sharedMemDir = join(omgRoot, SHARED_MEMORY_DIR);
