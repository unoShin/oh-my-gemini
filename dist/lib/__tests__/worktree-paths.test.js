...
import { validatePath, resolveOmgPath, resolveStatePath, ensureOmgDir, getWorktreeNotepadPath, getWorktreeProjectMemoryPath, getOmgRoot, resolvePlanPath, resolveResearchPath, resolveLogsPath, resolveWisdomPath, isPathUnderOmg, ensureAllOmgDirs, clearWorktreeCache, getProcessSessionId, resetProcessSessionId, validateSessionId, resolveToWorktreeRoot, validateWorkingDirectory, getWorktreeRoot, getProjectIdentifier, clearDualDirWarnings, } from '../worktree-paths.js';
...
describe('worktree-paths', () => {
    ...
    describe('resolveOmgPath', () => {
        it('should resolve paths under .omg directory', () => {
            const result = resolveOmgPath('state/ralph.json', TEST_DIR);
            expect(result).toBe(join(TEST_DIR, '.omg', 'state', 'ralph.json'));
        });
        it('should reject paths that escape .omg boundary', () => {
            expect(() => resolveOmgPath('../secret.txt', TEST_DIR)).toThrow('path traversal');
        });
    });
    ...
    describe('ensureOmgDir', () => {
        it('should create directories under .omg', () => {
            const result = ensureOmgDir('state', TEST_DIR);
    ...
    describe('helper functions', () => {
        ...
        it('getOmgRoot returns correct path', () => {
            const result = getOmgRoot(TEST_DIR);
    ...
    describe('isPathUnderOmg', () => {
        it('should return true for paths under .omg', () => {
            expect(isPathUnderOmg(join(TEST_DIR, '.omg', 'state', 'ralph.json'), TEST_DIR)).toBe(true);
            expect(isPathUnderOmg(join(TEST_DIR, '.omg'), TEST_DIR)).toBe(true);
        });
        it('should return false for paths outside .omg', () => {
            expect(isPathUnderOmg(join(TEST_DIR, 'src', 'file.ts'), TEST_DIR)).toBe(false);
            expect(isPathUnderOmg('/etc/passwd', TEST_DIR)).toBe(false);
        });
    });
    describe('ensureAllOmgDirs', () => {
        it('should create all standard .omg subdirectories', () => {
            ensureAllOmgDirs(TEST_DIR);
    ...
    // ==========================================================================
    // OMG_STATE_DIR TESTS (Issue #1014)
    // ==========================================================================
    ...
    describe('getOmgRoot with OMG_STATE_DIR (Issue #1014)', () => {
        it('should return default .omg path when OMG_STATE_DIR is not set', () => {
            delete process.env.OMG_STATE_DIR;
            const result = getOmgRoot(TEST_DIR);
            expect(result).toBe(join(TEST_DIR, '.omg'));
        });
        it('should return centralized path when OMG_STATE_DIR is set', () => {
            const stateDir = mkdtempSync('/tmp/omg-state-dir-');
            try {
    ...
    describe('path functions with OMG_STATE_DIR', () => {
        let stateDir;
        beforeEach(() => {
            stateDir = mkdtempSync('/tmp/omg-state-dir-paths-');
            process.env.OMG_STATE_DIR = stateDir;
        });
        afterEach(() => {
            delete process.env.OMG_STATE_DIR;
        ...
        it('resolveOmgPath should resolve under centralized dir', () => {
            const result = resolveOmgPath('state/ralph.json', TEST_DIR);
            const projectId = getProjectIdentifier(TEST_DIR);
        ...
        it('isPathUnderOmg should check against centralized dir', () => {
            const projectId = getProjectIdentifier(TEST_DIR);
            const centralPath = join(stateDir, projectId, 'state', 'ralph.json');
            expect(isPathUnderOmg(centralPath, TEST_DIR)).toBe(true);
            // Legacy path should NOT be under omg when centralized
            expect(isPathUnderOmg(join(TEST_DIR, '.omg', 'state', 'ralph.json'), TEST_DIR)).toBe(false);
        });
        it('ensureAllOmgDirs should create dirs under centralized path', () => {
            ensureAllOmgDirs(TEST_DIR);
            const projectId = getProjectIdentifier(TEST_DIR);
        ...
        it('ensureOmgDir should create dir under centralized path', () => {
            const result = ensureOmgDir('state', TEST_DIR);
            const projectId = getProjectIdentifier(TEST_DIR);
