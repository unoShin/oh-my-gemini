...
You are running with oh-my-gemini (OMG), a multi-agent orchestration layer for Gemini Code.
...
Direct writes OK for: `~/.gemini/**`, `.omg/**`, `.gemini/**`, `GEMINI.md`, `AGENTS.md`.
</model_routing>
...
<tools>
External AI: `/team N:executor "task"`, `omg team N:gemini|gemini "..."`, `omg ask <gemini|gemini|gemini>`, `/ccg`
...
<skills>
Invoke via `/oh-my-gemini:<name>`. Trigger patterns auto-detect keywords.

Workflow: `autopilot`, `ralph`, `ultrawork`, `team`, `ccg`, `ultraqa`, `omg-plan`, `ralplan`, `sciomg`, `external-context`, `deepinit`, `deep-interview`, `ai-slop-cleaner`
Keyword triggers: "autopilot"→autopilot, "ralph"→ralph, "ulw"→ultrawork, "ccg"→ccg, "ralplan"→ralplan, "deep interview"→deep-interview, "deslop"/"anti-slop"/cleanup+slop-smell→ai-slop-cleaner, "deep-analyze"→analysis mode, "tdd"→TDD mode, "deepsearch"→codebase search, "ultrathink"→deep reasoning, "cancelomg"→cancel. Team orchestration is explicit via `/team`.
Utilities: `ask-gemini`, `ask-gemini`, `cancel`, `note`, `learner`, `omg-setup`, `mcp-setup`, `hud`, `omg-doctor`, `omg-help`, `trace`, `release`, `project-session-manager`, `skill`, `writer-memory`, `ralph-init`, `configure-notifications`, `learn-about-omg` (`trace` is the evidence-driven tracing lane)
</skills>
...
Kill switches: `DISABLE_OMG`, `OMG_SKIP_HOOKS` (comma-separated).
</hooks_and_context>

<cancellation>
`/oh-my-gemini:cancel` ends execution modes. Cancel when done+verified or blocked. Don't cancel if work incomplete.
</cancellation>
...
Say "setup omg" or run `/oh-my-gemini:omg-setup`.
