...
var OMG_DISPATCH_LOCK_TIMEOUT_ENV, DEFAULT_DISPATCH_LOCK_TIMEOUT_MS, MIN_DISPATCH_LOCK_TIMEOUT_MS, MAX_DISPATCH_LOCK_TIMEOUT_MS, DISPATCH_LOCK_INITIAL_POLL_MS, DISPATCH_LOCK_MAX_POLL_MS, LOCK_STALE_MS;
var init_dispatch_queue = __esm({
  "src/team/dispatch-queue.ts"() {
    ...
    init_fs_utils();
    init_contracts();
    OMG_DISPATCH_LOCK_TIMEOUT_ENV = "OMG_TEAM_DISPATCH_LOCK_TIMEOUT_MS";
...
function paneLooksReady(captured) {
  ...
  const isGeminiPromptPresent = lines.some((line) => /^\s*[›❯]\s*/u.test(line));
  return isGeminiPromptPresent;
}
...
// src/utils/omg-cli-rendering.ts
...
function resolveOmgCliPrefix(options = {}) {
...
function formatOmgCliInvocation(commandSuffix, options = {}) {
  const suffix = commandSuffix.trim().replace(/^omg\s+/, "");
  return `${resolveOmgCliPrefix(options)} ${suffix}`.trim();
}
var OMG_CLI_BINARY, OMG_PLUGIN_BRIDGE_PREFIX;
var init_omg_cli_rendering = __esm({
  "src/utils/omg-cli-rendering.ts"() {
    "use strict";
    OMG_CLI_BINARY = "omg";
    OMG_PLUGIN_BRIDGE_PREFIX = 'node "$GEMINI_PLUGIN_ROOT"/bridge/cli.cjs';
  }
});
...
function validateGeminiBaseUrl(urlString) {
...
var GEMINI_FAMILY_DEFAULTS, BUILTIN_TIER_MODEL_DEFAULTS, GEMINI_FAMILY_HIGH_VARIANTS;
var init_models = __esm({
  "src/config/models.ts"() {
    "use strict";
    init_ssrf_guard();
    GEMINI_FAMILY_DEFAULTS = {
      FLASH: "gemini-flash",
      PRO: "gemini-pro",
      ULTRA: "gemini-ultra"
    };
...
function getTrustedPrefixes() {
  ...
  const custom = (process.env.OMG_TRUSTED_CLI_DIRS ?? "").split(":").map((part) => part.trim()).filter(Boolean).filter((part) => isAbsolute4(part));
  trusted.push(...custom);
  return trusted;
}
...
function resolveGeminiWorkerModel(env = process.env) {
...
function buildInstructionPath(...parts) {
  return join8(...parts).replaceAll("\\", "/");
}
function generateTriggerMessage(teamName, workerName, teamStateRoot3 = ".omg/state") {
  const inboxPath = buildInstructionPath(teamStateRoot3, "team", teamName, "workers", workerName, "inbox.md");
  if (teamStateRoot3 !== ".omg/state") {
...
function generateMailboxTriggerMessage(teamName, workerName, count = 1, teamStateRoot3 = ".omg/state") {
...
function agentTypeGuidance(agentType) {
  const teamApiCommand = formatOmgCliInvocation("team api");
  const claimTaskCommand = formatOmgCliInvocation("team api claim-task");
  const transitionTaskStatusCommand = formatOmgCliInvocation("team api transition-task-status");
  switch (agentType) {
    case "gemini":
    default:
      return [
...
function generateWorkerOverlay(params) {
  ...
  const claimTaskCommand = formatOmgCliInvocation(`team api claim-task --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\",\\"worker\\":\\"${workerName}\\"}" --json`);
  const sendAckCommand = formatOmgCliInvocation(`team api send-message --input "{\\"team_name\\":\\"${teamName}\\",\\"from_worker\\":\\"${workerName}\\",\\"to_worker\\":\\"leader-fixed\\",\\"body\\":\\"ACK: ${workerName} initialized\\"}" --json`);
  const completeTaskCommand = formatOmgCliInvocation(`team api transition-task-status --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\",\\"from\\":\\"in_progress\\",\\"to\\":\\"completed\\",\\"claim_token\\":\\"<claim_token>\\"}" --json`);
  const failTaskCommand = formatOmgCliInvocation(`team api transition-task-status --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\",\\"from\\":\\"in_progress\\",\\"to\\":\\"failed\\",\\"claim_token\\":\\"<claim_token>\\"}" --json`);
  const readTaskCommand = formatOmgCliInvocation(`team api read-task --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\"}" --json`);
  const releaseClaimCommand = formatOmgCliInvocation(`team api release-task-claim --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\",\\"claim_token\\":\\"<claim_token>\\",\\"worker\\":\\"${workerName}\\"}" --json`);
  const mailboxListCommand = formatOmgCliInvocation(`team api mailbox-list --input "{\\"team_name\\":\\"${teamName}\\",\\"worker\\":\\"${workerName}\\"}" --json`);
  const mailboxDeliveredCommand = formatOmgCliInvocation(`team api mailbox-mark-delivered --input "{\\"team_name\\":\\"${teamName}\\",\\"worker\\":\\"${workerName}\\",\\"message_id\\":\\"<id>\\"}" --json`);
  const teamApiCommand = formatOmgCliInvocation("team api");
  const teamCommand2 = formatOmgCliInvocation("team");
...
- **Environment**: OMG_TEAM_WORKER=${teamName}/${workerName}
...
- To leader: \`${formatOmgCliInvocation(`team api send-message --input "{\\"team_name\\":\\"${teamName}\\",\\"from_worker\\":\\"${workerName}\\",\\"to_worker\\":\\"leader-fixed\\",\\"body\\":\\"<message>\\"}" --json`)}\`
...
## BEFORE YOU EXIT
You MUST call \`${formatOmgCliInvocation("team api transition-task-status")}\` to mark your task as "completed" or "failed" before exiting.
...
function readMetadata(repoRoot, teamName) {
  ...
  if (!existsSync7(metaPath)) return [];
  try {
    return JSON.parse(readFileSync4(metaPath, "utf-8"));
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    process.stderr.write(`[omg] warning: worktrees.json parse error: ${msg}
`);
    return [];
  }
}
...
function buildV2TaskInstruction(teamName, workerName, task, taskId) {
  ...
  const failTaskCommand = formatOmgCliInvocation(
...
async function spawnV2Worker(opts) {
  ...
  const modelForAgent = (() => {
    if (opts.agentType === "gemini") {
      return process.env.OMG_EXTERNAL_MODELS_DEFAULT_ANTHROPIC_MODEL || process.env.OMG_GEMINI_DEFAULT_MODEL || resolveGeminiWorkerModel();
    }
    return void 0;
  })();
    ...
    if (!settled) {
      ...
      if (!settledAfterRetry) {
        return {
          paneId,
          startupAssigned: false,
          startupFailureReason: "gemini_startup_evidence_missing"
        };
      }
    }
  }
...
async function startTeamV2(config) {
  ...
  if (unownedTaskIndices.length > 0) {
    ...
    const allocationWorkers = workerNames.map((name, i) => ({
      name,
      role: config.workerRoles?.[i] ?? (agentTypes[i % agentTypes.length] ?? agentTypes[0] ?? "gemini"),
      currentLoad: 0
    }));
  ...
  for (let i = 0; i < workerNames.length; i++) {
    ...
    const agentType = agentTypes[i % agentTypes.length] ?? agentTypes[0] ?? "gemini";
  ...
  const teamConfig = {
    ...
    agent_type: agentTypes[0] || "gemini",
    worker_launch_mode: "interactive",
  ...
  const permissionsSnapshot = {
    approval_mode: process.env.OMG_APPROVAL_MODE || "default",
    sandbox_mode: process.env.OMG_SANDBOX_MODE || "default",
    network_access: process.env.OMG_NETWORK_ACCESS === "1"
  };
  const teamManifest = {
    schema_version: 2,
    name: sanitized,
  ...
  for (const decision of initialStartupAllocations) {
    ...
    const workerLaunch = await spawnV2Worker({
      ...
      workerName: wName,
      workerIndex,
      agentType: agentTypes[workerIndex % agentTypes.length] ?? agentTypes[0] ?? "gemini",
      task,
      taskId,
      cwd: leaderCwd,
      resolvedBinaryPaths
...
import { readFileSync as readFileSync6, readdirSync as readdirSync3, existsSync as existsSync9, openSync as openSync4, closeSync as closeSync4, unlinkSync as unlinkSync4, writeSync as writeSync4, statSync as statSync3, constants as fsConstants2 } from "fs";
import { join as join12 } from "path";

// src/utils/paths.ts
import { join as join11 } from "path";
import { existsSync as existsSync8, readFileSync as readFileSync5, readdirSync as readdirSync2, statSync as statSync2, unlinkSync as unlinkSync3, rmSync } from "fs";
import { homedir } from "os";
function getStateDir() {
  if (process.platform === "win32") {
    return process.env.LOCALAPPDATA || join11(homedir(), "AppData", "Local");
  }
  return process.env.XDG_STATE_HOME || join11(homedir(), ".local", "state");
}
function prefersXdgOmgDirs() {
...
function getLegacyOmgDir() {
  return join11(getUserHomeDir(), ".omg");
}
function getGlobalOmgStateRoot() {
  const explicitRoot = process.env.OMG_HOME?.trim();
  if (explicitRoot) {
    return join11(explicitRoot, "state");
  }
  if (prefersXdgOmgDirs()) {
    return join11(getStateDir(), "omg");
  }
  return join11(getLegacyOmgDir(), "state");
}
function getGlobalOmgStatePath(...segments) {
  return join11(getGlobalOmgStateRoot(), ...segments);
}
...
async function shutdownTeam(teamName, sessionName2, cwd, timeoutMs = 3e4, workerPaneIds, leaderPaneId, ownsWindow) {
  ...
  const CLI_AGENT_TYPES = /* @__PURE__ */ new Set(["gemini"]);
...
function parseTeamWorkerContextFromEnv(env = process.env) {
  return parseTeamWorkerEnv(env.OMG_TEAM_WORKER) ?? parseTeamWorkerEnv(env.OMX_TEAM_WORKER);
}
function readTeamStateRootFromEnv(env = process.env) {
  const candidate = typeof env.OMG_TEAM_STATE_ROOT === "string" && env.OMG_TEAM_STATE_ROOT.trim() !== "" ? env.OMG_TEAM_STATE_ROOT.trim() : typeof env.OMX_TEAM_STATE_ROOT === "string" && env.OMX_TEAM_STATE_ROOT.trim() !== "" ? env.OMX_TEAM_STATE_ROOT.trim() : "";
...
async function executeTeamCleanupViaRuntime(teamName, cwd) {
  ...
  if (isLegacyRuntimeConfig(config)) {
    ...
    const sessionName2 = typeof legacyConfig.tmuxSession === "string" && legacyConfig.tmuxSession.trim() !== "" ? legacyConfig.tmuxSession.trim() : `omg-team-${teamName}`;
...
function resolveTeamWorkingDirectoryFromMetadata(teamName, candidateCwd, workerContext) {
  ...
  for (const manifestName of ["manifest.json", "manifest.v2.json"]) {
    const fromManifest = readTeamStateRootFromFile(join15(teamRoot, manifestName));
...
var WORKTREE_TRIGGER_STATE_ROOT = "$OMG_TEAM_STATE_ROOT";
...
init_monitor();
init_platform();
var JOB_ID_PATTERN = /^omg-[a-z0-9]{1,16}$/;
var VALID_CLI_AGENT_TYPES = /* @__PURE__ */ new Set(["gemini", "gemini", "gemini"]);
...
function getTeamWorkerIdentityFromEnv(env = process.env) {
  const omg = typeof env.OMG_TEAM_WORKER === "string" ? env.OMG_TEAM_WORKER.trim() : "";
  if (omg) return omg;
...
function resolveJobsDir(env = process.env) {
  return env.OMG_JOBS_DIR || getGlobalOmgStatePath("team-jobs");
}
function resolveRuntimeCliPath(env = process.env) {
  if (env.OMG_RUNTIME_CLI_PATH) {
    return env.OMG_RUNTIME_CLI_PATH;
  }
...
function generateJobId(now = Date.now()) {
  return `omg-${now.toString(36)}${randomUUID6().slice(0, 8)}`;
}
...
var TEAM_USAGE = `
Usage:
  omg team start --agent <gemini|gemini|gemini>[,<agent>...] --task "<task>" [--count N] [--name TEAM] [--cwd DIR] [--new-window] [--json]
...
Examples:
  omg team start --agent gemini --count 2 --task "review auth flow" --new-window
  omg team status omg-abc123
  ...
  omg team 3:gemini "refactor launch command"
`.trim();
