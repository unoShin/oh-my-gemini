...
var GEMINI_FAMILY_HIGH_VARIANTS = {
...
var OMG_CLI_BINARY = "omg";
var OMG_PLUGIN_BRIDGE_PREFIX = 'node "$GEMINI_PLUGIN_ROOT"/bridge/cli.cjs';
...
function buildInstructionPath(...parts) {
  return (0, import_path5.join)(...parts).replaceAll("\\", "/");
}
function generateTriggerMessage(teamName, workerName2, teamStateRoot2 = ".omg/state") {
  const inboxPath = buildInstructionPath(teamStateRoot2, "team", teamName, "workers", workerName2, "inbox.md");
  if (teamStateRoot2 !== ".omg/state") {
...
function generateWorkerOverlay(params) {
  ...
  const claimTaskCommand = formatOmgCliInvocation(`team api claim-task --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\",\\"worker\\":\\"${workerName2}\\"}" --json`);
  const sendAckCommand = formatOmgCliInvocation(`team api send-message --input "{\\"team_name\\":\\"${teamName}\\",\\"from_worker\\":\\"${workerName2}\\",\\"to_worker\\":\\"leader-fixed\\",\\"body\\":\\"ACK: ${workerName2} initialized\\"}" --json`);
  ...
  const releaseClaimCommand = formatOmgCliInvocation(`team api release-task-claim --input "{\\"team_name\\":\\"${teamName}\\",\\"task_id\\":\\"<id>\\",\\"claim_token\\":\\"<claim_token>\\",\\"worker\\":\\"${workerName2}\\"}" --json`);
  const mailboxListCommand = formatOmgCliInvocation(`team api mailbox-list --input "{\\"team_name\\":\\"${teamName}\\",\\"worker\\":\\"${workerName2}\\"}" --json`);
  const mailboxDeliveredCommand = formatOmgCliInvocation(`team api mailbox-mark-delivered --input "{\\"team_name\\":\\"${teamName}\\",\\"worker\\":\\"${workerName2}\\",\\"message_id\\":\\"<id>\\"}" --json`);
  const teamApiCommand = formatOmgCliInvocation("team api");
  const teamCommand = formatOmgCliInvocation("team");
...
- To leader: \`${formatOmgCliInvocation(`team api send-message --input "{\\"team_name\\":\\"${teamName}\\",\\"from_worker\\":\\"${workerName2}\\",\\"to_worker\\":\\"leader-fixed\\",\\"body\\":\\"<message>\\"}" --json`)}\`
...
async function writePanesTrackingFileIfPresent(runtime) {
  ...
  const panesPath = (0, import_path10.join)(omgJobsDir, `${jobId}-panes.json`);
...
async function spawnWorkerForTask(runtime, workerNameValue, taskIndex) {
  ...
  const agentType = runtime.config.agentTypes[workerIndex % runtime.config.agentTypes.length] ?? runtime.config.agentTypes[0] ?? "gemini";
...
function checkPaths(claims, policy) {
  ...
  for (const pathStr of allPaths) {
    ...
    if (!(0, import_fs11.existsSync)(pathStr)) {
...
function getConfigPaths() {
  ...
  return {
    user: (0, import_path13.join)(userConfigDir, "gemini-omg", "config.jsonc"),
    project: (0, import_path13.join)(process.cwd(), ".gemini", "omg.jsonc")
  };
}
function loadJsoncFile(path4) {
  if (!(0, import_fs12.existsSync)(path4)) {
    return null;
  }
  try {
    const content = (0, import_fs12.readFileSync)(path4, "utf-8");
...
function loadEnvConfig() {
  ...
  if (process.env.OMG_EXTERNAL_MODELS_DEFAULT_ANTHROPIC_MODEL) {
    externalModelsDefaults.geminiModelId = process.env.OMG_EXTERNAL_MODELS_DEFAULT_ANTHROPIC_MODEL;
  ...
  if (process.env.OMG_EXTERNAL_MODELS_FALLBACK_POLICY) {
    ...
    if (policy === "provider_chain" || policy === "cross_provider" || policy === "gemini_only") {
...
function loadConfig() {
  ...
  if (config.routing?.forceInherit !== true && process.env.OMG_ROUTING_FORCE_INHERIT === void 0 && isNonGeminiProvider()) {
    config.routing = {
      ...config.routing,
      forceInherit: true
    };
...
async function writeResultArtifact(output, finishedAt, jobId = process.env.OMG_JOB_ID, omgJobsDir = process.env.OMG_JOBS_DIR) {
  if (!jobId || !omgJobsDir) return;
  const resultPath = (0, import_path17.join)(omgJobsDir, `${jobId}-result.json`);
...
async function writePanesFile(jobId, paneIds, leaderPaneId, sessionName2, ownsWindow) {
  const omgJobsDir = process.env.OMG_JOBS_DIR;
  if (!jobId || !omgJobsDir) return;
  const panesPath = (0, import_path17.join)(omgJobsDir, `${jobId}-panes.json`);
  await (0, import_promises8.writeFile)(
    panesPath + ".tmp",
