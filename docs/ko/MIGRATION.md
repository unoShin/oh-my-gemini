...
이 가이드는 oh-my-gemini의 모든 마이그레이션 경로를 다룹니다. 아래에서 현재 사용 중인 버전을 찾아주세요.
...
| 제거된 스킬          | 대체 명령어                            |
| -------------------- | -------------------------------------- |
| `cancel-autopilot`   | `/oh-my-gemini:cancel`             |
| `cancel-ralph`       | `/oh-my-gemini:cancel`             |
| `cancel-ultrawork`   | `/oh-my-gemini:cancel`             |
| `cancel-ultraqa`     | `/oh-my-gemini:cancel`             |
| `cancel-`            | `/oh-my-gemini:cancel`             |
| `omg-default`        | `/oh-my-gemini:omg-setup --local`  |
| `omg-default-global` | `/oh-my-gemini:omg-setup --global` |
| `planner`            | `/oh-my-gemini:plan`               |
...
```bash
/oh-my-gemini:cancel-ralph      # ralph만 취소
/oh-my-gemini:omg-default       # 로컬 프로젝트 설정
/oh-my-gemini:planner "task"    # 플래닝 시작
```

**v3.5.3 이후:**

```bash
/oh-my-gemini:cancel            # 활성 모드를 자동 감지하여 취소
/oh-my-gemini:omg-setup --local # 로컬 프로젝트 설정
/oh-my-gemini:plan "task"       # 플래닝 시작 (인터뷰 모드 포함)
```

### 새로운 기능

**새 스킬: `/learn-about-omg`**
...
```bash
/oh-my-gemini:plan --consensus "task"  # critic 리뷰가 포함된 반복적 플래닝
/oh-my-gemini:ralplan "task"           # plan --consensus의 별칭
```
...
3. GEMINI.md 설정을 업데이트하려면 **`/omg-setup`을 재실행**하세요
...
**3.0 이전:** `/oh-my-gemini:ralph "task"`, `/oh-my-gemini:ultrawork "task"` 등 25개 이상의 명령어를 명시적으로 호출

**3.0 이후:** 자연스럽게 작업하면 Gemini가 자동으로 적절한 동작을 활성화합니다. 최초 설정: "setup omg"라고 말하기만 하면 됩니다

### 프로젝트 리브랜딩

프로젝트의 목적을 더 잘 반영하고 검색성을 개선하기 위해 리브랜딩되었습니다.

- **프로젝트/브랜드명**: `oh-my-gemini` (GitHub 저장소, 플러그인명, 명령어)
- **npm 패키지명**: `oh-my-gemini` (변경 없음)

> **왜 이름이 다른가요?** npm 패키지명 `oh-my-gemini`는 기존 설치와의 하위 호환성을 위해 유지되었습니다. 프로젝트, GitHub 저장소, 플러그인 및 모든 명령어는 `oh-my-gemini`를 사용합니다.
...
/oh-my-gemini:ralph "implement user authentication"       # 지속성 모드
/oh-my-gemini:ultrawork "refactor the API layer"          # 최대 병렬 처리
/oh-my-gemini:planner "plan the new dashboard"            # 플래닝 인터뷰
/oh-my-gemini:deepsearch "find database schema files"     # 딥 서치
/oh-my-gemini:git-master "commit these changes"           # Git 전문가
/oh-my-gemini:deepinit ./src                              # 코드베이스 인덱싱
/oh-my-gemini:analyze "why is this test failing?"         # 심층 분석
```

#### 이후 (3.0): 자동 활성화 + 키워드

자연스럽게 작업하세요. Gemini가 의도를 감지하여 자동으로 동작을 활성화합니다:
...
- **이전**: `~/.gemini/skills/sisyphus-learned/`
- **이후**: `~/.gemini/skills/omg-learned/`

#### 설정 파일

- **이전**: `~/.gemini/sisyphus/mnemosyne.json`
- **이후**: `~/.gemini/omg/learner.json`

### 환경 변수

모든 환경 변수가 `SISYPHUS_*`에서 `OMG_*`로 변경되었습니다:

| 이전                          | 이후                     |
| ----------------------------- | ------------------------ |
| SISYPHUS_USE_NODE_HOOKS       | OMG_USE_NODE_HOOKS       |
| SISYPHUS_USE_BASH_HOOKS       | OMG_USE_BASH_HOOKS       |
| SISYPHUS_PARALLEL_EXECUTION   | OMG_PARALLEL_EXECUTION   |
| SISYPHUS_LSP_TOOLS            | OMG_LSP_TOOLS            |
| SISYPHUS_MAX_BACKGROUND_TASKS | OMG_MAX_BACKGROUND_TASKS |
| SISYPHUS_ROUTING_ENABLED      | OMG_ROUTING_ENABLED      |
| SISYPHUS_ROUTING_DEFAULT_TIER | OMG_ROUTING_DEFAULT_TIER |
| SISYPHUS_ESCALATION_ENABLED   | OMG_ESCALATION_ENABLED   |
| SISYPHUS_DEBUG                | OMG_DEBUG                |
...
| -------------------------------------- | ---------------------------------------------------------- | ------------------- |
| `/oh-my-gemini:ralph "task"`       | "don't stop until done"이라고 말하거나 `ralph` 키워드 사용 | ✅ 예 (양쪽 모두)   |
| `/oh-my-gemini:ultrawork "task"`   | "fast" 또는 "parallel"이라고 말하거나 `ulw` 키워드 사용    | ✅ 예 (양쪽 모두)   |
| `/oh-my-gemini:ultrawork-ralph`    | "ralph ulw:" 접두사 사용                                   | ✅ 예 (키워드 조합) |
| `/oh-my-gemini:planner "task"`     | "plan this"라고 말하거나 `plan` 키워드 사용                | ✅ 예 (양쪽 모두)   |
| `/oh-my-gemini:plan "description"` | 자연스럽게 플래닝 시작                                     | ✅ 예               |
| `/oh-my-gemini:review [path]`      | 기존과 동일하게 호출                                       | ✅ 예 (변경 없음)   |
| `/oh-my-gemini:deepsearch "query"` | "find" 또는 "search"라고 말하기                            | ✅ 예 (자동 감지)   |
| `/oh-my-gemini:analyze "target"`   | "analyze"라고 말하기 — debugger/architect 에이전트로 라우팅 | ✅ 예 (키워드 라우트) |
| `/oh-my-gemini:deepinit [path]`    | 기존과 동일하게 호출                                       | ✅ 예 (변경 없음)   |
| `/oh-my-gemini:git-master`         | "git", "commit", "atomic commit"이라고 말하기              | ✅ 예 (자동 감지)   |
| `/oh-my-gemini:frontend-ui-ux`     | "UI", "styling", "component", "design"이라고 말하기        | ✅ 예 (자동 감지)   |
| `/oh-my-gemini:note "content"`     | "remember this" 또는 "save this"라고 말하기                | ✅ 예 (자동 감지)   |
| `/oh-my-gemini:cancel-ralph`       | "stop", "cancel" 또는 "abort"라고 말하기                   | ✅ 예 (자동 감지)   |
| `/oh-my-gemini:omg-doctor`         | 기존과 동일하게 호출                                       | ✅ 예 (변경 없음)   |
...
더 이상 `/oh-my-gemini:cancel-ralph`이 필요 없습니다 - 그냥 "cancel"이라고 말하세요!
...
# 스킬 디렉토리
mv ~/.gemini/skills/sisyphus-learned ~/.gemini/skills/omg-learned

# 설정 디렉토리
mv ~/.gemini/sisyphus ~/.gemini/omg
```
...
```bash
# 모든 SISYPHUS_* 변수를 OMG_*로 변경
# 예시:
# 이전: export SISYPHUS_ROUTING_ENABLED=true
# 이후: export OMG_ROUTING_ENABLED=true
```
...
- 패키지명: `oh-my-gemini` → `oh-my-gemini`
- 에이전트 이름: 위의 매핑 테이블 사용
- 명령어: 새로운 슬래시 명령어 사용
- 디렉토리 경로: `.sisyphus` → `.omg` 업데이트

#### 7. 최초 설정 실행

Gemini Code에서 "setup omg", "omg setup" 또는 이에 해당하는 자연어 표현을 사용하세요.
...
3. **간단한 명령어 테스트**:
   Gemini Code에서 `/oh-my-gemini:omg-help`를 실행하여 플러그인이 올바르게 로드되었는지 확인하세요.
...
```
이전: "OK, ultrawork를 사용하려면 /oh-my-gemini:ultrawork를 써야지..."
이후: "빨리 해줘!"
      ↓
      Gemini: "ultrawork 모드를 활성화합니다..."
```
...
- **사용자 레벨**: `~/.gemini/skills/omg-learned/` (이식 가능)

#### 4. HUD 상태 표시줄 (실시간 오케스트레이션)

상태 바에서 Gemini가 무엇을 하고 있는지 확인하세요:

```
[OMG] ralph:3/10 | US-002 | ultrawork skill:planner | ctx:67% | agents:2 | todos:2/5
```

설치하려면 `/oh-my-gemini:hud setup`을 실행하세요. 프리셋: minimal, focused, full.
...
또는 `/oh-my-gemini:note`를 사용하여 발견한 것을 수동으로 저장할 수 있습니다:

```bash
/oh-my-gemini:note Project uses PostgreSQL with Prisma ORM
```
...
```bash
/oh-my-gemini:ralph-init "implement OAuth with multiple providers"
    ↓
...
**Gemini가 멈추기 전에 작업이 완료됩니다:**

```
사용자: "사용자 대시보드 구현해줘"
    ↓
Gemini: "완료를 보장하기 위해 ralph-loop을 활성화합니다"
    ↓
...
```bash
/oh-my-gemini:pipeline explore:haiku -> architect:opus -> executor:sonnet
```
...
```bash
/oh-my-gemini: "refactor the authentication system"
```
...
- `/oh-my-gemini:cancel-ralph` (폐기됨)
- `/oh-my-gemini:cancel-ultraqa` (폐기됨)
- `/oh-my-gemini:cancel-ultrawork` (폐기됨)
- `/oh-my-gemini:cancel-` (폐기됨)
- `/oh-my-gemini:cancel-autopilot` (폐기됨)

대신 `/oh-my-gemini:cancel`을 사용하세요.
...
```typescript
Task(
  (subagent_type = "oh-my-gemini:explore-high"),
  (model = "opus"),
...
사용자는 `/oh-my-gemini:omg-setup`을 통해 기본 모드 선호도를 설정할 수 있습니다.
...
`~/.gemini/.omg-config.json`에서 선호하는 실행 모드를 설정하세요:
...
```bash
omg config- --disable
omg config-agent-tiers --disable-low
```
...
- 공지를 위해 [GitHub 저장소](https://github.com/unoShin/oh-my-gemini)를 watch하세요
...
```
/oh-my-gemini:ultrawork "implement the todo list feature"
```
...
"implement the todo list feature quickly"
    ↓
Gemini: "최대 병렬 처리를 위해 ultrawork를 활성화합니다"
```
...
```
/oh-my-gemini:ralph "debug the memory leak"
```
...
```
/oh-my-gemini:planner "design the new authentication system"
```
...
oh-my-gemini를 현재 프로젝트에만 적용합니다:

```
/oh-my-gemini:omg-default
```

생성 파일: `./.gemini/GEMINI.md`

### 글로벌 설정

모든 Gemini Code 세션에 적용합니다:

```
/oh-my-gemini:omg-default-global
```
...
**Q: 키워드를 반드시 사용해야 하나요?**
A: 아니요. 키워드는 선택적 단축키입니다. Gemini가 키워드 없이도 의도를 자동으로 감지합니다.
...
A: 계속 사용하세요! `/oh-my-gemini:ralph`, `/oh-my-gemini:ultrawork`, `/oh-my-gemini:plan`이 작동합니다. 참고: `/oh-my-gemini:planner`는 이제 `/oh-my-gemini:plan`으로 리다이렉트됩니다.

**Q: Gemini가 무엇을 하고 있는지 어떻게 알 수 있나요?**
A: Gemini가 주요 동작을 안내합니다: "ralph-loop을 활성화합니다..." 또는 실시간 상태를 위해 `/oh-my-gemini:hud`를 설정하세요.
...
- **이슈 진단**: `/oh-my-gemini:omg-doctor` 실행
- **모든 명령어 보기**: `/oh-my-gemini:omg-help` 실행
- **실시간 상태 보기**: `/oh-my-gemini:hud setup` 실행
 **상세 변경 로그 확인**: [CHANGELOG.md](../../CHANGELOG.md) 참조
- **버그 보고**: [GitHub Issues](https://github.com/unoShin/oh-my-gemini/issues)
