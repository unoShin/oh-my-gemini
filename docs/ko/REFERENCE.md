...
oh-my-gemini의 전체 레퍼런스입니다. 빠른 시작은 메인 [README.md](../../README.md)를 참조하세요.
...
**Gemini Code 플러그인 방식만 지원됩니다.** 다른 설치 방법 (npm, bun, curl)은 폐기되었으며 올바르게 작동하지 않을 수 있습니다.
...
이 방법은 Gemini Code의 플러그인 시스템과 직접 통합되며 Node.js 훅을 사용합니다.

> **참고**: npm/bun 글로벌 직접 설치는 **지원되지 않습니다**. 플러그인 시스템이 모든 설치 및 훅 설정을 자동으로 처리합니다.

### 요구 사항

- [Gemini Code](https://docs.anthropic.com/gemini-cli) 설치됨
- 다음 중 하나:
  - **Gemini Max/Pro 구독** (개인 사용자에게 권장)
...
현재 프로젝트에만 omg를 설정합니다:

```
/oh-my-gemini:omg-setup
```

- 현재 프로젝트에 `./.gemini/GEMINI.md`를 생성합니다
- 설정은 이 프로젝트에만 적용됩니다
- 다른 프로젝트나 글로벌 설정에는 영향을 주지 않습니다
- **안전**: 글로벌 GEMINI.md를 보존합니다

### 글로벌 설정

모든 Gemini Code 세션에 omg를 설정합니다:

```
/oh-my-gemini:omg-setup
```

- 글로벌로 `~/.gemini/GEMINI.md`를 생성합니다
- 설정은 모든 프로젝트에 적용됩니다
- **주의**: 기존 `~/.gemini/GEMINI.md`를 완전히 덮어씁니다
...
```
./.gemini/GEMINI.md  (프로젝트)   →  덮어씀  →  ~/.gemini/GEMINI.md  (글로벌)
```
...
- **다른 머신**: Gemini Code를 사용하는 각 머신에서 실행
- **새 프로젝트**: omg가 필요한 각 프로젝트에서 `/oh-my-gemini:omg-setup --local` 실행

> **참고**: 플러그인 업데이트 후 (`npm update`, `git pull` 또는 Gemini Code의 플러그인 업데이트를 통해), 최신 GEMINI.md 변경사항을 적용하려면 반드시 `/oh-my-gemini:omg-setup`을 재실행**해야 합니다**.

### 에이전트 커스터마이징

`~/.gemini/agents/`의 에이전트 파일을 편집하여 동작을 커스터마이징할 수 있습니다:
...
프로젝트별 지침을 위해 프로젝트에 `.gemini/GEMINI.md`를 생성하세요:
...
`omg config-stop-callback`으로 Telegram/Discord stop callback 태그를 설정합니다.
...
Task 도구를 통해 호출할 때는 항상 `oh-my-gemini:` 접두사를 사용하세요.
...
| `autopilot`   | 아이디어에서 작동하는 코드까지 완전 자율 실행 | `/oh-my-gemini:autopilot`  |
| `ultrawork`   | 병렬 에이전트를 통한 최대 성능                | `/oh-my-gemini:ultrawork`  |
| `pipeline`    | 순차적 에이전트 체이닝                        | `/oh-my-gemini:pipeline`   |
| ``            | 토큰 효율적 병렬 실행                         | `/oh-my-gemini:`           |
| `ralph`       | 완료될 때까지 자기 참조적 개발                | `/oh-my-gemini:ralph`      |
| `ralph-init`  | 구조화된 작업 추적을 위한 PRD 초기화          | `/oh-my-gemini:ralph-init` |
| `ultraqa`     | 자율 QA 사이클링 워크플로우                   | `/oh-my-gemini:ultraqa`    |
| `plan`        | 플래닝 세션 시작                              | `/oh-my-gemini:plan`       |
| `ralplan`     | 반복적 플래닝 (planner+architect+critic)      | `/oh-my-gemini:ralplan`    |
| `review`      | critic을 통한 작업 플랜 리뷰                  | `/oh-my-gemini:review`     |

### 향상 스킬

| 스킬              | 설명                                      | 수동 명령어                         |
| ----------------- | ----------------------------------------- | ----------------------------------- |
| `deepinit`        | 계층적 AGENTS.md 코드베이스 문서화        | `/oh-my-gemini:deepinit`        |
| `deepsearch`      | 다중 전략 코드베이스 검색                 | `/oh-my-gemini:deepsearch`      |
| `sciomg`          | 병렬 scientist 오케스트레이션             | `/oh-my-gemini:sciomg`          |
| `frontend-ui-ux`  | 디자이너 출신 개발자의 UI/UX 전문성       | (자동 활성화)                       |
| `git-master`      | 원자적 커밋 및 히스토리를 위한 Git 전문가 | (자동 활성화)                       |
| `learner`         | 세션에서 재사용 가능한 스킬 추출          | `/oh-my-gemini:learner`         |

### 유틸리티 스킬

| 스킬                      | 설명                                          | 수동 명령어                                 |
| ------------------------- | --------------------------------------------- | ------------------------------------------- |
| `note`                    | 컨텍스트 압축에 강한 노트패드에 메모 저장     | `/oh-my-gemini:note`                    |
| `cancel`                  | 모든 모드에 대한 통합 취소                    | `/oh-my-gemini:cancel`                  |
| `omg-setup`               | 최초 설정 마법사                              | `/oh-my-gemini:omg-setup`               |
| `omg-doctor`              | 설치 문제 진단 및 수정                        | `/oh-my-gemini:omg-doctor`              |
| `omg-help`                | OMG 사용 가이드 표시                          | `/oh-my-gemini:omg-help`                |
| `hud`                     | HUD 상태 표시줄 설정                          | `/oh-my-gemini:hud`                     |
| `release`                 | 자동 릴리스 워크플로우                        | `/oh-my-gemini:release`                 |
| `mcp-setup`               | MCP 서버 설정                                 | `/oh-my-gemini:mcp-setup`               |
| `writer-memory`           | 작성자를 위한 에이전틱 메모리 시스템          | `/oh-my-gemini:writer-memory`           |
| `project-session-manager` | 격리된 개발 환경 관리 (git worktrees + tmux)  | `/oh-my-gemini:project-session-manager` |
| `skill`                   | 로컬 스킬 관리 (목록, 추가, 제거, 검색, 편집) | `/oh-my-gemini:skill`                   |

---

## 슬래시 명령어

모든 스킬은 `/oh-my-gemini:` 접두사가 붙은 슬래시 명령어로 사용할 수 있습니다.

| 명령어                                       | 설명                                      |
| -------------------------------------------- | ----------------------------------------- |
| `/oh-my-gemini:orchestrate <task>`       | 멀티 에이전트 오케스트레이션 모드 활성화  |
| `/oh-my-gemini:autopilot <task>`         | 완전 자율 실행                            |
| `/oh-my-gemini:ultrawork <task>`         | 병렬 에이전트를 통한 최대 성능 모드       |
| `/oh-my-gemini:pipeline <stages>`        | 순차적 에이전트 체이닝                    |
| `/oh-my-gemini: <task>`                  | 토큰 효율적 병렬 실행                     |
| `/oh-my-gemini:ralph-init <task>`        | 구조화된 작업 추적을 위한 PRD 초기화      |
| `/oh-my-gemini:ralph <task>`             | 작업 완료까지 자기 참조 루프              |
| `/oh-my-gemini:ultraqa <goal>`           | 자율 QA 사이클링 워크플로우               |
| `/oh-my-gemini:plan <description>`       | 플래닝 세션 시작                          |
| `/oh-my-gemini:ralplan <description>`    | 합의를 통한 반복적 플래닝                 |
| `/oh-my-gemini:review [plan-path]`       | critic을 통한 플랜 리뷰                   |
| `/oh-my-gemini:deepsearch <query>`       | 다중 전략 코드베이스 검색                 |
| `/oh-my-gemini:deepinit [path]`          | 계층적 AGENTS.md 파일로 코드베이스 인덱싱 |
| `/oh-my-gemini:sciomg <topic>`           | 병렬 리서치 오케스트레이션                |
| `/oh-my-gemini:learner`                  | 세션에서 재사용 가능한 스킬 추출          |
| `/oh-my-gemini:note <content>`           | notepad.md에 메모 저장                    |
| `/oh-my-gemini:cancel`                   | 통합 취소                                 |
| `/oh-my-gemini:omg-setup`                | 최초 설정 마법사                          |
| `/oh-my-gemini:omg-doctor`               | 설치 문제 진단 및 수정                    |
| `/oh-my-gemini:omg-help`                 | OMG 사용 가이드 표시                      |
| `/oh-my-gemini:hud`                      | HUD 상태 표시줄 설정                      |
| `/oh-my-gemini:release`                  | 자동 릴리스 워크플로우                    |
| `/oh-my-gemini:mcp-setup`                | MCP 서버 설정                             |

---

## 훅 시스템

oh-my-gemini에는 Gemini Code의 동작을 향상시키는 31개의 라이프사이클 훅이 포함되어 있습니다.
...
| 변수                             | 값                                   | 설명                                                |
| -------------------------------- | ------------------------------------ | --------------------------------------------------- |
| `OMG_MCP_OUTPUT_PATH_POLICY`     | `strict` (기본값), `redirect_output` | 출력 파일 경로 적용 제어                            |
| `OMG_MCP_OUTPUT_REDIRECT_DIR`    | 경로 (기본값: `.omg/outputs`)        | 정책이 `redirect_output`일 때 리다이렉트할 디렉토리 |
| `OMG_MCP_ALLOW_EXTERNAL_PROMPT`  | `0` (기본값), `1`                    | working directory 외부의 프롬프트 파일 허용         |
| `OMG_ALLOW_EXTERNAL_WORKDIR`     | 미설정 (기본값), `1`                 | 프로젝트 worktree 외부의 working_directory 허용     |
| `OMG_DISCORD_WEBHOOK_URL`        | URL                                  | 알림용 Discord 웹훅 URL                             |
| `OMG_DISCORD_NOTIFIER_BOT_TOKEN` | 토큰                                 | Bot API 알림용 Discord 봇 토큰                      |
| `OMG_DISCORD_NOTIFIER_CHANNEL`   | 채널 ID                              | Bot API 알림용 Discord 채널 ID                      |
| `OMG_DISCORD_MENTION`            | `<@uid>` 또는 `<@&role_id>`          | Discord 메시지에 추가할 멘션                        |
| `OMG_TELEGRAM_BOT_TOKEN`         | 토큰                                 | 알림용 Telegram 봇 토큰                             |
| `OMG_TELEGRAM_CHAT_ID`           | 채팅 ID                              | 알림용 Telegram 채팅 ID                             |
| `OMG_SLACK_WEBHOOK_URL`          | URL                                  | 알림용 Slack 수신 웹훅 URL                          |

### 정책 설명

**`OMG_MCP_OUTPUT_PATH_POLICY=strict` (기본값)**

- 출력 파일은 `working_directory` 내에 있어야 합니다
- 경계 외부에 쓰려는 시도는 `E_PATH_OUTSIDE_WORKDIR_OUTPUT`으로 실패합니다
- 가장 안전한 옵션 - 프로덕션에 권장

**`OMG_MCP_OUTPUT_PATH_POLICY=redirect_output`**

- 출력 파일이 자동으로 `OMG_MCP_OUTPUT_REDIRECT_DIR`로 리다이렉트됩니다
- 파일명만 보존되며 디렉토리 구조는 평탄화됩니다
- 모든 출력을 한 곳에 모으고 싶을 때 유용합니다
- `[MCP Config]` 수준에서 리다이렉트를 로깅합니다

**`OMG_MCP_ALLOW_EXTERNAL_PROMPT=1`**

- `working_directory` 외부의 프롬프트 파일 읽기를 허용합니다
- **보안 경고**: 파일시스템의 임의 파일 읽기를 가능하게 합니다
- 신뢰할 수 있는 환경에서만 사용하세요

**`OMG_ALLOW_EXTERNAL_WORKDIR=1`**
...
| `E_PATH_OUTSIDE_WORKDIR_OUTPUT` 오류                | output_file이 working_directory 외부에 있음   | working directory 내의 상대 경로를 사용하거나 `OMG_MCP_OUTPUT_PATH_POLICY=redirect_output` 설정 |
...
| `working_directory is outside the project worktree` | working_directory가 git worktree 내에 없음    | `OMG_ALLOW_EXTERNAL_WORKDIR=1` 설정 또는 프로젝트 내부의 working directory 사용                 |
| 출력 파일이 예상 위치에 없음                        | `redirect_output` 정책 활성 상태              | `OMG_MCP_OUTPUT_REDIRECT_DIR` 확인 (기본값: `.omg/outputs`)                                     |
...
> **고급**: macOS/Linux에서 Node.js 훅을 사용하려면 `OMG_USE_NODE_HOOKS=1`을 설정하세요.
...
oh-my-gemini에는 에이전트 성능, 토큰 사용량 및 병렬 워크플로우 디버깅을 위한 종합 모니터링이 포함되어 있습니다.
...
| **Session-End Summaries** | 세션 종료 시 기록되는 요약 및 콜백 페이로드 | `.omg/sessions/*.json`, `session-end` |
...
```bash
omg hud                              # 현재 HUD 상태줄 렌더링
omg team status <team-name>          # 실행 중인 팀 작업 확인
...
- **[MarginLab.ai](https://marginlab.ai)** - Gemini 모델 성능 저하를 감지하기 위한 통계적 유의성 테스트가 포함된 SWE-Bench-Pro 성능 추적
...
`~/.gemini/settings.json`에서 HUD 요소를 설정합니다:
...
| 문제                     | 해결 방법                                                                            |
| ------------------------ | ------------------------------------------------------------------------------------ |
| 명령어를 찾을 수 없음    | `/oh-my-gemini:omg-setup` 재실행                                                 |
| 훅이 실행되지 않음       | 훅 권한 확인: `chmod +x ~/.gemini/hooks/**/*.sh`                                     |
| 에이전트가 위임하지 않음 | GEMINI.md가 로드되었는지 확인: `./.gemini/GEMINI.md` 또는 `~/.gemini/GEMINI.md` 확인 |
| LSP 도구가 작동하지 않음 | 언어 서버 설치: `npm install -g typescript-language-server`                          |
| 토큰 제한 오류           | 토큰 효율적 실행을 위해 `/oh-my-gemini:` 사용                                    |

### 자동 업데이트

oh-my-gemini에는 백그라운드에서 업데이트를 확인하는 무음 자동 업데이트 시스템이 포함되어 있습니다.
...
수동으로 업데이트하려면 플러그인 설치 명령어를 재실행하거나 Gemini Code의 내장 업데이트 메커니즘을 사용하세요.

### 제거

```bash
curl -fsSL https://raw.githubusercontent.com/unoShin/oh-my-gemini/main/scripts/uninstall.sh | bash
```

또는 수동으로:

```bash
rm ~/.gemini/agents/{architect,document-specialist,explore,designer,writer,vision,critic,analyst,executor,qa-tester}.md
rm ~/.gemini/commands/{analyze,autopilot,deepsearch,plan,review,ultrawork}.md
```
