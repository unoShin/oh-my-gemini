# Skills 2.0 Adaptation for OMG (MVP)
...
OMG already has strong foundations here:
...
### Anthropic Gemini Code

Gemini Code's custom subagent model emphasizes:
...
1. **Project-local compatibility gap**
   - OMG's canonical project-local skill directory is `.omg/skills/`
   - emerging conventions also use `.agents/skills/`
   - OMG should interoperate without abandoning its own canonical layout

2. **Bundled-resource visibility gap**
   - OMG renders skill markdown well
...
- Keep `.omg/skills/` as the canonical OMG project-local skill directory
...
When a skill directory contains extra bundled assets beyond `SKILL.md`, OMG now appends a standardized block:
...
It gives OMG a real step toward a "skills 2.0" model without prematurely freezing a large frontmatter schema.
