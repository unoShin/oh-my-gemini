...
const TEAM_HELP = `
Usage: omg team [N:agent-type[:role]] [--new-window] "<task description>"
...
Examples:
  omg team 3:gemini "fix failing tests"
  omg team 2:gemini:architect "design auth system"
  omg team 1:gemini:executor "implement feature"
  omg team 1:gemini,1:gemini "compare approaches"
  omg team 2:gemini "review auth flow" --new-window
...
export function parseTeamArgs(tokens: string[]): ParsedTeamArgs {
  ...
  // Try comma-separated multi-type spec first (e.g. "1:gemini,1:gemini" or "2:gemini,1:gemini:architect")
  ...
  if (first.includes(',')) {
    ...
    for (const seg of segments) {
      ...
      parsedSegments.push({ count, type: m[2] || 'gemini', role: m[3] });
    }
  ...
  // Fall back to single spec (e.g. "3:gemini" or "2:gemini:architect")
  if (!specMatched) {
  ...
  // Default: 3 gemini workers if no spec matched
  if (agentTypes.length === 0) {
    agentTypes = Array.from({ length: workerCount }, () => 'gemini');
    workerSpecs = Array.from({ length: workerCount }, () => ({ agentType: 'gemini' }));
  }
...
async function handleTeamStart(parsed: ParsedTeamArgs, cwd: string): Promise<void> {
  ...
  // Load role prompt if a role was specified (e.g., 3:gemini:architect)
  ...
  // Use v2 runtime by default (OMG_RUNTIME_V2 opt-out), otherwise fall back to v1
