...
export function buildAutoresearchSetupPrompt(input: AutoresearchSetupSessionInput): string {
  ...
  return [
    'You are a short-lived Gemini Code setup assistant for OMG autoresearch.',
...
export function runAutoresearchSetupSession(input: AutoresearchSetupSessionInput): AutoresearchSetupHandoff {
  ...
  const result = spawnSync('gemini', ['-p', prompt], {
    ...
    env: {
      ...process.env,
      GEMINI_CODE_ENTRYPOINT: AUTORESEARCH_SETUP_ENTRYPOINT,
    },
  });
  ...
  if (result.status !== 0) {
    throw new Error(`gemini_autoresearch_setup_failed:${result.status ?? 'unknown'}`);
  }
