...
interface RunOptions {
  preserveGeminiSessionEnv?: boolean;
}
...
function writeFakeProviderBinary(dir: string, provider: 'gemini' | 'gemini'): string {
...
function writeFakeGeminiBinary(dir: string): string {
  const binDir = join(dir, 'bin');
  mkdirSync(binDir, { recursive: true });
...
describe('omg ask command', () => {
  ...
  it('loads --agent-prompt role from resolved prompts dir and prepends role content', () => {
    ...
    try {
      ...
      const result = runCli(
        ['ask', 'gemini', '--agent-prompt=executor', '--prompt', 'ship feature'],
        wd,
        { OMG_ASK_ADVISOR_SCRIPT: stubPath },
      );
...
describe('resolveAskAdvisorScriptPath', () => {
  it('resolves canonical env and supports package-root relative paths', () => {
    ...
    expect(resolveAskAdvisorScriptPath(packageRoot, { OMG_ASK_ADVISOR_SCRIPT: 'scripts/custom.js' } as NodeJS.ProcessEnv))
      .toBe('/tmp/pkg-root/scripts/custom.js');
    expect(resolveAskAdvisorScriptPath(packageRoot, { OMG_ASK_ADVISOR_SCRIPT: '/opt/custom.js' } as NodeJS.ProcessEnv))
      .toBe('/opt/custom.js');
  });
});
