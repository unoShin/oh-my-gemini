...
export function prepareAutoresearchSetupGeminiHome(repoRoot: string, sessionName: string): string {
  ...
  writeFileSync(
    join(tempGeminiHome, '.omx-config.json'),
...
export function spawnAutoresearchSetupTmux(repoRoot: string): void {
  ...
  const paneId = execFileSync(
    'tmux',
    ['new-session', '-d', '-P', '-F', '#{pane_id}', '-s', sessionName, '-c', repoRoot, wrappedGeminiCommand],
    { encoding: 'utf-8' },
  ).trim();
