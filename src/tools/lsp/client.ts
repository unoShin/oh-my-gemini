...
/** Default timeout (ms) for LSP requests. Override with OMG_LSP_TIMEOUT_MS env var. */
export const DEFAULT_LSP_REQUEST_TIMEOUT_MS: number = (() => {
  return readPositiveIntEnv('OMG_LSP_TIMEOUT_MS', 15_000);
})();
...
export const IDLE_TIMEOUT_MS = readPositiveIntEnv('OMG_LSP_IDLE_TIMEOUT_MS', 5 * 60 * 1000);
/** Check for idle clients every 60 seconds */
export const IDLE_CHECK_INTERVAL_MS = readPositiveIntEnv('OMG_LSP_IDLE_CHECK_INTERVAL_MS', 60 * 1000);
...
const LSP_CLIENT_MANAGER_KEY = '__omgLspClientManager';
