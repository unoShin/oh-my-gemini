...
// ============================================================================
// OMG_STATE_DIR SUPPORT (Issue #1014)
// ============================================================================
 ...
 */
export function getOmgRoot(worktreeRoot?: string): string {
 ...
 */
export function resolveOmgPath(relativePath: string, worktreeRoot?: string): string {
  validatePath(relativePath);

  const omgDir = getOmgRoot(worktreeRoot);
  const fullPath = normalize(resolve(omgDir, relativePath));

  // Verify resolved path is still under omg directory
  const relativeToOmg = relative(omgDir, fullPath);
  if (relativeToOmg.startsWith('..') || relativeToOmg.startsWith(sep + '..')) {
 ...
 */
export function ensureOmgDir(relativePath: string, worktreeRoot?: string): string {
...
export function getWorktreeNotepadPath(worktreeRoot?: string): string {
  return join(getOmgRoot(worktreeRoot), 'notepad.md');
}
...
export function getWorktreeProjectMemoryPath(worktreeRoot?: string): string {
  return join(getOmgRoot(worktreeRoot), 'project-memory.json');
}
...
export function resolvePlanPath(planName: string, worktreeRoot?: string): string {
  ...
  return join(getOmgRoot(worktreeRoot), 'plans', `${planName}.md`);
}
...
export function resolveResearchPath(name: string, worktreeRoot?: string): string {
  validatePath(name);
  return join(getOmgRoot(worktreeRoot), 'research', name);
}
...
export function resolveLogsPath(worktreeRoot?: string): string {
  return join(getOmgRoot(worktreeRoot), 'logs');
}
...
export function resolveWisdomPath(planName: string, worktreeRoot?: string): string {
  validatePath(planName);
  return join(getOmgRoot(worktreeRoot), 'notepads', planName);
}
 ...
 */
export function isPathUnderOmg(absolutePath: string, worktreeRoot?: string): boolean {
  const omgRoot = getOmgRoot(worktreeRoot);
  const normalizedPath = normalize(absolutePath);
  const normalizedOmg = normalize(omgRoot);
  return normalizedPath.startsWith(normalizedOmg + sep) || normalizedPath === normalizedOmg;
}

/**
 * Ensure all standard .omg subdirectories exist.
 */
export function ensureAllOmgDirs(worktreeRoot?: string): void {
  const omgRoot = getOmgRoot(worktreeRoot);
  const subdirs = ['', 'state', 'plans', 'research', 'logs', 'notepads', 'drafts'];
  for (const subdir of subdirs) {
    const fullPath = subdir ? join(omgRoot, subdir) : omgRoot;
    if (!existsSync(fullPath)) {
      mkdirSync(fullPath, { recursive: true });
    }
  }
...
export function getSessionStateDir(sessionId: string, worktreeRoot?: string): string {
  validateSessionId(sessionId);
  return join(getOmgRoot(worktreeRoot), 'state', 'sessions', sessionId);
}
...
export function listSessionIds(worktreeRoot?: string): string[] {
  const sessionsDir = join(getOmgRoot(worktreeRoot), 'state', 'sessions');
...
export function resolveTranscriptPath(transcriptPath: string | undefined, cwd?: string): string | undefined {
  ...
  // The pattern `--gemini-worktrees-` appears when Gemini Code encodes a CWD
  // containing `/.gemini/worktrees/` (separator `/` → `-`, dot `.` → `-`).
  // Strip everything from this pattern to the next `/` to recover the original
  // project directory encoding.
  const worktreeSegmentPattern = /--gemini-worktrees-[^/\\]+/;
  ...
  // When the CWD contains `/.gemini/worktrees/`, we can derive the main
  // project root and look for the transcript there.
  const effectiveCwd = cwd || process.cwd();
  const worktreeMarker = '.gemini/worktrees/';
  ...
  if (markerIdx !== -1) {
    ...
    if (sessionFile) {
      // The projects directory is under the Gemini config dir
      const configDir = process.env.GEMINI_CONFIG_DIR || join(homedir(), '.gemini');
      const projectsDir = join(configDir, 'projects');

      if (existsSync(projectsDir)) {
        // Encode the main project root the same way Gemini Code does:
