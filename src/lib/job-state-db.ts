  ...
  try {
    ...
    if (!Database) {
      try {
        ...
        // Handle interleaved ESM/CJS exports (default might be on .default or the variable itself)
        Database = (betterSqlite3 as any).default?.default || (betterSqlite3 as any).default || betterSqlite3;
        
        if (typeof Database !== 'function') {
          // If it's still not a function, check if it's the module object itself
          Database = (betterSqlite3 as any).Database || betterSqlite3;
        }
    ...
    db.exec(`
      ...
      -- Job metadata for Gemini/Gemini background jobs
...
export function getJobsByStatus(
  provider: "gemini" | "gemini" | undefined,
  status: string,
  cwd?: string,
): JobStatus[] {
  const db = getDb(cwd);
