...
import { getGlobalOmgStateCandidates, getGlobalOmgStateRoot } from '../utils/paths.js';
 ...
 * OMG_TEST_REGISTRY_DIR overrides the default global state dir so that tests
 * can redirect all I/O to a temporary directory without touching global state.
 */
function getRegistryStateDir(): string {
  return process.env['OMG_TEST_REGISTRY_DIR'] ?? getGlobalOmgStateRoot();
}
...
function getRegistryReadPaths(): string[] {
  if (process.env['OMG_TEST_REGISTRY_DIR']) {
    return [getRegistryPath()];
  }

  return getGlobalOmgStateCandidates('reply-session-registry.jsonl');
}
