...
import { getGlobalOmgStateRoot } from '../utils/paths.js';
import {
  capturePaneContent,
  sendToPane,
  isTmuxAvailable,
 ...
 * OMG_* notification env vars are forwarded so the daemon can call getNotificationConfig().
 */
...
/** Default paths */
const DEFAULT_STATE_DIR = getGlobalOmgStateRoot();
...
function createMinimalDaemonEnv(): NodeJS.ProcessEnv {
  ...
  // Forward OMG_* env vars so the daemon can call getNotificationConfig()
        ...
        try {
          ...
          await fetch(
            `https://discord.com/api/v10/channels/${config.discordChannelId}/messages`,
            {
              ...
              body: JSON.stringify({
                content: `${mentionPrefix}Injected into Gemini Code session.`,
