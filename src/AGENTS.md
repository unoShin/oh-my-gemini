...
TypeScript source code for oh-my-gemini - the core library that powers multi-agent orchestration.
...
| File | Description |
|------|-------------|
| `index.ts` | Main entry point - exports `createOmgSession()` |
...
2. **Entry Point Pattern**:
   ...
   export { createOmgSession } from './session';
   export { lspTools, astTools, allCustomTools } from './tools';
   export { getAgentDefinitions, omgSystemPrompt } from './agents/definitions';
   ```
