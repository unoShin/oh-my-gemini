 ...
 * - OMG_MCP_OUTPUT_PATH_POLICY=strict|redirect_output (default: strict)
 * - OMG_MCP_OUTPUT_REDIRECT_DIR=.omg/outputs (default: .omg/outputs)
 * - OMG_MCP_ALLOW_EXTERNAL_PROMPT=0|1 (default: 0)
 *
...
export function loadMcpConfig(): McpConfig {
  const outputPathPolicy = parseOutputPathPolicy(process.env.OMG_MCP_OUTPUT_PATH_POLICY);
  const outputRedirectDir = process.env.OMG_MCP_OUTPUT_REDIRECT_DIR || DEFAULT_MCP_CONFIG.outputRedirectDir;
  const allowExternalPrompt = parseBooleanEnv(process.env.OMG_MCP_ALLOW_EXTERNAL_PROMPT, DEFAULT_MCP_CONFIG.allowExternalPrompt);
  ...
  if (config.allowExternalPrompt) {
    console.warn('[MCP Config] WARNING: OMG_MCP_ALLOW_EXTERNAL_PROMPT is enabled. External prompt files outside the working directory are allowed. This may pose a security risk.');
  }
