...
function readResultArtifact(omgJobsDir: string, jobId: string): ArtifactOutcome {
...
export function convergeJobWithResultArtifact(
  ...
  jobId: string,
  omgJobsDir: string,
): { job: OmgTeamJob; changed: boolean } {
...
export function isJobTerminal(job: OmgTeamJob): boolean {
  return job.status === 'completed' || job.status === 'failed' || job.status === 'timeout';
}

export function clearScopedTeamState(job: Pick<OmgTeamJob, 'cwd' | 'teamName'>): string {
