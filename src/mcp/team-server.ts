...
import type { OmgTeamJob } from './team-job-convergence.js';
import { getGlobalOmgStatePath } from '../utils/paths.js';

const omgTeamJobs = new Map<string, OmgTeamJob>();
...
function buildCliReplacement(toolName: DeprecatedTeamToolName, args: unknown): string {
  ...
  if (toolName === 'omg_run_team_start') {
    ...
    const flags: string[] = ['omg', 'team', 'start'];
...
function persistJob(jobId: string, job: OmgTeamJob): void {
  try {
    if (!existsSync(OMG_JOBS_DIR)) mkdirSync(OMG_JOBS_DIR, { recursive: true });
    writeFileSync(join(OMG_JOBS_DIR, `${jobId}.json`), JSON.stringify(job), 'utf-8');
  } catch { /* best-effort */ }
}

function loadJobFromDisk(jobId: string): OmgTeamJob | undefined {
  try {
    return JSON.parse(readFileSync(join(OMG_JOBS_DIR, `${jobId}.json`), 'utf-8')) as OmgTeamJob;
  } catch {
    return undefined;
  }
}
...
function saveJobState(jobId: string, job: OmgTeamJob): OmgTeamJob {
  omgTeamJobs.set(jobId, job);
  persistJob(jobId, job);
  return job;
}

function makeJobResponse(jobId: string, job: OmgTeamJob, extra: Record<string, unknown> = {}): { content: Array<{ type: 'text'; text: string }> } {
...
async function handleStart(args: unknown): Promise<{ content: Array<{ type: 'text'; text: string }> }> {
  ...
  const job: OmgTeamJob = { status: 'running', startedAt: Date.now(), teamName: input.teamName, cwd: input.cwd };
  omgTeamJobs.set(jobId, job);
