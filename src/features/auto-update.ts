...
import {
  install as installOmg,
  HOOKS_DIR,
  isProjectScopedPlugin,
  isRunningAsPlugin,
  getInstalledOmgPluginRoots,
 ...
 */
export function getOMGConfig(): OMGConfig {
  ...
  try {
    ...
    const config = JSON.parse(content) as OMGConfig;
    return {
...
function resolveOmgBinaryPath(): string {
...
export async function silentAutoUpdate(config: SilentUpdateConfig = {}): Promise<UpdateResult | null> {
  const {
    ...
    autoApply = true,
    logFile = join(GEMINI_CONFIG_DIR, '.omg-update.log'),
    maxRetries = 3
  } = config;
