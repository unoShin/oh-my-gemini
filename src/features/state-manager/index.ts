 ...
 * - Global state: XDG-aware user OMG state with legacy ~/.omg/state fallback
 *
 * Features:
...
import { getGlobalOmgStateRoot, getLegacyOmgPath } from "../../utils/paths.js";
import {
  StateLocation,
  StateConfig,
  StateReadResult,
 ...
 */
const GLOBAL_STATE_DIR = getGlobalOmgStateRoot();
...
export function getLegacyPaths(name: string, location: StateLocation = StateLocation.LOCAL): string[] {
  ...
  if (location === StateLocation.GLOBAL) {
    legacyPaths.push(getLegacyOmgPath("state", `${name}.json`));
  }
