...
import { rewriteOmgCliInvocations } from '../../utils/omg-cli-rendering.js';
...
/**
 * Gemini Code native commands that must not be shadowed by OMG skill short names.
...
function loadSkillFromFile(skillPath: string, skillName: string): BuiltinSkill[] {
  try {
    ...
    const renderedBody = rewriteOmgCliInvocations(body.trim());
    const template = [
      renderedBody,
