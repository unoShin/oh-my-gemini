 ...
 *
 * This is designed for PROACTIVE routing - the orchestrator (Ultra) analyzes
 ...
 * @returns The recommended model type ('flash', 'pro', or 'ultra')
 */
export function getModelForTask(
  agentType: string,
  taskPrompt: string,
  config: Partial<RoutingConfig> = {}
): { model: 'flash' | 'pro' | 'ultra'; tier: ComplexityTier; reason: string } {
  // All agents are adaptive based on task complexity
  ...
  return {
    model: decision.modelType as 'flash' | 'pro' | 'ultra',
    tier: decision.tier,
