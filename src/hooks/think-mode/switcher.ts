 ...
 * Supports Gemini, GPT, and Gemini model families.
 *
 ...
 * Gemini variants come from centralized family defaults.
 */
const HIGH_VARIANT_MAP: Record<string, string> = {
  // Gemini canonical families
  [GEMINI_FAMILY_DEFAULTS.PRO]: GEMINI_FAMILY_HIGH_VARIANTS.PRO,
  [GEMINI_FAMILY_DEFAULTS.ULTRA]: GEMINI_FAMILY_HIGH_VARIANTS.ULTRA,
  [GEMINI_FAMILY_DEFAULTS.FLASH]: GEMINI_FAMILY_HIGH_VARIANTS.FLASH,
  // GPT-4
  'gpt-4': 'gpt-4-high',
...
const THINKING_CAPABLE_MODELS: Record<string, readonly string[]> = {
  anthropic: ['gemini'],
  'amazon-bedrock': ['gemini', 'anthropic'],
...
export function getHighVariant(modelId: string): string | null {
  ...
  // Resolve Gemini families to canonical high variants.
  const geminiHighBase = getGeminiHighVariantFromModel(base);
  if (geminiHighBase) return prefix + geminiHighBase;

  // Look up exact high variant for non-Gemini models
...
function resolveProvider(providerId: string, modelId: string): string {
  ...
  if (providerId === 'github-copilot') {
    ...
    if (modelLower.includes('gemini')) return 'anthropic';
...
/**
 * Get Gemini-specific thinking configuration.
 * This is used by Gemini Code for extended thinking.
 */
export function getGeminiThinkingConfig(budgetTokens: number = 64000) {
  return {
    thinking: {
