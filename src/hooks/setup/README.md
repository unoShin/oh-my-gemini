...
### `init`
Initializes OMG directory structure and environment on first run or explicit setup.
...
- Sets environment variables (`OMG_INITIALIZED=true`) if `GEMINI_ENV_FILE` is available
...
```json
{
  ...
  "hookSpecificOutput": {
    "hookEventName": "Setup",
    "additionalContext": "OMG initialized:\n- 5 directories created\n- 1 configs validated\n- Environment variables set: OMG_INITIALIZED"
  }
}
```

### `maintenance`
Performs periodic maintenance tasks to keep OMG state clean.
...
Creates all required OMG directories.
...
Sets environment variables for OMG initialization.

**Returns:** Array of environment variable names set.

**Note:** Only works if `process.env.GEMINI_ENV_FILE` is set.
...
- Environment variable setting requires `GEMINI_ENV_FILE` to be set
