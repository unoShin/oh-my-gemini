...
import { getGeminiConfigDir, getGlobalOmgConfigCandidates } from '../../utils/paths.js';
import {
  readUltraworkState,
  writeUltraworkState,
...
import { resolveToWorktreeRoot, resolveSessionStatePath, getOmgRoot } from '../../lib/worktree-paths.js';
...
export function getIdleNotificationCooldownSeconds(): number {
  for (const configPath of getGlobalOmgConfigCandidates('config.json')) {
    try {
...
function checkArchitectApprovalInTranscript(sessionId: string): boolean {
  ...
  const possiblePaths = [
    join(geminiDir, 'sessions', sessionId, 'transcript.md'),
    join(geminiDir, 'sessions', sessionId, 'messages.json'),
    join(geminiDir, 'transcripts', `${sessionId}.md`)
  ];
  ...
  if (verificationState?.pending) {
    ...
    if (sessionId) {
      ...
      if (checkArchitectApprovalInTranscript(sessionId)) {
        ...
        clearRalphState(workingDir, sessionId);
        deactivateUltrawork(workingDir, sessionId);
        const criticLabel = verificationState.critic_mode === 'gemini'
          ? 'Gemini critic'
...
4. When FULLY complete (after ${state.critic_mode === 'gemini' ? 'Gemini critic' : state.critic_mode === 'critic' ? 'Critic' : 'Architect'} verification), run \`/oh-my-gemini:cancel\` to cleanly exit and clean up state files. If cancel fails, retry with \`/oh-my-gemini:cancel --force\`.
...
function readStopBreaker(directory: string, name: string, sessionId?: string, ttlMs?: number): number {
  const stateDir = sessionId
    ? join(getOmgRoot(directory), 'state', 'sessions', sessionId)
    : join(getOmgRoot(directory), 'state');
...
When done, run \`/oh-my-gemini:cancel\` to cleanly exit.
...
export async function checkPersistentModes(
  ...
  // Blocking these causes a deadlock where Gemini Code cannot compact or exit.
  ...
  // When the API returns 429 / quota-exhausted, Gemini Code stops the session.
  // Blocking these stops creates an infinite retry loop: the hook injects a
  // continuation prompt → Gemini hits the rate limit again → stops again → loops.
