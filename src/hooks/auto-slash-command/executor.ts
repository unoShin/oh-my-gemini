...
import { formatOmgCliInvocation, rewriteOmgCliInvocations } from '../../utils/omg-cli-rendering.js';
...
const GEMINI_CONFIG_DIR = getGeminiConfigDir();

/**
 * Gemini Code native commands that must not be shadowed by user skills.
...
export function discoverAllCommands(): CommandInfo[] {
  const userCommandsDir = join(GEMINI_CONFIG_DIR, 'commands');
  const projectCommandsDir = join(process.cwd(), '.gemini', 'commands');
  const projectOmgSkillsDir = join(process.cwd(), '.omg', 'skills');
  const projectAgentSkillsDir = join(process.cwd(), '.agents', 'skills');
  const userSkillsDir = join(GEMINI_CONFIG_DIR, 'skills');

  const userCommands = discoverCommandsFromDir(userCommandsDir, 'user');
  const projectCommands = discoverCommandsFromDir(projectCommandsDir, 'project');
  const projectOmgSkills = discoverSkillsFromDir(projectOmgSkillsDir);
  ...
  // Priority: project commands > user commands > project OMG skills > project compatibility skills > user skills > builtin skills
  const prioritized = [
    ...projectCommands,
    ...userCommands,
    ...projectOmgSkills,
...
function renderDeepInterviewAutoresearchGuidance(args: string): string {
  ...
  const lines = [
    ...
    `This deep-interview invocation was launched as the zero-learning-curve setup lane for \`${formatOmgCliInvocation('autoresearch')}\`.`,
    '',
    ...
    `  \`${formatOmgCliInvocation('autoresearch --mission "<mission>" --eval "<evaluator>" [--keep-policy <policy>] [--slug <slug>]')}\``,
    '- Do **not** hand off to `omg-plan`, `autopilot`, `ralph`, or `team` in this mode.',
  ];
...
function formatCommandTemplate(cmd: CommandInfo, args: string): string {
  ...
  const injectedContent = rewriteOmgCliInvocations(resolveLiveData(resolvedContent));
...
export function executeSlashCommand(parsed: ParsedSlashCommand): ExecuteResult {
  ...
  if (!command) {
    return {
      success: false,
      error: `Command "/${parsed.command}" not found. Available commands are in $GEMINI_CONFIG_DIR/commands/ (or ~/.gemini/commands/ by default) or .gemini/commands/`,
    };
  }
