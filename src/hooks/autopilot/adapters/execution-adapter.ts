...
- Simple tasks (single file, config): \`executor\` with \`model="flash"\`
- Standard implementation: \`executor\` with \`model="pro"\`
- Complex work (architecture, refactoring): \`executor\` with \`model="ultra"\`
- Build issues: \`debugger\` with \`model="pro"\`
- Test creation: \`test-engineer\` with \`model="pro"\`
- UI work: \`designer\` with \`model="pro"\`
...
Task(subagent_type="oh-my-gemini:executor", model="flash", prompt="...")

// For standard implementation (feature, multiple methods)
Task(subagent_type="oh-my-gemini:executor", model="pro", prompt="...")

// For complex work (architecture, debugging, refactoring)
Task(subagent_type="oh-my-gemini:executor", model="ultra", prompt="...")
\`\`\`
