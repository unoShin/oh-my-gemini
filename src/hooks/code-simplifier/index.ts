...
import { getGlobalOmgConfigCandidates } from '../../utils/paths.js';
 ...
 */
export function readOmgConfig(): OmgGlobalConfig | null {
  for (const configPath of getGlobalOmgConfigCandidates('config.json')) {
    ...
    try {
      return JSON.parse(readFileSync(configPath, 'utf-8')) as OmgGlobalConfig;
    } catch {
      return null;
    }
  }
...
Use: Task(subagent_type="oh-my-gemini:code-simplifier", prompt="Simplify the recently modified files:\\n${fileArgs}")`;
}
