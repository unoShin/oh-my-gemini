...
export interface HookOutput {
  continue: boolean;
  /** System message for context injection (Gemini Code compatible) */
...
export function getCheckpointPath(directory: string): string {
  const checkpointDir = join(getOmgRoot(directory), "state", CHECKPOINT_DIR);
...
export async function exportWisdomToNotepad(
  ...
  const notepadsDir = join(getOmgRoot(directory), "notepads");
...
function readTodoSummary(directory: string): {
  ...
  const todoPaths = [
    ...
    join(getOmgRoot(directory), "state", "todos.json"),
  ];
 ...
 * Queries .omg/state/jobs.db for Gemini/Gemini job statuses
 */
...
export function formatCompactSummary(checkpoint: CompactCheckpoint): string {
  ...
  if (jobs && (jobs.active.length > 0 || jobs.recent.length > 0)) {
    lines.push("## Background Jobs (Gemini/Gemini)");
    lines.push("");
