 ...
 */
export function getOmgProgressPath(directory: string): string {
  return join(getOmgRoot(directory), PROGRESS_FILENAME);
}
...
export function findProgressPath(directory: string): string | null {
  ...
  const omgPath = getOmgProgressPath(directory);
...
export function initProgress(directory: string): boolean {
  ...
  const progressPath = getOmgProgressPath(directory);
...
export function appendProgress(
  ...
  if (!progressPath) {
    ...
    progressPath = getOmgProgressPath(directory);
  }
