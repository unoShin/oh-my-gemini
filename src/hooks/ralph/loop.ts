...
export const RALPH_CRITIC_MODES = ['architect', 'critic', 'gemini'] as const;
...
export function createRalphLoopHook(directory: string): RalphLoopHook {
  const startLoop = (
    ...
    if (isUltraQAActive(directory, sessionId)) {
      console.error(
        "Cannot start Ralph Loop while UltraQA is active. Cancel UltraQA first with /oh-my-gemini:cancel.",
      );
      return false;
    }
