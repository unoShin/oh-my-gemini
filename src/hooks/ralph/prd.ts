 ...
 */
export function getOmgPrdPath(directory: string): string {
  return join(getOmgRoot(directory), PRD_FILENAME);
}
...
export function findPrdPath(directory: string): string | null {
  ...
  const omgPath = getOmgPrdPath(directory);
  if (existsSync(omgPath)) {
    return omgPath;
  }
...
export function writePrd(directory: string, prd: PRD): boolean {
  ...
  if (!prdPath) {
    ...
    prdPath = getOmgPrdPath(directory);
  }
...
5. If ALL stories are done, run \`/oh-my-gemini:cancel\` to cleanly exit ralph mode and clean up all state files
