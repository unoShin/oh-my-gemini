 ...
 * Checks: .omg/config.json → ~/.gemini/.omg-config.json → default (warn)
 */
function getEnforcementLevel(directory: string): EnforcementLevel {
  ...
  const localConfig = path.join(getOmgRoot(directory), 'config.json');
  const globalConfig = path.join(getGeminiConfigDir(), '.omg-config.json');
...
export function processOrchestratorPreTool(input: ToolExecuteInput): ToolExecuteOutput {
  ...
  // Gemini Code sends file_path (snake_case) for Write/Edit tools and notebook_path for NotebookEdit.
 ...
 */
export function createOmgOrchestratorHook(directory: string) {
  return {
    /**
