...
export const interopSendTaskTool: ToolDefinition<{
  ...
  description: 'Send a task to the other tool (OMG -> OMX or OMX -> OMG) for execution. The task will be queued in shared state for the target tool to pick up.',
  schema: {
  ...
  handler: async (args) => {
    ...
    try {
      ...
      const tasks = readSharedTasks(cwd, {
        source: source as 'omg' | 'omx' | undefined,
...
export const interopListOmxTeamsTool: ToolDefinition<{
  ...
  description: 'List active OMX (oh-my-gemini) teams from .omx/state/team/. Shows team names and basic configuration.',
  schema: {
    workingDirectory: z.string().optional().describe('Working directory (defaults to cwd)'),
  },
  handler: async (args) => {
