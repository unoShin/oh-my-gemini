...
export function buildAgentRegistry(agents: AgentConfig[]): string {
  ...
  if (tieredAgents.length > 0) {
    ...
    lines.push('- **HIGH tier (ultra)**: Complex analysis, architecture, debugging');
    lines.push('- **MEDIUM tier (pro)**: Standard tasks, moderate complexity');
    lines.push('- **LOW tier (flash)**: Simple lookups, trivial operations');
    lines.push('');
...
export function buildToolSelectionSection(agents: AgentConfig[]): string {
  ...
  for (const [category, categoryAgents] of categorizedAgents) {
    ...
    for (const agent of categoryAgents) {
      lines.push(`**${agent.name}** (${agent.model || 'pro'}):`);
