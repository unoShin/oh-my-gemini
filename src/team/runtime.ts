...
export async function monitorTeam(teamName: string, cwd: string, workerPaneIds: string[]): Promise<TeamSnapshot> {
  ...
  for (let i = 0; i < workerPaneIds.length; i++) {
    ...
    // Note: CLI workers (gemini/gemini) may not write heartbeat.json — stall is advisory only
  }
...
export async function shutdownTeam(
  ...
  // CLI workers (gemini-cli) never write shutdown-ack.json.
  // Detect CLI worker teams and skip ACK polling.
  const CLI_AGENT_TYPES = new Set<string>(['gemini']);
