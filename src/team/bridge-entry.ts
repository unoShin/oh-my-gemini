 ...
 */
export function validateConfigPath(configPath: string, homeDir: string, geminiConfigDir: string): boolean {
  ...
  const normalizedConfigDir = resolve(geminiConfigDir);
  const normalizedOmgDir = resolve(homeDir, '.omg');
  ...
  const isTrustedSubpath =
    ...
    resolved === normalizedOmgDir ||
    resolved.startsWith(normalizedOmgDir + '/') ||
    hasOmgComponent;
...
function main(): void {
  ...
  if (!validateConfigPath(configPath, home, geminiConfigDir)) {
    console.error(`Config path must be under ~/ with ${geminiConfigDir} or ~/.omg/ subpath: ${configPath}`);
    process.exit(1);
  }
