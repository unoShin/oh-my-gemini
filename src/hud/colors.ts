 ...
 * - Ultra: Magenta (high-powered)
 * - Pro: Yellow (standard)
 * - Flash: Green (lightweight)
 */
export function getModelTierColor(model: string | undefined): string {
  ...
  if (tier.includes('ultra')) return MAGENTA;
  if (tier.includes('pro')) return YELLOW;
  if (tier.includes('flash')) return GREEN;
