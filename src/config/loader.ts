...
export function loadEnvConfig(): Partial<PluginConfig> {
  ...
  const aliasKeys = ["FLASH", "PRO", "ULTRA"] as const;
  ...
  if (process.env.OMG_DELEGATION_ROUTING_DEFAULT_PROVIDER) {
    ...
    if (["gemini", "gemini", "gemini"].includes(provider)) {
      config.delegationRouting = {
        ...config.delegationRouting,
        defaultProvider: provider as "gemini" | "gemini" | "gemini",
      };
    }
  }
...
function looksLikeOmgGuidance(content: string): boolean {
  return (
    content.includes("<guidance_schema_contract>") &&
    /oh-my-(gemini-cli|gemini)/i.test(content) &&
    OMG_STARTUP_COMPACTABLE_SECTIONS.some(
      (section) =>
...
export function compactOmgStartupGuidance(content: string): string {
      ...
      routing: {
        ...
        properties: {
          ...
          forceInherit: {
            ...
            default: false,
            description:
              "Force all agents to inherit the parent model, bypassing OMG model routing. When true, no model parameter is passed to Task/Agent calls, so agents use the user's Gemini Code model setting. Auto-enabled for non-Gemini providers (CC Switch, custom ANTHROPIC_BASE_URL), AWS Bedrock, and Google Vertex AI.",
          },
        },
      },
      externalModels: {
      ...
      delegationRouting: {
        ...
        properties: {
          enabled: {
            ...
            default: false,
            description:
              "Enable delegation routing to external providers (Gemini, Gemini)",
          },
          defaultProvider: {
            type: "string",
