...
function countIncompleteTasks(sessionId) {
  ...
  const taskDir = join(homedir(), ".gemini", "tasks", sessionId);
...
async function main() {
  try {
      ...
      if (sessionMatches) {
        ...
        console.log(
          JSON.stringify({
            continue: false,
            decision: "block",
            reason: `[RALPH LOOP - EXTENDED] Max iterations reached; extending to ${ralph.state.max_iterations} and continuing. When FULLY complete (after Architect verification), run /oh-my-gemini:cancel (or --force).`,
          }),
        );
        return;
      }
    ...
    // Priority 6: Team (omg-teams / staged pipeline)
    if (
      team.state?.active &&
    ...
    try {
      ...
      // If stdout write fails, the hook will timeout and Gemini Code will proceed
