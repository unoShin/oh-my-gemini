...
async function main() {
  try {
    ...
    const omgTeams = readStateFileWithSession(
      stateDir,
      globalStateDir,
    ...
    if (
      ...
      if (sessionMatches) {
        ...
        const ralphExtendedReason = `[RALPH LOOP - EXTENDED] Max iterations reached; extending to ${ralph.state.max_iterations} and continuing. When FULLY complete (after Architect verification), run /oh-my-gemini:cancel (or --force).`;
        console.log(
          JSON.stringify({
            decision: "block",
    ...
    if (
      ...
      if (incomplete > 0) {
        ...
        if (newCount <= 20) {
          ...
          let reason = `[ULTRAPILOT] ${incomplete} workers still running. Continue working. When all workers complete, run /oh-my-gemini:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-gemini:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
    ...
    if (
      ...
      if (pending > 0) {
        ...
        if (newCount <= 15) {
          ...
          let reason = `[SWARM ACTIVE] ${pending} tasks remain. Continue working. When all tasks are done, run /oh-my-gemini:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-gemini:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
    ...
    if (
      ...
      if (currentStage < totalStages) {
        ...
        if (newCount <= 15) {
          ...
          let reason = `[PIPELINE - Stage ${currentStage + 1}/${totalStages}] Pipeline not complete. Continue working. When all stages complete, run /oh-my-gemini:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-gemini:cancel --force.`;
          if (errorGuidance) {
            reason = errorGuidance + reason;
          }
    ...
    // Priority 6: Team (native Gemini Code teams / staged pipeline)
    if (
      team.state?.active &&
      ...
      if (sessionMatches) {
        ...
        if (phase) {
          ...
          if (newCount <= 20) {
            ...
            let reason = `[TEAM - Phase: ${phase}] Team mode active. Continue working. When all team tasks complete, run /oh-my-gemini:cancel to cleanly exit. If cancel fails, retry with /oh-my-gemini:cancel --force.`;
            if (errorGuidance) {
              reason = errorGuidance + reason;
            }
    ...
    if (
      omgTeams.state?.active &&
      !isStaleState(omgTeams.state) &&
      isStateForCurrentProject(omgTeams.state, directory, omgTeams.isGlobal)
    ) {
      const sessionMatches = hasValidSessionId
        ? omgTeams.state.session_id === sessionId
        : !omgTeams.state.session_id || omgTeams.state.session_id === sessionId;
      if (sessionMatches) {
        ...
        if (phase) {
          ...
          if (newCount <= 20) {
            ...
            let reason = `[OMG TEAMS - Phase: ${phase}] OMG Teams workers active. Continue working. When all workers complete, run /oh-my-gemini:cancel to cleanly exit. If cancel fails, retry with /oh-my-gemini:cancel --force.`;
            if (errorGuidance) {
              reason = errorGuidance + reason;
            }
    ...
    if (
      ...
      if (cycle < maxCycles && !ultraqa.state.all_passing) {
        ...
        let reason = `[ULTRAQA - Cycle ${cycle + 1}/${maxCycles}] Tests not all passing. Continue fixing. When all tests pass, run /oh-my-gemini:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-gemini:cancel --force.`;
        if (errorGuidance) {
          reason = errorGuidance + reason;
        }
