...
function activateState(directory, prompt, stateName, sessionId) {
  ...
  // Fallback: write to legacy local .omg/state directory
...
/**
 * Check if the team feature is enabled in Gemini Code settings.
 * Reads ~/.gemini/settings.json and checks for GEMINI_CODE_EXPERIMENTAL_AGENT_TEAMS env var.
...
function isTeamEnabled() {
  try {
    ...
    if (existsSync(settingsPath)) {
      ...
      if (settings.env?.GEMINI_CODE_EXPERIMENTAL_AGENT_TEAMS === '1' ||
          settings.env?.GEMINI_CODE_EXPERIMENTAL_AGENT_TEAMS === 'true') {
        return true;
      }
    }
    // Fallback: check env var (for dev/CI environments)
    if (process.env.GEMINI_CODE_EXPERIMENTAL_AGENT_TEAMS === '1' ||
        process.env.GEMINI_CODE_EXPERIMENTAL_AGENT_TEAMS === 'true') {
      return true;
    }
    return false;
  } catch {
...
/**
 * Create a skill invocation message that tells Gemini to use the Skill tool
 */
...
Skill: oh-my-gemini:${skillName}${argsSection}
...
Skill: oh-my-gemini:${s.name}${argsSection}`;
  }).join('\n\n');
...
/**
 * Create proper hook output with additionalContext (Gemini Code hooks API)
...
async function main() {
  ...
  if (process.env.DISABLE_OMG === '1' || _skipHooks.includes('keyword-detector')) {
  ...
  try {
    ...
    // Cancel keywords
    if (hasActionableKeyword(cleanPrompt, /\b(cancelomg|stopomg)\b/i)) {
    ...
    // This prevents infinite spawning when Gemini workers receive prompts containing "team".


    // CCG keywords (Gemini-Gemini-Gemini tri-model orchestration)
    if (hasActionableKeyword(cleanPrompt, /\b(ccg|gemini-clix-gemini)\b|(씨씨지)/i)) {
    ...
    if (followupPlanner && planningArtifacts) {
      ...
      const ralplanStatePath = sessionId && /^[a-zA-Z0-9][a-zA-Z0-9_-]{0,255}$/.test(sessionId)
        ? join(directory, '.omg', 'state', 'sessions', sessionId, 'ralplan-state.json')
        : join(directory, '.omg', 'state', 'ralplan-state.json');
