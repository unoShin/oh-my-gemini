...
const AGENT_OUTPUT_ANALYSIS_LIMIT = parseInt(process.env.OMG_AGENT_OUTPUT_ANALYSIS_LIMIT || '12000', 10);
const AGENT_OUTPUT_SUMMARY_LIMIT = parseInt(process.env.OMG_AGENT_OUTPUT_SUMMARY_LIMIT || '360', 10);
...
// Debug logging helper - gated behind OMG_DEBUG env var
const debugLog = (...args) => {
  if (process.env.OMG_DEBUG) console.error('[omg:debug:post-tool-verifier]', ...args);
};
...
// Pattern to match Gemini Code temp CWD permission errors (false positives on macOS)
// e.g. "zsh:1: permission denied: /var/folders/.../T/gemini-abc123-cwd"
const GEMINI_TEMP_CWD_PATTERN = /zsh:\d+: permission denied:.*\/T\/gemini-[a-z0-9]+-cwd/gi;

// Strip Gemini Code temp CWD noise before pattern matching
function stripGeminiTempCwdErrors(output) {
  return output.replace(GEMINI_TEMP_CWD_PATTERN, '');
}

// Pattern matching Gemini Code's "Error: Exit code N" prefix line
// Note: no /g flag — module-level regex with /g is stateful (.lastIndex persists across calls)
const GEMINI_EXIT_CODE_PREFIX = /^Error: Exit code \d+\s*$/m;

/**
 * Detect non-zero exit code with valid stdout (issue #960).
 * Returns true when output has Gemini Code's "Error: Exit code N" prefix
...
export function isNonZeroExitWithOutput(output) {
  ...
  const cleaned = stripGeminiTempCwdErrors(output);

  // Must contain Gemini Code's exit code prefix
  if (!GEMINI_EXIT_CODE_PREFIX.test(cleaned)) return false;

  // Strip exit code prefix line(s) and check remaining content
  const remaining = cleaned.replace(GEMINI_EXIT_CODE_PREFIX, '').trim();
  ...
  try {
    if (existsSync(trackingFile)) {
      ...
      if (quietLevel < 2 && running.length > 0) {
        parts.push(`Running: ${running.length} [${running.map(a => a.agent_type.replace('oh-my-gemini:', '')).join(', ')}]`);
      }
...
async function main() {
  ...
  if (process.env.DISABLE_OMG === '1' || _skipHooks.includes('post-tool-use')) {
