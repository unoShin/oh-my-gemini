...
/** Gemini config directory (respects GEMINI_CONFIG_DIR env var) */
...
function getRuntimeBaseDir() {
  return process.env.GEMINI_PLUGIN_ROOT || join(__dirname, '..');
}
...
// Extract OMG version from GEMINI.md content
function extractOmgVersion(content) {
  const match = content.match(/<!-- OMG:VERSION:(\d+\.\d+\.\d+[^\s]*?) -->/);
  return match ? match[1] : null;
}

// Get plugin version from GEMINI_PLUGIN_ROOT
...
// Get GEMINI.md version
function getGeminiMdVersion() {
  try {
    const geminiMdPath = join(configDir, 'GEMINI.md');
    if (!existsSync(geminiMdPath)) return null;  // File doesn't exist
    const content = readFileSync(geminiMdPath, 'utf-8');
    const version = extractOmgVersion(content);
...
function detectVersionDrift() {
  ...
  const geminiMdVersion = getGeminiMdVersion();
  ...
  if (npmVersion && npmVersion !== pluginVersion) {
    drift.push({ component: 'npm package (omg CLI)', current: npmVersion, expected: pluginVersion });
  }

  if (geminiMdVersion === 'unknown') {
    drift.push({
      component: 'GEMINI.md instructions',
      current: 'unknown (needs migration)',
      expected: pluginVersion
    });
  } else if (geminiMdVersion && geminiMdVersion !== pluginVersion) {
    drift.push({
      component: 'GEMINI.md instructions',
      current: geminiMdVersion,
      expected: pluginVersion
    });
  }
  ...
  return { pluginVersion, npmVersion, geminiMdVersion, drift };
}
...
function shouldNotifyDrift(driftInfo) {
  ...
  const driftKey = `plugin:${driftInfo.pluginVersion}-npm:${driftInfo.npmVersion}-gemini:${driftInfo.geminiMdVersion}`;
...
async function checkNpmUpdate(currentVersion) {
  ...
  try {
    const response = await fetch('https://registry.npmjs.org/oh-my-gemini/latest', {
...
async function checkHudInstallation(retryCount = 0) {
  ...
  const hudScriptOmg = join(hudDir, 'omg-hud.mjs');
  ...
  const hudScriptExists = existsSync(hudScriptOmg) || existsSync(hudScriptLegacy);
  if (!hudScriptExists) {
  ...
  try {
    if (existsSync(settingsFile)) {
      ...
      // If OMG HUD wrapper is configured, ensure at least one plugin cache version is built.
...
async function main() {
  try {
    ...
    if (driftInfo && shouldNotifyDrift(driftInfo)) {
      let driftMsg = `[OMG VERSION DRIFT DETECTED]\n\nPlugin version: ${driftInfo.pluginVersion}\n`;
    ...
    try {
      ...
      if (pluginVersion) {
        ...
        if (updateInfo) {
          messages.push(`<session-restore>\n\n[OMG UPDATE AVAILABLE]\n\nA new version of oh-my-gemini is available: v${updateInfo.latestVersion} (current: v${updateInfo.currentVersion})\n\nTo update, run: omg update\n(This syncs plugin, npm package, and GEMINI.md together)\n\n</session-restore>\n\n---\n`);
        }
      }
    } catch {}

    // Warn if silentAutoUpdate is enabled but running in plugin mode (#1773)
    if (process.env.GEMINI_PLUGIN_ROOT) {
      try {
        const omgConfigPath = join(configDir, '.omg-config.json');
        const omgConfig = readJsonFile(omgConfigPath);
        if (omgConfig?.silentAutoUpdate) {
          messages.push(`<session-restore>\n\n[OMG] silentAutoUpdate is enabled in .omg-config.json but has no effect in plugin mode.\nTo update, use: /plugin marketplace update omg && /omg-setup\nOr run manually: omg update\n\n</session-restore>\n\n---\n`);
        }
      } catch {}
    }
...
[OMG] HUD not configured (${hudCheck.reason}). Run /hud setup then restart Gemini Code.
</system-reminder>`);
    }
...
`);
    ...
    // Check for incomplete todos (project-local only, not global ~/.gemini/todos/)
...
${cleanContent}
    ...
    // plugin update whose GEMINI_PLUGIN_ROOT still points to the old version.
    try {
      const cacheBase = join(configDir, 'plugins', 'cache', 'omg', 'oh-my-gemini');
