 ...
 *   /abs/path/to/node "${GEMINI_PLUGIN_ROOT}/scripts/run.cjs" \
 *       "${GEMINI_PLUGIN_ROOT}/scripts/<hook>.mjs" [args...]
 *
...
if (!target) {
  // Nothing to run — exit cleanly so Gemini Code hooks are never blocked.
  process.exit(0);
}

/**
 * Resolve the hook script target path, handling stale GEMINI_PLUGIN_ROOT.
 *
 ...
 * GEMINI_PLUGIN_ROOT will fail with MODULE_NOT_FOUND.
 *
 * Resolution strategy:
 ...
 *
 * See: https://github.com/unoShin/oh-my-gemini/issues/1007
 */
function resolveTarget(targetPath) {
  ...
  // GEMINI_PLUGIN_ROOT is e.g. ~/.gemini/plugins/cache/omg/oh-my-gemini/4.2.14
  // We look one level up for sibling version directories.
  try {
    const pluginRoot = process.env.GEMINI_PLUGIN_ROOT;
    if (!pluginRoot) return null;

    const cacheBase = dirname(pluginRoot);          // .../oh-my-gemini/
...
if (!resolved) {
  ...
  // This is the graceful fallback for stale GEMINI_PLUGIN_ROOT paths.
  process.exit(0);
}
