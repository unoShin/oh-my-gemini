...
/**
 * OMG Persistent Mode Hook (Node.js)
 * Minimal continuation enforcer for all OMG modes.
...
/**
 * Count incomplete Tasks from Gemini Code's native Task system.
 */
function countIncompleteTasks(sessionId) {
  if (!sessionId || typeof sessionId !== "string") return 0;
  if (!/^[a-zA-Z0-9][a-zA-Z0-9_-]{0,255}$/.test(sessionId)) return 0;

  const cfgDir = process.env.GEMINI_CONFIG_DIR || join(homedir(), ".gemini");
...
function countIncompleteTodos(sessionId, projectDir) {
  ...
  for (const path of [
    ...
    join(projectDir, ".gemini", "todos.json"),
  ]) {
    try {
...
async function main() {
  try {
    ...
    // Blocking these causes a deadlock where Gemini Code cannot compact.
    // See: https://github.com/unoShin/oh-my-gemini/issues/213
    ...
    const omgTeams = readStateFileWithSession(stateDir, "omg-teams-state.json", sessionId);
    ...
    if (ralph.state?.active && !isAwaitingConfirmation(ralph.state) && !isStaleState(ralph.state) && isSessionMatch(ralph.state, sessionId)) {
      ...
      if (iteration < maxIter) {
        ...
        const ralphReason = `[RALPH LOOP - ITERATION ${iteration + 1}/${maxIter}] Work is NOT done. Continue working.\nWhen FULLY complete (after Architect verification), run /oh-my-gemini:cancel to cleanly exit ralph mode and clean up all state files. If cancel fails, retry with /oh-my-gemini:cancel --force.\n${ralph.state.prompt ? `Task: ${ralph.state.prompt}` : ""}`;
        console.log(
          JSON.stringify({
            decision: "block",
            reason: ralphReason,
        ...
        ralph.state.last_checked_at = new Date().toISOString();
        writeJsonFile(ralph.path, ralph.state);
        const extendReason = `[RALPH LOOP - EXTENDED] Max iterations reached; extending to ${ralph.state.max_iterations} and continuing. When FULLY complete (after Architect verification), run /oh-my-gemini:cancel (or --force).`;
      ...
      if (phase !== "complete") {
        ...
        if (newCount <= 20) {
          ...
          const cancelGuidance = typeof autopilot.state.session_id === "string" && autopilot.state.session_id === sessionId
            ? " When all phases are complete, run /oh-my-gemini:cancel to cleanly exit and clean up this session's autopilot state files. If cancel fails, retry with /oh-my-gemini:cancel --force."
            : "";
          console.log(
            JSON.stringify({
              decision: "block",
    ...
    if (ralplan.state?.active && !isAwaitingConfirmation(ralplan.state) && !isStaleState(ralplan.state) && isSessionMatch(ralplan.state, sessionId)) {
      ...
      if (!ralplanTerminal && !cancelInProgress) {
        ...
        if (breakerCount > RALPLAN_STOP_BLOCKER_MAX) {
          ...
          const ralplanReason = `[RALPLAN - CONSENSUS PLANNING | REINFORCEMENT ${breakerCount}/${RALPLAN_STOP_BLOCKER_MAX}] The ralplan consensus workflow is active. Continue the Planner/Architect/Critic loop. Do not stop until consensus is reached or the workflow completes. When done, run /oh-my-gemini:cancel to cleanly exit.`;
          console.log(JSON.stringify({
            decision: "block",
      ...
      if (incomplete > 0) {
        ...
        if (newCount <= 20) {
          ...
          console.log(
            JSON.stringify({
              decision: "block",
              reason: `[ULTRAPILOT] ${incomplete} workers still running. Continue working. When all workers complete, run /oh-my-gemini:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-gemini:cancel --force.`,
            }),
          );
          return;
        }
      ...
      if (pending > 0) {
        ...
        if (newCount <= 15) {
          ...
          console.log(
            JSON.stringify({
              decision: "block",
              reason: `[SWARM ACTIVE] ${pending} tasks remain. Continue working. When all tasks are done, run /oh-my-gemini:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-gemini:cancel --force.`,
            }),
          );
          return;
        }
      ...
      if (currentStage < totalStages) {
        ...
        if (newCount <= 15) {
          ...
          console.log(
            JSON.stringify({
              decision: "block",
              reason: `[PIPELINE - Stage ${currentStage + 1}/${totalStages}] Pipeline not complete. Continue working. When all stages complete, run /oh-my-gemini:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-gemini:cancel --force.`,
            }),
          );
          return;
        }
    ...
    // Priority 6: Team (native Gemini Code teams) — fallback for cases not handled by Priority 2.5
      ...
      if (phase) {
        ...
        if (newCount <= 20) {
          ...
          console.log(
            JSON.stringify({
              decision: "block",
              reason: `[TEAM - Phase: ${phase}] Team mode active. Continue working. When all team tasks complete, run /oh-my-gemini:cancel to cleanly exit. If cancel fails, retry with /oh-my-gemini:cancel --force.`,
            }),
          );
          return;
        }
    ...
    // Priority 6.5: OMG Teams (tmux CLI workers — independent of native team state)
    if (omgTeams.state?.active && !isStaleState(omgTeams.state) && isSessionMatch(omgTeams.state, sessionId)) {
      const phase = normalizeTeamPhase(omgTeams.state);
      if (phase) {
        const newCount = getSafeReinforcementCount(omgTeams.state.reinforcement_count) + 1;
        if (newCount <= 20) {
          omgTeams.state.reinforcement_count = newCount;
          omgTeams.state.last_checked_at = new Date().toISOString();
          writeJsonFile(omgTeams.path, omgTeams.state);

          // Fire-and-forget notification
          sendStopNotification('omg-teams', omgTeams.state, sessionId, directory).catch(() => {});

          console.log(
            JSON.stringify({
              decision: "block",
              reason: `[OMG TEAMS - Phase: ${phase}] OMG Teams workers active. Continue working. When all workers complete, run /oh-my-gemini:cancel to cleanly exit. If cancel fails, retry with /oh-my-gemini:cancel --force.`,
            }),
          );
          return;
        }
    ...
    if (ultraqa.state?.active && !isStaleState(ultraqa.state) && isSessionMatch(ultraqa.state, sessionId)) {
      ...
      if (cycle < maxCycles && !ultraqa.state.all_passing) {
        ...
        console.log(
          JSON.stringify({
            decision: "block",
            reason: `[ULTRAQA - Cycle ${cycle + 1}/${maxCycles}] Tests not all passing. Continue fixing. When all tests pass, run /oh-my-gemini:cancel to cleanly exit and clean up state files. If cancel fails, retry with /oh-my-gemini:cancel --force.`,
          }),
        );
        return;
      }
    ...
    if (
      ...
      if (totalIncomplete > 0) {
        ...
        reason += ` No incomplete tasks detected. You MUST invoke /oh-my-gemini:cancel immediately to exit ultrawork mode and clean up state files. Call state_clear(mode="ultrawork") if the cancel skill is unavailable.`;
      } else if (newCount >= 3) {
        // Suggest cancel after minimum iterations
        reason += ` If all work is complete, run /oh-my-gemini:cancel to cleanly exit ultrawork mode and clean up state files. If cancel fails, retry with /oh-my-gemini:cancel --force. Otherwise, continue working.`;
      } else {
    ...
    // No blocking needed — Gemini is truly idle.
