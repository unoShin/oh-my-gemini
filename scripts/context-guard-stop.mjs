 ...
 *
 * Configurable via OMG_CONTEXT_GUARD_THRESHOLD env var (default: 75%).
 *
 * Safety rules:
...
const THRESHOLD = parseInt(process.env.OMG_CONTEXT_GUARD_THRESHOLD || '75', 10);
 ...
 * When Gemini Code runs inside .gemini/worktrees/X, the encoded project directory
 * contains `--gemini-worktrees-X` which doesn't exist. Strip it to find the real path.
 */
function resolveTranscriptPath(transcriptPath, cwd) {
  ...
  // Strategy 1: Strip Gemini worktree segment from encoded project directory
...
function getGuardFilePath(sessionId) {
  const configDir = process.env.GEMINI_CONFIG_DIR || join(homedir(), '.gemini');
  const guardDir = join(configDir, 'projects', '.omg-guards');
...
function buildStopRecoveryAdvice(contextPercent, blockCount) {
  ...
  return `[OMG ${severity}] Context at ${contextPercent}% (threshold: ${THRESHOLD}%). ` +
